...
async def radius_authenticate(
    auth_request: RadiusAuthRequest, db: Session = Depends(get_db)
):
    """
...
async def start_radius_session(
    session_start: RadiusSessionStart, db: Session = Depends(get_db)
):
    """
...
async def stop_radius_session(
    session_stop: RadiusSessionStop, db: Session = Depends(get_db)
):
    """
...
async def update_session_accounting(
    accounting_update: RadiusAccountingUpdate, db: Session = Depends(get_db)
):
    """
...
async def get_customer_sessions(
    ...
    try:
        ...
        return CustomerSessionsResponse(
            customer_id=customer_id, total_sessions=len(sessions), sessions=session_data
        )
...
@router.get(
    "/service-plan/{service_plan_id}/enforcement",
    response_model=ServiceEnforcementResponse,
)
async def get_service_plan_enforcement(
    ...
    try:
        ...
        attributes = radius_service.get_service_plan_enforcement_attributes(
            service_plan_id
        )
        ...
        return ServiceEnforcementResponse(
            service_plan_id=service_plan_id, radius_attributes=attributes
        )
...
async def get_active_sessions(
    ...
    try:
        ...
        sessions = (
            db.query(RadiusSession)
            .filter(RadiusSession.status == "active")
            .limit(limit)
            .all()
        )
...
async def get_radius_statistics(
    ...
    """
    try:
        from ....models.radius import CustomerOnline, CustomerStatistics, RadiusSession

        # Get basic statistics
        total_sessions = db.query(RadiusSession).count()
        active_sessions = (
            db.query(RadiusSession).filter(RadiusSession.status == "active").count()
        )

        customers_online = (
            db.query(CustomerOnline).filter(CustomerOnline.status == "online").count()
        )
