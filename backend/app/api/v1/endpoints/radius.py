...
async def get_radius_sessions(
    ...
    return RadiusSessionList(
        sessions=paginated_sessions, total=total, page=page, per_page=per_page
    )
...
async def get_online_customers(
    ...
    return CustomerOnlineList(
        online_customers=paginated_customers, total=total, page=page, per_page=per_page
    )
...
async def get_customer_statistics(
    ...
    if customer_id:
        ...
        start_date = end_date - timedelta(days=days)
        statistics = (
            service.repo.db.query(service.repo.model)
            .filter(
    ...
    return CustomerStatisticsList(
        statistics=paginated_stats, total=total, page=page, per_page=per_page
    )
...
async def get_network_utilization(
    ...
    return NetworkUtilization(
        ...
        average_concurrent_users=float(
            utilization["current_online_users"]
        ),  # Approximation
