...
from app.schemas.oauth import TokenIntrospectionResponse
...
async def oauth_token(
    ...
    if not final_client_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Client ID is required"
        )
    ...
    try:
        if grant_type == "password":
            ...
            token_data = oauth_service.password_grant_flow(
                client=client, username=username, password=password, scope=scope
            )
