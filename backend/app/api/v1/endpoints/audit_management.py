...
from app.services.audit_processor import (
    audit_processor_manager,
    get_audit_processing_health,
)
...
async def get_audit_queue_stats(
    ...
    try:
        ...
        stats["recent_activity"] = {
            "last_hour": db.query(AuditQueue)
            .filter(AuditQueue.created_at >= last_hour)
            .count(),
            "last_day": db.query(AuditQueue)
            .filter(
                AuditQueue.created_at >= datetime.now(timezone.utc) - timedelta(days=1)
            )
            .count(),
        }
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve audit queue statistics"
        )
...
async def retry_audit_entry(
    ...
    try:
        ...
        if entry.status != "failed":
            raise HTTPException(
                status_code=400, detail="Only failed entries can be retried"
            )

        if not entry.can_retry():
            raise HTTPException(
                status_code=400, detail="Entry has exceeded maximum retry attempts"
            )
...
@router.get(
    "/configuration-snapshots", response_model=List[ConfigurationSnapshotResponse]
)
async def get_configuration_snapshots(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve configuration snapshots"
        )
...
async def create_configuration_snapshot(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail="Failed to create configuration snapshot"
        )
...
async def activate_configuration_snapshot(
    ...
    try:
        ...
        if not snapshot:
            raise HTTPException(
                status_code=404, detail="Configuration snapshot not found"
            )

        # Deactivate other snapshots for this config
        db.query(ConfigurationSnapshot).filter_by(
            config_type=snapshot.config_type, config_key=snapshot.config_key
        ...
        return {
            "message": "Configuration snapshot activated",
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail="Failed to activate configuration snapshot"
        )
...
async def get_cdc_log_stats(
    ...
    try:
        ...
        operation_stats = (
            db.query(CDCLog.operation, func.count(CDCLog.id).label("count"))
        ...
        # Get table activity
        table_stats = (
            db.query(CDCLog.table_name, func.count(CDCLog.id).label("count"))
            .filter(CDCLog.timestamp >= cutoff_time)
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve CDC log statistics"
        )
...
async def get_audit_processors(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve audit processors"
        )
...
async def control_audit_processor(
    ...
    try:
        if control_request.action == "start":
            ...
            background_tasks.add_task(
                audit_processor_manager.stop_processor, processor_name
            )
...
async def get_processor_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve processor statistics"
        )
...
async def get_audit_health(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve audit health status"
        )
...
async def retry_failed_entries(
    ...
    try:
        retry_count = await audit_processor_manager.retry_failed_entries(
            db, max_retries
        )
        return {
            "message": f"Queued {retry_count} failed entries for retry",
...
async def cleanup_old_entries(
    ...
    try:
        deleted_count = await audit_processor_manager.cleanup_old_entries(
            db, days_to_keep
        )
        return {
...
async def get_audit_dashboard(
    ...
    try:
        ...
        recent_activity = {
            "audit_entries": db.query(AuditQueue)
            .filter(AuditQueue.created_at >= last_24h)
            .count(),
            "cdc_entries": db.query(CDCLog)
            .filter(CDCLog.timestamp >= last_24h)
            .count(),
            "config_snapshots": db.query(ConfigurationSnapshot)
            .filter(ConfigurationSnapshot.created_at >= last_24h)
            .count(),
        }
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve audit dashboard data"
        )
