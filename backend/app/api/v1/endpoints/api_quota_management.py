...
async def aggregate_daily_usage(
    target_date: Optional[date] = Query(
        None, description="Date to aggregate (defaults to today)"
    ),
    current_admin: Administrator = Depends(get_current_admin),
    db: Session = Depends(get_db),
):
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} triggered usage aggregation for {target_date or 'today'}"
        )
...
async def get_reseller_usage_summary(
    reseller_id: int,
    period_days: int = Query(
        30, ge=1, le=365, description="Period in days for summary"
    ),
    current_admin: Administrator = Depends(get_current_admin),
    db: Session = Depends(get_db),
):
...
async def get_quota_violation_alerts(
    hours: int = Query(
        24, ge=1, le=168, description="Hours to look back for violations"
    ),
    current_admin: Administrator = Depends(get_current_admin),
    db: Session = Depends(get_db),
):
...
async def reset_api_key_quota(
    ...
    quota_type: str = Query(
        ..., pattern="^(daily|monthly)$", description="Type of quota to reset"
    ),
    current_admin: Administrator = Depends(get_current_admin),
    db: Session = Depends(get_db),
):
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} reset {quota_type} quota for API key {api_key[:8]}..."
        )
