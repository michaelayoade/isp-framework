    ...
    try:
        invoice = invoice_service.create_invoice(
            ...
            invoice_data=invoice_data.dict(
                exclude={"billing_account_id", "invoice_items"}
            ),
...
async def create_payment_method(
    ...
    try:
        payment_method = payment_service.create_payment_method(
            ...
            method_data=payment_method_data.dict(
                exclude={"billing_account_id", "method_type"}
            ),
        )
...
@router.get(
    "/accounts/{account_id}/payment-methods", response_model=List[PaymentMethodResponse]
)
...
async def create_credit_note(
    ...
    try:
        ...
        credit_note = CreditNote(credit_number=credit_number, **credit_note_data.dict())
...
@router.get(
    "/accounts/{account_id}/credit-notes", response_model=List[CreditNoteResponse]
)
...
@router.get(
    "/accounts/{account_id}/payment-plans", response_model=List[PaymentPlanResponse]
)
...
async def get_billing_overview(
    ...
    billing_type: Optional[BillingType] = Query(
        None, description="Filter by billing type"
    ),
    db: Session = Depends(get_db),
    current_admin: Administrator = Depends(get_current_admin),
):
...
async def get_account_transactions(
    ...
    start_date: Optional[datetime] = Query(
        None, description="Start date for transactions"
    ),
    ...
    try:
        from app.repositories.billing_repository import (
            get_billing_transaction_repository,
        )
        ...
        if start_date and end_date:
            ...
            transactions = transaction_repo.get_by_transaction_type(
