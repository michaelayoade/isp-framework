...
@router.post(
    "/customers/{customer_id}/services/internet",
...
async def create_internet_service(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} created Internet service {service.id} for customer {customer_id}"
        )
        return service
...
@router.get(
    "/customers/{customer_id}/services/internet", response_model=InternetServiceList
)
...
async def update_internet_service(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} updated Internet service {service_id}"
        )
        return service
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
...
@router.post("/services/internet/{service_id}/provision", response_model=dict)
async def provision_internet_service(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} queued provisioning for Internet service {service_id}"
        )
        return {
...
async def suspend_internet_service(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} suspended Internet service {service_id}"
        )
        return {
...
async def reactivate_internet_service(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} reactivated Internet service {service_id}"
        )
...
async def list_all_internet_services(
    ...
    try:
        result = internet_service.list_all_services(
            page=page, per_page=per_page, status=status, tariff_id=tariff_id
        )
        return result
    except Exception as e:
