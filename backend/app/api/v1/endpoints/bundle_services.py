...
@router.post(
    "/customers/{customer_id}/services/bundle",
...
async def create_bundle_service(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} created Bundle service {service.id} for customer {customer_id}"
        )
        return service
...
@router.get(
    "/customers/{customer_id}/services/bundle", response_model=BundleServiceList
)
...
async def update_bundle_service(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} updated Bundle service {service_id}"
        )
        return service
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
...
@router.post("/services/bundle/{service_id}/provision", response_model=dict)
async def provision_bundle_service(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} queued provisioning for Bundle service {service_id}"
        )
        return {
...
async def suspend_bundle_service(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} suspended Bundle service {service_id}"
        )
        return {
...
async def reactivate_bundle_service(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} reactivated Bundle service {service_id}"
        )
...
async def list_all_bundle_services(
    ...
    try:
        result = bundle_service.list_all_services(
            page=page, per_page=per_page, status=status, package_id=package_id
        )
        return result
    except Exception as e:
