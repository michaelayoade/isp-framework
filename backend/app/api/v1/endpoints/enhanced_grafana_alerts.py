...
async def create_error_handling_alert_rules(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to create error handling alert rules"
        )
...
async def create_operational_dashboards(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to create operational dashboards"
        )
...
async def get_alert_effectiveness_report(
    ...
    try:
        ...
        report = {
            ...
            "actionable_alerts_percentage": effectiveness.get(
                "actionable_alerts_percentage", 0
            ),
        ...
        if effectiveness.get("false_positive_rate", 0) > 10:
            report["recommendations"].append(
                "Consider tuning alert thresholds to reduce false positives"
            )

        if effectiveness.get("actionable_alerts_percentage", 0) < 80:
            report["recommendations"].append(
                "Review alert rules to ensure they trigger actionable alerts"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve alert effectiveness report"
        )
...
async def test_alert_integration(
    ...
    try:
        ...
        test_alert = {
            ...
            "source": "api_test",
            "metadata": {"test": True, "initiated_by": current_admin.id},
        }
    ...
    try:
        ...
        status = {
            ...
            "critical_alerts": len(
                [
                    a
                    for a in system_health.get("alerts", [])
                    if a.get("severity") == "critical"
                ]
            ),
            "recent_alerts_24h": alert_stats.get("alerts_24h", 0),
            "resolution_rate": metrics.get("resolution_metrics", {}).get(
                "resolution_rate", 0
            ),
            "components": {
                "dead_letter_queue": system_health.get("components", {}).get(
                    "dead_letter_queue", {}
                ),
                "task_execution": system_health.get("components", {}).get(
                    "task_execution", {}
                ),
                "customer_services": system_health.get("components", {}).get(
                    "customer_services", {}
                ),
            },
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve alert system status"
        )
