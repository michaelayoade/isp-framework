...
async def get_file_download_url(
    file_id: int,
    expires_in: int = Query(3600, le=86400, description="URL expiration time in seconds"),
    db: Session = Depends(get_db),
    current_admin: Administrator = Depends(get_current_admin)
):
    ...
    try:
        ...
        if request.import_type == "customers":
            background_tasks.add_task(
                ...
                db, import_job.id, current_admin.id
            )
        
        return ImportJobResponse.from_orm(import_job)
    except Exception as e:
        logger.error(f"Failed to create import job: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Import job creation failed: {str(e)}")
...
async def list_import_jobs(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Import job listing failed: {str(e)}")
...
async def get_import_job(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Import job retrieval failed: {str(e)}")
...
async def process_import_job(
    ...
    try:
        ...
        if job.status != ImportStatus.PENDING:
            raise HTTPException(status_code=400, detail="Import job is not in pending status")
        
        # Update job with mapping and start processing
        repo.update_import_job(job_id, {
        ...
        background_tasks.add_task(
            ...
            db, job_id, mapping.column_mapping, current_admin.id
        )
...
async def create_export_job(
    ...
    try:
        ...
        background_tasks.add_task(
            _process_export_job,
            db, export_job.id, current_admin.id
        )
...
async def list_export_jobs(
    ...
    try:
        ...
        total_count = repo.count_export_jobs(
            status=status,
            export_type=export_type
        )
        ...
        raise HTTPException(status_code=500, detail=f"Export job listing failed: {str(e)}")
...
async def get_export_job(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Export job retrieval failed: {str(e)}")
...
async def _process_customer_import_with_mapping(
    ...
    try:
        ...
        logger.error(f"Failed to process customer import job {job_id} with mapping: {str(e)}")
