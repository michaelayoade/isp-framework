...
async def start_watchdog(
    check_interval_seconds: int = Query(
        300, ge=60, le=3600, description="Check interval in seconds"
    ),
    current_admin: Administrator = Depends(get_current_admin),
    db: Session = Depends(get_db),
):
...
async def get_plugin_metrics(
    ...
    period_hours: int = Query(
        24, ge=1, le=168, description="Period in hours for metrics"
    ),
    current_admin: Administrator = Depends(get_current_admin),
    db: Session = Depends(get_db),
):
...
async def get_recent_health_alerts(
    ...
    severity: Optional[str] = Query(
        None, pattern="^(warning|critical)$", description="Filter by severity"
    ),
    current_admin: Administrator = Depends(get_current_admin),
    db: Session = Depends(get_db),
):
