...
from app.schemas.customer_service import (
    CustomerServiceResponse, CustomerServiceCreate, CustomerServiceUpdate,
    CustomerServiceSearch, CustomerServiceSearchResponse, ServiceProvisioningRequest,
    ServiceProvisioningResponse, CustomerServicesOverview, ServicePlanAssignments
)
...
async def search_customer_services(
    ...
    service_plan_id: Optional[int] = Query(None, description="Filter by service plan ID"),
    status: Optional[str] = Query(None, description="Filter by status"),
    service_type: Optional[str] = Query(None, description="Filter by service plan type"),
...
@router.post("/", response_model=CustomerServiceResponse, status_code=http_status.HTTP_201_CREATED)
...
async def activate_customer_service(
    ...
    try:
        customer_service = customer_service_service.activate_service(service_id, current_admin.id)
        return customer_service
...
async def suspend_customer_service(
    ...
    try:
        customer_service = customer_service_service.suspend_service(service_id, current_admin.id)
        return customer_service
...
async def terminate_customer_service(
    ...
    try:
        customer_service = customer_service_service.terminate_service(service_id, current_admin.id)
        return customer_service
...
@router.post("/provision", response_model=ServiceProvisioningResponse, status_code=http_status.HTTP_201_CREATED)
async def provision_service(
    ...
    try:
        result = customer_service_service.provision_service(provision_request, current_admin.id)
...
@router.get("/customer/{customer_id}/services", response_model=List[CustomerServiceResponse])
...
@router.get("/service-plan/{service_plan_id}/assignments", response_model=ServicePlanAssignments)
async def get_service_plan_assignments(
    ...
    try:
        assignments = customer_service_service.get_service_plan_assignments(service_plan_id)
        return assignments
        ...
        logger.error(f"Error getting service plan assignments for {service_plan_id}: {e}")
        raise HTTPException(
            status_code=http_status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_expiring_services(
    days_ahead: int = Query(30, ge=1, le=365, description="Days ahead to check for expiring services"),
    current_admin: Administrator = Depends(get_current_admin),
    db: Session = Depends(get_db)
):
...
async def get_revenue_analytics(
    service_plan_id: Optional[int] = Query(None, description="Filter by specific service plan"),
    current_admin: Administrator = Depends(get_current_admin),
    db: Session = Depends(get_db)
):
