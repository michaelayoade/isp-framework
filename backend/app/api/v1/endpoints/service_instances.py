...
from app.schemas.service_management import (  # Customer Service Schemas; Internet Service Schemas; Voice Service Schemas
    BulkOperationResult,
    BulkServiceOperation,
    CustomerInternetService,
    CustomerInternetServiceCreate,
    CustomerInternetServiceUpdate,
    CustomerService,
    CustomerServiceCreate,
    CustomerServiceListResponse,
    CustomerServiceUpdate,
    CustomerVoiceService,
    CustomerVoiceServiceCreate,
    ServiceBillingIntegration,
...
async def create_customer_service(
    ...
    try:
        service = await service_service.create_customer_service(
            service_data=service_data.dict(), admin_id=current_admin.id
        )
        return service
...
async def list_customer_services(
    ...
    status: Optional[ServiceStatus] = Query(
        None, description="Filter by service status"
    ),
    service_type: Optional[ServiceType] = Query(
        None, description="Filter by service type"
    ),
    activation_date_from: Optional[str] = Query(
        None, description="Filter by activation date from (YYYY-MM-DD)"
    ),
    activation_date_to: Optional[str] = Query(
        None, description="Filter by activation date to (YYYY-MM-DD)"
    ),
    search_query: Optional[str] = Query(
        None, description="Search in service notes and configuration"
    ),
...
async def terminate_customer_service(
    ...
    try:
        success = await service_service.terminate_customer_service(
            service_id=service_id, reason=reason, admin_id=current_admin.id
        )
        if not success:
            raise HTTPException(status_code=404, detail="Service not found")
    except HTTPException:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to terminate service: {str(e)}"
        )
...
async def activate_customer_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to activate service: {str(e)}"
        )
...
async def suspend_customer_service(
    ...
    grace_period_hours: Optional[int] = Query(
        None, description="Grace period in hours"
    ),
    db: Session = Depends(get_db),
    current_admin: Administrator = Depends(get_current_admin),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to suspend service: {str(e)}"
        )
...
async def restore_customer_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to restore service: {str(e)}"
        )
...
@router.post(
    "/internet",
    response_model=CustomerInternetService,
...
async def create_internet_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create internet service: {str(e)}"
        )
...
async def list_internet_services(
    ...
    has_assigned_ip: Optional[bool] = Query(
        None, description="Filter by IP assignment"
    ),
    page: int = Query(1, ge=1, description="Page number"),
    size: int = Query(20, ge=1, le=100, description="Page size"),
    db: Session = Depends(get_db),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list internet services: {str(e)}"
        )
...
async def get_internet_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get internet service: {str(e)}"
        )
...
async def update_internet_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update internet service: {str(e)}"
        )
...
@router.post(
    "/internet/{service_id}/change-speed", response_model=CustomerInternetService
)
...
@router.post(
    "/voice", response_model=CustomerVoiceService, status_code=status.HTTP_201_CREATED
)
async def create_voice_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create voice service: {str(e)}"
        )
...
async def list_voice_services(
    ...
    has_balance: Optional[bool] = Query(
        None, description="Filter by call balance presence"
    ),
    page: int = Query(1, ge=1, description="Page number"),
    size: int = Query(20, ge=1, le=100, description="Page size"),
    db: Session = Depends(get_db),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list voice services: {str(e)}"
        )
...
async def get_voice_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get voice service: {str(e)}"
        )
...
async def add_voice_service_balance(
    ...
    try:
        service = await voice_service.add_call_balance(
            service_id=service_id, amount=amount, admin_id=current_admin.id
        )
        if not service:
            raise HTTPException(status_code=404, detail="Voice service not found")
        return service
...
async def perform_bulk_service_operation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to perform bulk operation: {str(e)}"
        )
...
@router.get(
    "/{service_id}/billing-integration", response_model=ServiceBillingIntegration
)
async def get_service_billing_integration(
    ...
    try:
        ...
        if not billing_data:
            raise HTTPException(
                status_code=404, detail="Service or billing data not found"
            )
        return billing_data
    except HTTPException:
        raise
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get billing integration: {str(e)}"
        )


@router.get(
    "/{service_id}/network-integration", response_model=ServiceNetworkIntegration
)
async def get_service_network_integration(
    ...
    try:
        ...
        if not network_data:
            raise HTTPException(
                status_code=404, detail="Service or network data not found"
            )
        return network_data
    except HTTPException:
        raise
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get network integration: {str(e)}"
        )
...
async def get_customer_services(
    ...
    try:
        services, _ = await service_service.search_customer_services(
            filters=filters, page=1, size=100  # Large size for customer overview
        )
        return services
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get customer services: {str(e)}"
        )
...
async def get_customer_services_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get customer services summary: {str(e)}"
        )
