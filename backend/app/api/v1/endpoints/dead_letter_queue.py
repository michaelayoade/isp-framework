...
from app.core.celery import get_active_tasks, get_task_status, revoke_task
from app.core.database import get_db
from app.models.auth.base import Administrator
from app.models.foundation import DeadLetterQueue, TaskExecutionLog
from app.tasks.maintenance_tasks import get_dead_letter_stats, retry_dead_letter_task
...
class RetryTaskRequest(BaseModel):
    ...
    notes: Optional[str] = Field(
        None, description="Optional notes for the retry attempt"
    )
...
async def get_dead_letter_queue_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve dead letter queue statistics"
        )
...
async def list_dead_letter_queue_items(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve dead letter queue items"
        )
...
async def get_dead_letter_queue_item(
    ...
    try:
        ...
        if not item:
            raise HTTPException(
                status_code=404, detail="Dead letter queue item not found"
            )
        ...
        logger.error(
            "Failed to get dead letter queue item", dlq_id=dlq_id, error=str(e)
        )
        raise HTTPException(
            status_code=500, detail="Failed to retrieve dead letter queue item"
        )
...
async def retry_dead_letter_queue_item(
    ...
    try:
        ...
        logger.error(
            "Failed to retry dead letter queue item", dlq_id=dlq_id, error=str(e)
        )
        raise HTTPException(
            status_code=500, detail="Failed to retry dead letter queue item"
        )
...
async def delete_dead_letter_queue_item(
    ...
    try:
        ...
        logger.error(
            "Failed to delete dead letter queue item", dlq_id=dlq_id, error=str(e)
        )
        raise HTTPException(
            status_code=500, detail="Failed to delete dead letter queue item"
        )


@router.get("/{task_id}/status", response_model=TaskStatusResponse)
async def get_task_status_endpoint(
    task_id: str, current_admin: Administrator = Depends(get_current_admin_user)
) -> TaskStatusResponse:
...
async def list_task_execution_logs(
    ...
    try:
        ...
        query = db.query(TaskExecutionLog).filter(
            TaskExecutionLog.created_at >= cutoff_time
        )
        ...
        offset = (page - 1) * size
        items = (
            query.order_by(TaskExecutionLog.created_at.desc())
            .offset(offset)
            .limit(size)
            .all()
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve task execution logs"
        )
...
async def get_dead_letter_dashboard(
    ...
    try:
        ...
        total_processed = stats.get("processed_count", 0) + stats.get(
            "requeued_count", 0
        )
        success_rate = (
