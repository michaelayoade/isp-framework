...
from app.schemas.voice_service import (
    ...
    VoiceServiceList, VoiceServiceProvisioningRequest
)
...
@router.post("/customers/{customer_id}/services/voice", 
...
async def create_voice_service(
    ...
    try:
        ...
        logger.info(f"Admin {current_admin.username} created Voice service {service.id} for customer {customer_id}")
        return service
...
@router.get("/customers/{customer_id}/services/voice", 
            response_model=VoiceServiceList)
...
async def update_voice_service(
    ...
    try:
        ...
        logger.info(f"Admin {current_admin.username} updated Voice service {service_id}")
        return service
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except ValidationError as e:
...
@router.post("/services/voice/{service_id}/provision", 
             response_model=dict)
async def provision_voice_service(
    ...
    try:
        ...
        logger.info(f"Admin {current_admin.username} queued provisioning for Voice service {service_id}")
        return {
...
async def suspend_voice_service(
    ...
    try:
        ...
        logger.info(f"Admin {current_admin.username} suspended Voice service {service_id}")
