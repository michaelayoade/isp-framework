...
from app.schemas.ticketing import (
    FieldWorkOrderCreate,
    FieldWorkOrderResponse,
    FieldWorkStatusEnum,
...
async def create_ticket(
    ...
    try:
        ...
        ticket = ticket_service.create_ticket(
            ticket_data.dict(), created_by=current_admin.id
        )
        return ticket
    except Exception as e:
...
async def update_ticket(
    ...
    try:
        ...
        ticket = ticket_service.update_ticket(
            ticket_id, update_data, updated_by=current_admin.id
        )
        return ticket
...
async def get_field_work_orders(
    ...
    try:
        ...
        if assigned_technician:
            query = query.filter(
                FieldWorkOrder.assigned_technician == assigned_technician
            )

        field_work_orders = (
            query.order_by(FieldWorkOrder.scheduled_date.desc())
...
async def create_knowledge_article(
    ...
    try:
        ...
        article = kb_service.create_article(
            article_data.dict(), author_id=current_admin.id
        )
        return article
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@router.get(
    "/knowledge-base/articles", response_model=List[KnowledgeBaseArticleResponse]
)
async def search_knowledge_articles(
    ...
    try:
        ...
        articles, total = kb_service.search_articles(
            query or "", filters, page, per_page
        )
        return articles
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@router.get(
    "/knowledge-base/articles/{article_id}", response_model=KnowledgeBaseArticleResponse
)
async def get_knowledge_article(
    ...
    article = (
        ...
        .filter(KnowledgeBaseArticle.id == article_id)
        .first()
    )
    if not article:
...
async def get_ticketing_dashboard(
    ...
    try:
        ...
        ticket_service = TicketService(db)
        stats = ticket_service.get_ticket_statistics({"created_after": start_date})

        recent_tickets, _ = ticket_service.search_tickets(
            {"created_after": start_date}, page=1, per_page=10
        )

        overdue_tickets, _ = ticket_service.search_tickets(
            {"overdue": True}, page=1, per_page=10
        )
