...
from app.schemas.customer import (
    ...
    CustomerCreate,
    CustomerExtended,
    CustomerExtendedCreate,
    CustomerExtendedUpdate,
    CustomerList,
    CustomerUpdate,
    CustomerWithExtended,
)
...
async def update_customer_status(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} updated customer {customer_id} status to {new_status}"
        )
        return customer
    except Exception as e:
...
@router.post(
    "/{customer_id}/extended",
    response_model=CustomerExtended,
...
async def create_customer_extended(
    ...
    try:
        extended_info = customer_service.create_customer_extended(
            customer_id, extended_data
        )
        logger.info(
            f"Admin {current_admin.username} created extended info for customer {customer_id}"
        )
...
async def update_customer_extended(
    ...
    try:
        extended_info = customer_service.update_customer_extended(
            customer_id, extended_data
        )
        logger.info(
            f"Admin {current_admin.username} updated extended info for customer {customer_id}"
        )
        return extended_info
    except Exception as e:
...
async def create_customer_contact(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} created contact for customer {customer_id}"
        )
        return contact
    except Exception as e:
...
async def update_customer_contact(
    ...
    try:
        contact = customer_service.update_customer_contact(
            customer_id, contact_id, contact_data
        )
        logger.info(
            f"Admin {current_admin.username} updated contact {contact_id} for customer {customer_id}"
        )
        return contact
    except Exception as e:
        logger.error(
            f"Error updating contact {contact_id} for customer {customer_id}: {e}"
        )
        if "not found" in str(e):
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
        raise HTTPException(
...
async def delete_customer_contact(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} deleted contact {contact_id} for customer {customer_id}"
        )
    except Exception as e:
        logger.error(
            f"Error deleting contact {contact_id} for customer {customer_id}: {e}"
        )
        if "not found" in str(e):
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
        raise HTTPException(
