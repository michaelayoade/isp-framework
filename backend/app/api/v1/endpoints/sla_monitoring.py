...
async def process_escalation_queue(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} triggered escalation queue processing"
        )
        return result
    except Exception as e:
...
async def get_sla_performance_metrics(
    period_days: int = Query(
        30, ge=1, le=365, description="Period in days for metrics calculation"
    ),
    current_admin: Administrator = Depends(get_current_admin),
    db: Session = Depends(get_db),
):
...
@router.post(
    "/escalation/rules", response_model=dict, status_code=status.HTTP_201_CREATED
)
async def create_escalation_rule(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} created escalation rule {rule['id']}"
        )
        return rule
...
async def manual_escalate_ticket(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} manually escalated ticket {ticket_id}"
        )
        return result
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
