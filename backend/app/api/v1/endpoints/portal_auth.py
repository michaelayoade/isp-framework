...
class PortalIDValidationRequest(BaseModel):
    ...
    exclude_customer_id: int = Field(None, description="Customer ID to exclude from uniqueness check")
...
async def authenticate_customer_portal_id(
    auth_request: PortalAuthRequest,
...
async def migrate_customers_to_portal_ids(
    ...
    try:
        ...
        results = customer_service.migrate_existing_customers_to_portal_ids(current_admin.id)
