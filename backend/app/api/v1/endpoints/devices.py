...
async def list_devices(
    ...
    is_auto_registered: Optional[bool] = Query(
        None, description="Filter by auto-registration"
    ),
...
async def bulk_approve_devices(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} bulk {action} {len(bulk_data.device_ids)} devices"
        )
...
async def bulk_update_devices(
    ...
    try:
        ...
        logger.info(
            f"Admin {current_admin.username} bulk updated {len(bulk_data.device_ids)} devices"
        )
...
async def radius_authenticate_device(
    request: RadiusDeviceRequest, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        logger.info(
            f"RADIUS auth for MAC {request.mac_address}: {'granted' if response.access_granted else 'denied'}"
        )
