...
@router.post("/internet", response_model=InternetServiceTemplate, status_code=status.HTTP_201_CREATED)
async def create_internet_service_template(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create internet template: {str(e)}")


@router.get("/internet", response_model=List[InternetServiceTemplate])
async def list_internet_service_templates(
    min_download_speed: Optional[int] = Query(None, description="Minimum download speed in kbps"),
    max_download_speed: Optional[int] = Query(None, description="Maximum download speed in kbps"),
    has_data_limit: Optional[bool] = Query(None, description="Filter by data limit presence"),
    static_ip_included: Optional[bool] = Query(None, description="Filter by static IP inclusion"),
    router_included: Optional[bool] = Query(None, description="Filter by router inclusion"),
    page: int = Query(1, ge=1, description="Page number"),
    size: int = Query(20, ge=1, le=100, description="Page size"),
    db: Session = Depends(get_db),
    current_admin: Administrator = Depends(get_current_admin)
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list internet templates: {str(e)}")
...
async def get_internet_service_template(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get internet template: {str(e)}")
...
async def update_internet_service_template(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update internet template: {str(e)}")
...
@router.post("/voice", response_model=VoiceServiceTemplate, status_code=status.HTTP_201_CREATED)
async def create_voice_service_template(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create voice template: {str(e)}")


@router.get("/voice", response_model=List[VoiceServiceTemplate])
async def list_voice_service_templates(
    is_unlimited: Optional[bool] = Query(None, description="Filter by unlimited minutes"),
    international_calling: Optional[bool] = Query(None, description="Filter by international calling"),
    voicemail: Optional[bool] = Query(None, description="Filter by voicemail feature"),
    conference_calling: Optional[bool] = Query(None, description="Filter by conference calling"),
    page: int = Query(1, ge=1, description="Page number"),
    size: int = Query(20, ge=1, le=100, description="Page size"),
    db: Session = Depends(get_db),
    current_admin: Administrator = Depends(get_current_admin)
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list voice templates: {str(e)}")
...
async def get_voice_service_template(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get voice template: {str(e)}")
...
@router.post("/bundle", response_model=BundleServiceTemplate, status_code=status.HTTP_201_CREATED)
async def create_bundle_service_template(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create bundle template: {str(e)}")


@router.get("/bundle", response_model=List[BundleServiceTemplate])
async def list_bundle_service_templates(
    service_types: Optional[List[ServiceType]] = Query(None, description="Filter by included service types"),
    min_discount: Optional[float] = Query(None, description="Minimum discount percentage"),
    has_contract: Optional[bool] = Query(None, description="Filter by contract requirement"),
    page: int = Query(1, ge=1, description="Page number"),
    size: int = Query(20, ge=1, le=100, description="Page size"),
    db: Session = Depends(get_db),
    current_admin: Administrator = Depends(get_current_admin)
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list bundle templates: {str(e)}")
...
async def get_bundle_service_template(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get bundle template: {str(e)}")
...
async def get_available_templates(
    location_id: Optional[int] = Query(None, description="Filter by location availability"),
    service_type: Optional[ServiceType] = Query(None, description="Filter by service type"),
    db: Session = Depends(get_db),
    current_admin: Administrator = Depends(get_current_admin)
):
...
async def export_templates(
    ...
    try:
        templates, _ = await template_service.search_templates(
            filters=filters,
            page=1,
            size=1000  # Large size for export
        )
        return templates
    except Exception as e:
