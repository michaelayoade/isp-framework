...
async def receive_grafana_webhook(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Alert processing failed: {str(e)}"
        )
...
async def create_error_handling_rules(
    ...
    try:
        ...
        logger.info(
            "Created error handling alert rules",
            extra={
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create alert rules: {str(e)}"
        )
...
async def create_operational_dashboards(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create dashboards: {str(e)}"
        )
...
async def test_alert_integration(
    ...
    try:
        ...
        logger.info(
            "Tested alert integration",
            extra={"admin_id": current_admin.id, "test_result": result.get("status")},
        )
...
async def _handle_email_alert(
    comm_service: CommunicationService, payload: Dict[str, Any]
):
...
async def _handle_webhook_alert(
    ...
    try:
        ...
        await comm_service.send_webhook(
            url=webhook_url, payload=payload, headers={"X-Alert-Source": "grafana"}
        )

        logger.info(
            "Webhook alert sent",
            extra={"webhook_url": webhook_url, "alert_name": payload.get("alert_name")},
        )
...
async def _handle_sms_alert(
    ...
    try:
        ...
        for recipient in recipients:
            await comm_service.send_sms(to_number=recipient, message=message)
