...
from app.core.security_middleware import cleanup_security_store, get_security_stats
...
async def rotate_jwt_signing_keys(
    force: bool = False, current_admin: Administrator = Depends(get_current_admin_user)
):
...
async def cleanup_jwt_keys(
    keep_days: int = 30, current_admin: Administrator = Depends(get_current_admin_user)
):
...
async def get_jwt_keys_info(
    ...
    try:
        ...
        return {"keys": key_info, "total_keys": len(key_info)}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def run_vulnerability_scan(
    ...
    try:
        ...
        return JSONResponse(status_code=status_code, content=scan_results)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def check_scan_requirements(
    ...
    try:
        ...
        return {
            ...
            "install_command": (
                f"pip install {' '.join(missing_tools)}" if missing_tools else None
            ),
        }
    except Exception as e:
        raise HTTPException(
        ...
        health_status = {
            ...
            "security_tools": {
                ...
                "available_tools": len(
                    [
                        t
                        for t in ["bandit", "safety", "pip-audit"]
