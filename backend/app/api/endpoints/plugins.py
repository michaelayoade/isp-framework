...
@router.post("/{plugin_id}/config", response_model=PluginConfiguration, status_code=status.HTTP_201_CREATED)
...
async def update_plugin_configuration(
    ...
    try:
        ...
        configuration = config_repo.update(config_id, config_data.dict(exclude_unset=True))
...
@router.post("/{plugin_id}/hooks", response_model=PluginHook, status_code=status.HTTP_201_CREATED)
...
@router.post("/templates", response_model=PluginTemplate, status_code=status.HTTP_201_CREATED)
async def create_plugin_template(
    ...
    try:
        ...
        logger.info(f"Plugin template {template.name} created by admin {current_admin.id}")
        return template
...
async def bulk_plugin_operation(
    ...
    try:
        ...
        logger.info(f"Bulk operation {operation_request.operation} completed by admin {current_admin.id}")
