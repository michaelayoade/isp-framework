...
class ServiceLayerFactory:
    ...
    def get_internet_template_service(self) -> InternetServiceTemplateService:
        ...
        if 'internet_template' not in self._services:
            self._services['internet_template'] = InternetServiceTemplateService(self.db)
    ...
    def get_provisioning_template_service(self) -> ProvisioningTemplateService:
        ...
        if 'provisioning_template' not in self._services:
            self._services['provisioning_template'] = ProvisioningTemplateService(self.db)
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'template_services': len([k for k in self._services.keys() if 'template' in k]),
            'instance_services': len([k for k in self._services.keys() if 'service' in k and 'template' not in k]),
            'provisioning_services': len([k for k in self._services.keys() if 'provisioning' in k]),
            'management_services': len([k for k in self._services.keys() if k in ['ip_assignment', 'status_history', 'suspension', 'usage_tracking', 'alert']])
        }
        ...
        for service_name, service_class in service_types:
            try:
                ...
                if not hasattr(service_instance, 'db'):
                    validation_results['warnings'].append(f"Service {service_name} missing db attribute")
            except Exception as e:
                validation_results['is_valid'] = False
                validation_results['errors'].append(f"Failed to instantiate {service_name}: {str(e)}")
...
def get_service_layer_factory(db: Session, cache_key: Optional[str] = None) -> ServiceLayerFactory:
    """
...
class ServiceBusinessIntelligence:
    ...
    async def get_service_overview_dashboard(self) -> Dict[str, Any]:
        ...
        dashboard_data['service_stats'] = await customer_service.get_service_statistics()
