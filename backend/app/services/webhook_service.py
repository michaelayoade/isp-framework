...
class WebhookEventTypeService:
    ...
    def get_event_type(self, event_type_id: int) -> Optional[WebhookEventType]:
        ...
        return (
            ...
            .filter(WebhookEventType.id == event_type_id)
            .first()
        )
    ...
    def get_event_type_by_name(self, name: str) -> Optional[WebhookEventType]:
        ...
        return (
            self.db.query(WebhookEventType)
            .filter(WebhookEventType.name == name)
            .first()
        )
    ...
    def update_event_type(
        self, event_type_id: int, updates: dict
...
class WebhookEndpointService:
    ...
    def create_endpoint(
        self, endpoint_data: WebhookEndpointCreate, created_by: int
    ) -> WebhookEndpoint:
        ...
        if endpoint_data.subscribed_event_ids:
            for event_type_id in endpoint_data.subscribed_event_ids:
                ...
                subscription = WebhookEndpointEvent(
                    endpoint_id=endpoint.id, event_type_id=event_type_id
                )
    ...
    def search_endpoints(
        self, search: WebhookEndpointSearch, skip: int = 0, limit: int = 100
        ...
        if search.last_delivery_after:
            query = query.filter(
                WebhookEndpoint.last_delivery_at >= search.last_delivery_after
            )
        if search.last_delivery_before:
            query = query.filter(
                WebhookEndpoint.last_delivery_at <= search.last_delivery_before
            )
    ...
    def update_endpoint(
        self, endpoint_id: int, updates: WebhookEndpointUpdate
...
class WebhookEventService:
    ...
    def search_events(
        self, search: WebhookEventSearch, skip: int = 0, limit: int = 100
        ...
        if search.triggered_by_user_id:
            query = query.filter(
                WebhookEvent.triggered_by_user_id == search.triggered_by_user_id
            )
        if search.triggered_by_customer_id:
            query = query.filter(
                WebhookEvent.triggered_by_customer_id == search.triggered_by_customer_id
            )

        total = query.count()
        events = (
            query.order_by(desc(WebhookEvent.occurred_at))
            .offset(skip)
            .limit(limit)
            .all()
        )
...
class WebhookDeliveryService:
    ...
    def search_deliveries(
        self, search: WebhookDeliverySearch, skip: int = 0, limit: int = 100
        ...
        if search.scheduled_before:
            query = query.filter(
                WebhookDelivery.scheduled_at <= search.scheduled_before
            )
        ...
        if search.delivered_before:
            query = query.filter(
                WebhookDelivery.delivered_at <= search.delivered_before
            )
        ...
        total = query.count()
        deliveries = (
            query.order_by(desc(WebhookDelivery.scheduled_at))
            .offset(skip)
            .limit(limit)
            .all()
        )
...
class WebhookFilterService:
    ...
    def create_filter(
        self, endpoint_id: int, filter_data: WebhookFilterCreate
    ) -> WebhookFilter:
    ...
    def _evaluate_single_filter(
        self, webhook_filter: WebhookFilter, payload: Dict[str, Any]
    ) -> bool:
...
class WebhookDeliveryEngine:
    ...
    async def deliver_webhook(self, delivery: WebhookDelivery) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient(
                timeout=endpoint.timeout_seconds, verify=endpoint.verify_ssl
            ) as client:
                response = await client.request(
                    ...
                    headers=headers,
                    json=(
                        payload if endpoint.content_type == "application/json" else None
                    ),
                    data=(
                        payload if endpoint.content_type != "application/json" else None
                    ),
    ...
    def _generate_signature(
        self, payload: str, secret: str, algorithm: SignatureAlgorithm
    ) -> str:
