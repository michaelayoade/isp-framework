...
class AnsibleInventoryService:
    ...
    def generate_inventory(
        self, site_id: Optional[int] = None, device_type: Optional[str] = None
    ) -> Dict[str, Any]:
        """Generate dynamic Ansible inventory in JSON format."""
        try:
            inventory = {
                "_meta": {"hostvars": {}},
                "all": {"children": ["sites", "device_types", "vendors"]},
                "sites": {"children": []},
                "device_types": {"children": []},
                "vendors": {"children": []},
            }

            # Build query with filters
            query = (
                self.db.query(AutomationDevice)
                .options(
            ...
            for device in devices:
                # Site grouping
                site_name = (
                    f"site_{device.site.code}" if device.site else "site_unknown"
                )
                ...
                ssh_cred = next(
                    (
                        ...
                        if c.credential_type == "ssh" and c.is_active
                    ),
                    None,
                )
                if ssh_cred:
    ...
    async def execute_playbook(self, task_id: int) -> bool:
        ...
        try:
            task = (
                ...
                .filter(ProvisioningTask.id == task_id)
                .first()
            )
            if not task:
            ...
            try:
                ...
                cmd = [
                    ...
                    playbook_path,
                    "--extra-vars",
                    json.dumps(task.ansible_variables),
                ]
                ...
                if task.device_id:
                    device = (
                        self.db.query(AutomationDevice)
                        .filter(AutomationDevice.id == task.device_id)
                        .first()
                    )
                    if device:
...
class DeviceManagementService:
    ...
    def backup_device_config(self, device_id: int) -> bool:
        ...
        try:
            device = (
                self.db.query(AutomationDevice)
                .filter(AutomationDevice.id == device_id)
                .first()
            )
            if not device:
                return False
    ...
    def provision_customer_service(self, service_id: int) -> bool:
        ...
        try:
            ...
            playbook_name = playbook_map.get(
                service_template.category, "provision_generic_service.yml"
            )
            ...
            task = runner.create_provisioning_task(
                ...
                variables={
                    ...
                    "bandwidth_down": getattr(
                        service.internet_config, "download_speed_kbps", None
                    ),
                    "bandwidth_up": getattr(
                        service.internet_config, "upload_speed_kbps", None
                    ),
                },
            )
            ...
            logger.error(
                f"Error creating provisioning task for service {service_id}: {str(e)}"
            )
            return False
    ...
    def update_device_monitoring(self, device_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error creating monitoring task for device {device_id}: {str(e)}"
            )
            return {"error": str(e)}
...
class AutomationService:
    ...
    def get_automation_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_devices = (
                self.db.query(AutomationDevice)
                .filter(AutomationDevice.status == "active")
                .count()
            )
            managed_devices = (
                self.db.query(AutomationDevice)
            ...
            pending_tasks = (
                self.db.query(ProvisioningTask)
                .filter(ProvisioningTask.status == "pending")
                .count()
            )
            running_tasks = (
                self.db.query(ProvisioningTask)
                .filter(ProvisioningTask.status == "running")
                .count()
            )
            failed_tasks = (
                self.db.query(ProvisioningTask)
                .filter(ProvisioningTask.status == "failed")
                .count()
            )
            ...
            recent_tasks = (
                self.db.query(ProvisioningTask)
                .order_by(desc(ProvisioningTask.created_at))
                .limit(10)
                .all()
            )
            ...
            return {
                ...
                "sites": {"total": total_sites, "active": active_sites},
                "tasks": {
                    "total": total_tasks,
