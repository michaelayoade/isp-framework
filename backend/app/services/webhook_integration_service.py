...
from app.services.webhook_service import WebhookDeliveryEngine, WebhookEventService
...
class WebhookIntegrationService:
    ...
    async def _process_event_async(self, event_id: int):
        ...
        try:
            ...
            try:
                ...
                event = (
                    db.query(WebhookEvent).filter(WebhookEvent.id == event_id).first()
                )
                if event:
    ...
    def _get_event_type_id(self, event_type_name: str) -> int:
        ...
        event_type = (
            self.db.query(WebhookEventType)
            .filter(WebhookEventType.name == event_type_name)
            .first()
        )
        ...
        if not event_type:
            ...
            event_type = (
                self.db.query(WebhookEventType)
                .filter(WebhookEventType.name == "customer.created")
                .first()
            )
...
class WebhookTriggers:
    ...
    def customer_created(
        self, customer_data: Dict[str, Any], user_id: Optional[int] = None
    ):
    ...
    def customer_deleted(
        ...
        return self.integration_service.trigger_event(
            "customer.deleted", payload, triggered_by_user_id=user_id
        )

    def customer_service_created(
        self, service_data: Dict[str, Any], user_id: Optional[int] = None
    ):
        """Trigger customer service creation event"""
        return self.integration_service.trigger_event(
            "customer.service.created", service_data, triggered_by_user_id=user_id
        )

    def customer_service_updated(
        self, service_data: Dict[str, Any], user_id: Optional[int] = None
    ):
        """Trigger customer service update event"""
        return self.integration_service.trigger_event(
            "customer.service.updated", service_data, triggered_by_user_id=user_id
        )

    def customer_services_overview(
        self, overview_data: Dict[str, Any], user_id: Optional[int] = None
    ):
        """Trigger customer services overview event"""
        return self.integration_service.trigger_event(
            "customer.services.overview", overview_data, triggered_by_user_id=user_id
        )

    # Service Events
    def service_activated(
        self, service_data: Dict[str, Any], user_id: Optional[int] = None
    ):
    ...
    def service_suspended(
        self, service_data: Dict[str, Any], reason: str, user_id: Optional[int] = None
    ):
    ...
    # Billing Events
    def invoice_created(
        self, invoice_data: Dict[str, Any], user_id: Optional[int] = None
    ):
    ...
    def auth_login(self, user_data: Dict[str, Any], ip_address: str, user_agent: str):
        ...
        return self.integration_service.trigger_event(
            "auth.login", payload, triggered_by_user_id=user_data.get("id")
        )
    ...
    def auth_logout(self, user_data: Dict[str, Any], ip_address: str):
        ...
        return self.integration_service.trigger_event(
            "auth.logout", payload, triggered_by_user_id=user_data.get("id")
        )

    # Network Events
    def network_device_down(
        self, device_data: Dict[str, Any], site_data: Dict[str, Any]
    ):
        ...
        return self.integration_service.trigger_event("network.device.down", payload)
    ...
    def network_site_created(
        self, site_data: Dict[str, Any], user_id: Optional[int] = None
    ):
        """Trigger network site creation event"""
        return self.integration_service.trigger_event(
            "network.site.created", site_data, triggered_by_user_id=user_id
        )

    def network_device_created(
        self, device_data: Dict[str, Any], user_id: Optional[int] = None
    ):
        """Trigger network device creation event"""
        return self.integration_service.trigger_event(
            "network.device.created", device_data, triggered_by_user_id=user_id
        )

    # Ticketing Events
    def ticket_created(
        self, ticket_data: Dict[str, Any], user_id: Optional[int] = None
    ):
    ...
    def ticket_resolved(
        ...
        return self.integration_service.trigger_event(
            "ticket.resolved", payload, triggered_by_user_id=user_id
        )
    ...
    def ticket_status_changed(
        ...
        return self.integration_service.trigger_event(
            "ticket.status_changed", ticket_data, triggered_by_user_id=user_id
        )
    ...
    def ticket_assigned(
        ...
        return self.integration_service.trigger_event(
            "ticket.assigned", ticket_data, triggered_by_user_id=user_id
        )
