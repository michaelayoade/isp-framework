...
class PortalIDService:
        ...
        try:
            ...
            for config in configs:
                if portal_id.startswith(config.prefix):
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Extracted customer ID {customer_id} from portal ID {portal_id}"
                        )
                        return customer_id
                    except ValueError:
                        continue
            ...
            if portal_id.startswith("1000"):
                ...
                try:
                    customer_id = int(customer_id_str)
                    logger.debug(
                        f"Extracted customer ID {customer_id} from portal ID {portal_id} using default prefix"
                    )
                    return customer_id
                except ValueError:
                    pass
            ...
            logger.error(
                f"Error extracting customer ID from portal ID {portal_id}: {e}"
            )
            return None

    def get_portal_prefix(
        self, partner_id: Optional[int] = None, service_type: str = "internet"
    ) -> str:
        """
        ...
        try:
            ...
            filters = {"service_type": service_type, "is_active": True}
            ...
            default_configs = self.portal_config_repo.get_all(
            ...
            # Ultimate fallback
            logger.warning(
                f"No portal config found for partner {partner_id}, service {service_type}. Using default prefix."
            )
            return "1000"
    ...
    def create_portal_config(
        ...
        try:
            ...
            logger.info(
                f"Created portal config: partner={partner_id}, prefix={prefix}, service={service_type}"
            )
            return config
    ...
    def get_portal_configs(
        self, partner_id: Optional[int] = None, service_type: Optional[str] = None
    ) -> List[PortalConfig]:
        """
    ...
    def log_portal_id_change(
        self,
        old_portal_id: Optional[str],
        new_portal_id: str,
