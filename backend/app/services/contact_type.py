...
class ContactTypeService:
    ...
    def get_contact_type(self, contact_type_id: int) -> ContactType:
        ...
        contact_type = (
            self.db.query(ContactType).filter(ContactType.id == contact_type_id).first()
        )
        if not contact_type:
    ...
    def create_contact_type(self, contact_type_data: ContactTypeCreate) -> ContactType:
        ...
        if existing:
            raise DuplicateError(
                f"Contact type with code '{contact_type_data.code}' already exists"
            )

        # Create contact type
        contact_type_dict = contact_type_data.model_dump()
        contact_type_dict["is_system"] = (
            False  # User-created types are not system types
        )
    ...
    def update_contact_type(
        self, contact_type_id: int, contact_type_data: ContactTypeUpdate
    ) -> ContactType:
    ...
    def delete_contact_type(self, contact_type_id: int):
        ...
        if contact_type.is_system:
            raise ValidationError(
                f"System contact type '{contact_type.code}' cannot be deleted"
            )
        ...
        contacts_using_type = (
            ...
            .filter(CustomerContact.contact_type_id == contact_type_id)
            .count()
        )

        if contacts_using_type > 0:
            raise ValidationError(
                f"Contact type '{contact_type.code}' is in use by {contacts_using_type} contacts and cannot be deleted"
            )
    ...
    def get_default_contact_type(self) -> ContactType:
        ...
        if not default_type:
            ...
            default_type = (
                ...
                .filter(ContactType.is_active is True)
                .order_by(ContactType.sort_order)
                .first()
            )
