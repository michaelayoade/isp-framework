...
class SettingsService:
    ...
    def get_setting(self, key: str, default: Any = None, use_cache: bool = True) -> Any:
        ...
        setting = (
            ...
            .filter(and_(Setting.key == key, Setting.is_active is True))
            .first()
        )
    ...
    def _validate_setting_value(self, setting: Setting, value: Any):
        ...
        if setting.setting_type == SettingType.INTEGER:
            try:
                ...
                if setting.min_value and int_value < int(setting.min_value):
                    raise ValueError(
                        f"Value {int_value} is below minimum {setting.min_value}"
                    )
                if setting.max_value and int_value > int(setting.max_value):
                    raise ValueError(
                        f"Value {int_value} is above maximum {setting.max_value}"
                    )
            ...
            try:
                ...
                if setting.min_value and float_value < float(setting.min_value):
                    raise ValueError(
                        f"Value {float_value} is below minimum {setting.min_value}"
                    )
                if setting.max_value and float_value > float(setting.max_value):
                    raise ValueError(
                        f"Value {float_value} is above maximum {setting.max_value}"
                    )
            except ValueError as e:
        ...
        if setting.validation_regex and isinstance(value, str):
            if not re.match(setting.validation_regex, value):
                raise ValueError(
                    f"Value does not match required pattern: {setting.validation_regex}"
                )
    ...
    def get_settings_by_category(self, category: SettingCategory) -> List[Setting]:
        ...
        return (
            self.db.query(Setting)
            .filter(and_(Setting.category == category, Setting.is_active is True))
            .order_by(Setting.display_order, Setting.key)
            .all()
        )
    ...
    def get_setting_history(self, key: str, limit: int = 50) -> List[SettingHistory]:
        ...
        return (
            self.db.query(SettingHistory)
            .filter(SettingHistory.setting_id == setting.id)
...
class FeatureFlagService:
    ...
    def is_enabled(
        ...
        flag = (
            ...
            .filter(and_(FeatureFlag.name == flag_name, FeatureFlag.is_enabled is True))
            .first()
        )
    ...
    def create_flag(self, name: str, description: str = None, **kwargs) -> FeatureFlag:
        ...
        log_audit_event(domain="feature_flags", event="flag_created", flag_name=name)
    ...
    def update_flag(self, name: str, **kwargs) -> FeatureFlag:
        ...
        log_audit_event(
            domain="feature_flags", event="flag_updated", flag_name=name, changes=kwargs
        )
    ...
    with get_settings_service() as service:
        ...
        for key in settings:
            ...
            if env_value is not None:
                try:
                    service.set_setting(
                        key, env_value, change_reason="Environment variable override"
                    )
                    logger.info(
                        "Setting overridden from environment", key=key, env_key=env_key
                    )
                except Exception as e:
                    logger.warning(
                        "Failed to override setting from environment",
                        key=key,
                        env_key=env_key,
                        error=str(e),
                    )
