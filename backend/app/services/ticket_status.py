...
class TicketStatusService:
    ...
    def get_ticket_status(self, status_id: int) -> TicketStatus:
        ...
        status = (
            self.db.query(TicketStatus).filter(TicketStatus.id == status_id).first()
        )
        if not status:
    ...
    def create_ticket_status(self, status_data: TicketStatusCreate) -> TicketStatus:
        ...
        if existing:
            raise DuplicateError(
                f"Ticket status with code '{status_data.code}' already exists"
            )
    ...
    def update_ticket_status(
        self, status_id: int, status_data: TicketStatusUpdate
    ) -> TicketStatus:
    ...
    def delete_ticket_status(self, status_id: int):
        ...
        if status.is_system:
            raise ValidationError(
                f"System ticket status '{status.code}' cannot be deleted"
            )
        ...
        try:
            ...
            tickets_using_status = (
                self.db.query(Ticket).filter(Ticket.status_id == status_id).count()
            )

            if tickets_using_status > 0:
                raise ValidationError(
                    f"Ticket status '{status.code}' is in use by {tickets_using_status} tickets and cannot be deleted"
                )
        except ImportError:
