...
class APIManagementService:
    ...
    def validate_api_key(self, api_key: str, client_ip: str = None, user_agent: str = None) -> APIKey:
    ...
    def check_rate_limit(self, api_key_id: int, endpoint: str = None) -> bool:
        ...
        rate_limit = self.db.query(APIRateLimit).filter(
            and_(
    ...
    def get_usage_analytics(
        ...
        unique_endpoints = query.with_entities(
            func.count(func.distinct(APIUsage.endpoint))
        ).scalar() or 0
        unique_users = query.with_entities(
            func.count(func.distinct(APIUsage.api_key_id))
        ).scalar() or 0
        ...
        endpoint_data = query.with_entities(
            APIUsage.endpoint,
            func.count().label('count')
        ...
        error_data = query.with_entities(
            APIUsage.error_type,
            func.count().label('count')
        ...
        status_data = query.with_entities(
    ...
    def create_api_endpoint(self, endpoint_data: APIEndpointCreate) -> APIEndpoint:
        ...
        logger.info(f"Created API endpoint: {endpoint_data.method} {endpoint_data.path}")
        return endpoint
    ...
    def cleanup_old_usage_data(self, days_to_keep: int = 90) -> int:
        ...
        deleted_count = self.db.query(APIUsage).filter(
