...
from ..schemas.services import (
    InternetServiceCreate, InternetServiceUpdate,
    VoiceServiceCreate, VoiceServiceUpdate,
    BundleServiceCreate, BundleServiceUpdate,
    RecurringServiceCreate, RecurringServiceUpdate,
    ServiceOverview, ServiceSearchFilters
)
...
class ServiceManagementService:
    ...
    async def create_internet_service(self, service_data: InternetServiceCreate) -> InternetService:
        """Create a new internet service"""
        try:
            # Check for duplicate name
            existing = self.repo.internet.get_all({"name": service_data.name})
            if existing:
                raise DuplicateError(f"Internet service with name '{service_data.name}' already exists")
            ...
            logger.info(f"Created internet service: {created_service.name} (ID: {created_service.id})")
    ...
    async def update_internet_service(self, service_id: int, service_data: InternetServiceUpdate) -> InternetService:
        ...
        if update_data:
            ...
            logger.info(f"Updated internet service: {updated_service.name} (ID: {updated_service.id})")
    ...
    async def list_internet_services(self, filters: Optional[Dict] = None) -> List[InternetService]:
    ...
    async def create_voice_service(self, service_data: VoiceServiceCreate) -> VoiceService:
        """Create a new voice service"""
        try:
            # Check for duplicate name
            existing = self.repo.voice.get_all({"name": service_data.name})
            if existing:
                raise DuplicateError(f"Voice service with name '{service_data.name}' already exists")
            ...
            logger.info(f"Created voice service: {created_service.name} (ID: {created_service.id})")
            return created_service
    ...
    async def update_voice_service(self, service_id: int, service_data: VoiceServiceUpdate) -> VoiceService:
        ...
        if update_data:
            ...
            logger.info(f"Updated voice service: {updated_service.name} (ID: {updated_service.id})")
            return updated_service
    ...
    async def list_voice_services(self, filters: Optional[Dict] = None) -> List[VoiceService]:
        """List voice services with optional filters"""
        return self.repo.voice.get_all(filters or {})
    
    # Bundle Service Operations
    async def create_bundle_service(self, service_data: BundleServiceCreate) -> BundleService:
        """Create a new bundle service"""
        try:
            # Check for duplicate name
            existing = self.repo.bundle.get_all({"name": service_data.name})
            if existing:
                raise DuplicateError(f"Bundle service with name '{service_data.name}' already exists")
            
            # Validate referenced services exist
            if service_data.internet_service_id:
                internet_service = self.repo.internet.get_by_id(service_data.internet_service_id)
                if not internet_service:
                    raise NotFoundError(f"Internet service with ID {service_data.internet_service_id} not found")
            
            if service_data.voice_service_id:
                voice_service = self.repo.voice.get_by_id(service_data.voice_service_id)
                if not voice_service:
                    raise NotFoundError(f"Voice service with ID {service_data.voice_service_id} not found")
            ...
            logger.info(f"Created bundle service: {created_service.name} (ID: {created_service.id})")
            return created_service
    ...
    async def update_bundle_service(self, service_id: int, service_data: BundleServiceUpdate) -> BundleService:
        ...
        if update_data:
            ...
            logger.info(f"Updated bundle service: {updated_service.name} (ID: {updated_service.id})")
            return updated_service
    ...
    async def list_bundle_services(self, filters: Optional[Dict] = None) -> List[BundleService]:
        """List bundle services with optional filters"""
        return self.repo.bundle.get_all(filters or {})
    
    # Recurring Service Operations
    async def create_recurring_service(self, service_data: RecurringServiceCreate) -> RecurringService:
        """Create a new recurring service"""
        try:
            # Check for duplicate name
            existing = self.repo.recurring.get_all({"name": service_data.name})
            if existing:
                raise DuplicateError(f"Recurring service with name '{service_data.name}' already exists")
            ...
            logger.info(f"Created recurring service: {created_service.name} (ID: {created_service.id})")
            return created_service
    ...
    async def update_recurring_service(self, service_id: int, service_data: RecurringServiceUpdate) -> RecurringService:
        ...
        if update_data:
            ...
            logger.info(f"Updated recurring service: {updated_service.name} (ID: {updated_service.id})")
            return updated_service
    ...
    async def list_recurring_services(self, filters: Optional[Dict] = None) -> List[RecurringService]:
    ...
    async def search_services(self, filters: ServiceSearchFilters) -> Dict[str, List]:
        ...
        if filters.search_term:
            results = self.repo.search_all_services(filters.search_term, filters.service_type)
        else:
    ...
    async def get_service_for_radius(self, service_id: int, service_type: str) -> Optional[Dict[str, Any]]:
