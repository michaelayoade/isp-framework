...
class VoiceServiceService:
    ...
    def create_voice_service(
        self, customer_id: int, service_data: VoiceServiceCreate
    ) -> VoiceService:
        ...
        try:
            ...
            self.webhook_triggers.service_created(
                {
                    ...
                    "monthly_fee": (
                        float(voice_service.monthly_fee)
                        if voice_service.monthly_fee
                        else 0
                    ),
                }
            )
            ...
            logger.info(
                f"Created voice service {voice_service.id} for customer {customer_id}"
            )
            ...
            self.db.rollback()
            logger.error(
                f"Error creating voice service for customer {customer_id}: {e}"
            )
            raise
    ...
    def get_customer_voice_services(self, customer_id: int) -> List[VoiceService]:
        ...
        return (
            ...
            .filter(VoiceService.customer_id == customer_id)
            .all()
        )

    def update_voice_service(
        self, service_id: int, update_data: VoiceServiceUpdate
    ...
    def provision_voice_service(
        self, service_id: int, provisioning_data: VoiceServiceProvisioningRequest
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if voice_service.status != "pending_activation":
                raise ValidationError(
                    f"Voice service {service_id} is not in pending_activation status"
                )
    ...
    def suspend_voice_service(
        self, service_id: int, reason: str = None
    ) -> VoiceService:
    ...
    def get_voice_service_statistics(self, customer_id: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            suspended_services = query.filter(
                VoiceService.status == "suspended"
            ).count()
            pending_services = query.filter(
                VoiceService.status == "pending_activation"
            ).count()
