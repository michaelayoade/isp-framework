...
class RBACService:
    ...
    def list_permissions(self, active_only: bool = True, category: Optional[str] = None) -> List[Permission]:
        ...
        permissions = query.order_by(Permission.category, Permission.sort_order, Permission.name).all()
        return permissions
    ...
    def create_permission(self, permission_data: PermissionCreate) -> Permission:
        ...
        if existing:
            raise DuplicateError(f"Permission with code '{permission_data.code}' already exists")
        
        # Create permission
        permission_dict = permission_data.model_dump()
        permission_dict['is_system'] = False  # User-created permissions are not system permissions
    ...
    def delete_permission(self, permission_id: int):
        ...
        if permission.is_system:
            raise ValidationError(f"System permission '{permission.code}' cannot be deleted")
        
        # Check if it's in use by roles
        roles_using_permission = self.db.query(RolePermission).filter(
            RolePermission.permission_id == permission_id
        ).count()
        
        if roles_using_permission > 0:
            raise ValidationError(f"Permission '{permission.code}' is assigned to {roles_using_permission} roles and cannot be deleted")
    ...
    def list_roles(self, active_only: bool = True, include_permissions: bool = False) -> List[Role]:
    ...
    def delete_role(self, role_id: int):
        ...
        if users_with_role > 0:
            raise ValidationError(f"Role '{role.code}' is assigned to {users_with_role} users and cannot be deleted")
    ...
    def assign_role_to_user(self, user_role_data: UserRoleCreate, assigned_by_id: int) -> UserRole:
        ...
        logger.info(f"Assigned role {user_role_data.role_id} to user {user_role_data.user_id}")
        return user_role
    ...
    def check_permission_detailed(self, user_id: int, permission_code: str, resource_id: Optional[int] = None) -> PermissionCheckResult:
