...
from app.models.services import ServiceTemplate as ServicePlan
...
class RadiusServiceIntegration:
    ...
    def authenticate_customer(
        self, portal_id: str, password: str, nas_ip: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            customer = self.customer_service.authenticate_by_portal_id(
                portal_id, password
            )
            ...
            if not service_plan:
                ...
                return {"authenticated": False, "reason": "No active service plan"}
            ...
            logger.info(
                f"Authentication successful for portal ID: {portal_id}, customer: {customer.id}"
            )
            ...
            logger.error(
                f"Error during authentication for portal ID {portal_id}: {str(e)}"
            )
            return {"authenticated": False, "reason": "Authentication system error"}

    def start_session(
        self, auth_result: Dict[str, Any], session_data: Dict[str, Any]
        ...
        try:
            ...
            # Log session start
            logger.info(
                f"RADIUS session started for customer {customer_id}, session ID: {session.id}"
            )
    ...
    def stop_session(self, session_id: str, session_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            session.termination_cause = session_data.get(
            ...
            # Update customer online status
            self._update_customer_online_status(
                session.customer_id, session.id, "offline"
            )
            ...
            logger.info(
                f"RADIUS session stopped for customer {session.customer_id}, session ID: {session.id}"
            )
    ...
    def update_session_accounting(
        self, session_id: str, accounting_data: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            if not session:
                logger.warning(
                    f"Active session not found for accounting update: {session_id}"
                )
                return False
            ...
            session.packets_out = accounting_data.get(
            ...
            logger.error(
                f"Error updating accounting for session {session_id}: {str(e)}"
            )
            return False

    def get_customer_sessions(
        self, customer_id: int, active_only: bool = False
    ) -> List[RadiusSession]:
        """
        ...
        try:
            ...
            sessions = self.radius_session_repo.get_all(
                filters=filters, order_by="start_time", order_desc=True
            )
    ...
    def get_service_plan_enforcement_attributes(
        self, service_plan_id: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(
                f"Error getting enforcement attributes for service plan {service_plan_id}: {str(e)}"
            )
            return {}
    ...
    def _get_active_service_plan(self, customer_id: int) -> Optional[ServicePlan]:
        ...
        try:
            ...
            logger.error(
                f"Error getting active service plan for customer {customer_id}: {str(e)}"
            )
            return None

    def _get_radius_attributes(
        self, service_plan: ServicePlan, customer: Optional[Customer]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if service_plan.service_type == "internet":
                ...
                internet_service = (
                    self.db.query(InternetService)
                    .filter(InternetService.service_plan_id == service_plan.id)
                    .first()
                )
                ...
                if internet_service:
                    ...
                    if internet_service.download_speed:
                        attributes["WISPr-Bandwidth-Max-Down"] = (
                            internet_service.download_speed * 1000
                        )  # Convert Kbps to bps

                    if internet_service.upload_speed:
                        attributes["WISPr-Bandwidth-Max-Up"] = (
                            internet_service.upload_speed * 1000
                        )  # Convert Kbps to bps

                    # Data limit attributes
                    if internet_service.data_limit:
                        attributes["ChilliSpot-Max-Total-Octets"] = (
                            internet_service.data_limit * 1024 * 1024
                        )  # Convert MB to bytes
            ...
            # Voice service attributes
                ...
                voice_service = (
                    self.db.query(VoiceService)
                    .filter(VoiceService.service_plan_id == service_plan.id)
                    .first()
                )
                ...
                if voice_service:
                    ...
                    if voice_service.concurrent_calls:
                        attributes["Voice-Max-Concurrent-Calls"] = (
                            voice_service.concurrent_calls
                        )
            ...
            logger.debug(
                f"Generated RADIUS attributes for service plan {service_plan.id}: {attributes}"
            )
    ...
    def _assign_ip_address(
        self, customer_id: int, service_plan_id: int
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            available_ip = (
                self.db.query(IPv4IP)
                .filter(
                    and_(IPv4IP.status == "available", IPv4IP.ip_address.isnot(None))
                )
                .first()
            )
            ...
            if available_ip:
                ...
                logger.info(
                    f"Assigned IP {available_ip.ip_address} to customer {customer_id}"
                )
            ...
            logger.error(
                f"Error assigning IP address to customer {customer_id}: {str(e)}"
            )
            return None
    ...
    def _release_ip_address(self, ip_address: str) -> bool:
        ...
        try:
            ip_record = (
                self.db.query(IPv4IP).filter(IPv4IP.ip_address == ip_address).first()
            )
    ...
    def _update_customer_online_status(
        self, customer_id: int, session_id: int, status: str
    ) -> None:
        """
        Update customer online status
        """
        try:
            ...
            online_record = (
                self.db.query(CustomerOnline)
                .filter(CustomerOnline.customer_id == customer_id)
                .first()
            )
    ...
    def _update_customer_statistics(self, session: RadiusSession) -> None:
        ...
        try:
            ...
            stats = (
                ...
                .filter(CustomerStatistics.customer_id == session.customer_id)
                .first()
            )
        ...
        try:
            ...
            if internet_service and internet_service.data_limit:
                ...
                if total_usage >= limit_bytes:
                    logger.warning(
                        f"Customer {session.customer_id} exceeded data limit in session {session.id}"
                    )
            ...
            logger.error(
                f"Error checking usage limits for session {session.id}: {str(e)}"
            )
