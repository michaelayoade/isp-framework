...
class FileStorageRetentionService:
    ...
    def scan_expired_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"File expiry scan completed: {len(expired_files)} expired, {len(expiring_soon)} expiring soon"
            )
    ...
    def purge_expired_files(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"File purge completed: {len(purged_files)} files purged, {total_bytes_freed} bytes freed"
            )
    ...
    def check_storage_quotas(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Storage quota check completed: {len(quota_violations)} violations, {len(quota_warnings)} warnings"
            )
    ...
    def set_file_retention_policy(
        self, file_id: int, retention_policy: FileRetentionPolicy
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Updated retention policy for file {file_id} to {retention_policy}"
            )
            ...
            for file_record in archive_candidates:
                try:
                    ...
                    if archive_result["success"]:
                        ...
                        # Update database record
                        self._mark_file_as_archived(
                            file_record["id"], archive_result["archive_path"]
                        )
                    else:
                    ...
                    archive_failures.append(
                        {"file_id": file_record["id"], "error": str(e)}
                    )

            logger.info(
                f"File archiving completed: {len(archived_files)} files archived, {total_bytes_archived} bytes"
            )
    ...
    def _check_file_expiry(
        self, file_record: Dict[str, Any], now: datetime
    ) -> Dict[str, Any]:
    ...
    def _check_entity_quota(self, entity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if usage_bytes > quota_bytes:
            ...
            return {"status": "warning", "usage_percentage": usage_percentage}
        else:
            return {"status": "ok", "usage_percentage": usage_percentage}
    ...
    def _calculate_expiry_date(
        self, retention_policy: FileRetentionPolicy
    ) -> Optional[datetime]:
    ...
    def _update_file_retention_policy(
        self,
        file_id: int,
