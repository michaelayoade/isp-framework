...
from app.models.auth.two_factor import (
    ApiKey,
    TwoFactorAuth,
    TwoFactorBackupCode,
    TwoFactorSession,
...
class TwoFactorService:
    ...
    def setup_totp(
        self, admin_id: int, issuer_name: str = "ISP Framework"
    ) -> Dict[str, Any]:
    ...
    def verify_totp_setup(
        self, admin_id: int, verification_code: str
    ) -> Dict[str, Any]:
        ...
        if not totp.verify(
            verification_code, valid_window=2
        ):  # Allow 2 time windows (±60 seconds)
    ...
    def verify_totp_code(self, admin_id: int, code: str, client_ip: str = None) -> bool:
        ...
        if failed_attempts >= 5:
            ...
            logger.warning(
                f"2FA locked due to failed attempts for admin ID: {admin_id}"
            )
    ...
    def verify_2fa_session(
        self, session_token: str, verification_code: str
        ...
        if not self.verify_totp_code(
            session.admin_id, verification_code, session.client_ip
        ):
            return None

        # Mark session as verified
        self.session_repo.update(
            session.id, {"is_verified": True, "verified_at": datetime.utcnow()}
        )
    ...
    def get_2fa_status(self, admin_id: int) -> Dict[str, Any]:
        ...
        if not tfa:
            return {"enabled": False, "method": None, "backup_codes_remaining": 0}
    ...
    def _generate_backup_codes(self, admin_id: int, count: int = 10) -> List[str]:
        ...
        for _ in range(count):
            ...
            code_data = {"admin_id": admin_id, "code_hash": get_password_hash(code)}
            self.backup_repo.create(code_data)

        return backup_codes

    def _verify_backup_code(
        self, admin_id: int, code: str, client_ip: str = None
    ) -> bool:
