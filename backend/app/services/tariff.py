...
class TariffService:
        ...
        try:
            ...
            if self.internet_tariff_repo.get_by_title(tariff_data.get("title")):
                raise ValidationError(
                    f"Tariff with title '{tariff_data.get('title')}' already exists"
                )

            if tariff_data.get(
                "service_name"
            ) and self.internet_tariff_repo.get_by_service_name(
                tariff_data.get("service_name")
            ):
                raise ValidationError(
                    f"Tariff with service name '{tariff_data.get('service_name')}' already exists"
                )
    ...
    def update_internet_tariff(
        self, tariff_id: int, update_data: Dict[str, Any]
    ) -> InternetTariff:
        ...
        try:
            ...
            if "title" in update_data and update_data["title"] != tariff.title:
                ...
                if existing and existing.id != tariff_id:
                    raise ValidationError(
                        f"Tariff with title '{update_data['title']}' already exists"
                    )

            if (
                "service_name" in update_data
                and update_data["service_name"] != tariff.service_name
            ):
                existing = self.internet_tariff_repo.get_by_service_name(
                    update_data["service_name"]
                )
                if existing and existing.id != tariff_id:
                    raise ValidationError(
                        f"Tariff with service name '{update_data['service_name']}' already exists"
                    )

            # Update tariff
            updated_tariff = self.internet_tariff_repo.update(tariff_id, update_data)
            logger.info(
                f"Updated Internet tariff: {updated_tariff.title} (ID: {tariff_id})"
            )
    ...
    def delete_internet_tariff(self, tariff_id: int) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Deleted Internet tariff: {tariff.title} (ID: {tariff_id})"
                )
            return success
    ...
    def search_internet_tariffs(
        ...
        tariffs, total = self.internet_tariff_repo.search_tariffs(
            filters, skip, per_page
        )
    ...
    def validate_tariff_for_service(
        self, tariff_id: int, partner_id: int = None
    ) -> bool:
