...
class ServiceIPAssignmentService:
    ...
    async def get_network_utilization(self, network_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            network = (
                self.db.query(IPv4Network).filter(IPv4Network.id == network_id).first()
            )
            if not network:
            ...
            utilization_percentage = (
                (basic_stats["active_assignments"] / total_ips * 100)
                if total_ips > 0
                else 0
            )
...
class ServiceStatusHistoryService:
    ...
    async def record_status_change(
        ...
        logger.info(
            f"Recording status change for service {service_id}: {old_status} -> {new_status}"
        )
...
class ServiceSuspensionService:
    ...
    async def restore_service(
        self, suspension_id: int, admin_id: int, restoration_notes: Optional[str] = None
    ) -> ServiceSuspension:
        ...
        try:
            ...
            # Restore suspension
            now = datetime.now(timezone.utc)
            suspension = self.suspension_repo.update(
                suspension,
                {
                    "is_active": False,
                    "restored_at": now,
