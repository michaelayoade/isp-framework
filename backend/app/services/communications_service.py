...
class TemplateService:
    ...
    def create_template(self, template_data: CommunicationTemplateCreate, created_by: int) -> CommunicationTemplate:
        ...
        logger.info(f"Created communication template: {template.name} (ID: {template.id})")
        return template
    ...
    def get_templates(
        ...
        total = query.count()
        templates = query.order_by(CommunicationTemplate.created_at.desc()).offset(skip).limit(limit).all()
        
        return templates, total
    
    def update_template(self, template_id: int, template_data: CommunicationTemplateUpdate) -> Optional[CommunicationTemplate]:
        ...
        logger.info(f"Updated communication template: {template.name} (ID: {template.id})")
        return template
    ...
    def delete_template(self, template_id: int) -> bool:
        ...
        usage_count = self.db.query(CommunicationLog).filter(
        ...
        if usage_count > 0:
            ...
            self.db.commit()
            logger.info(f"Deactivated communication template: {template.name} (ID: {template.id})")
        else:
            self.db.delete(template)
            self.db.commit()
            logger.info(f"Deleted communication template: {template.name} (ID: {template.id})")
        
        return True
    
    def render_template(self, template_id: int, variables: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            ...
            logger.error(f"Template rendering error for template {template_id}: {str(e)}")
...
class ProviderService:
    ...
    def create_provider(self, provider_data: CommunicationProviderCreate) -> CommunicationProvider:
        ...
        logger.info(f"Created communication provider: {provider.name} (ID: {provider.id})")
        return provider
    ...
    def get_providers(
        ...
        total = query.count()
        providers = query.order_by(CommunicationProvider.priority.asc()).offset(skip).limit(limit).all()
        
        return providers, total
    
    def get_default_provider(self, provider_type: CommunicationType) -> Optional[CommunicationProvider]:
    ...
    def update_provider(self, provider_id: int, provider_data: CommunicationProviderUpdate) -> Optional[CommunicationProvider]:
        ...
        logger.info(f"Updated communication provider: {provider.name} (ID: {provider.id})")
        return provider
    ...
    def delete_provider(self, provider_id: int) -> bool:
        ...
        usage_count = self.db.query(CommunicationLog).filter(
            CommunicationLog.provider_id == provider_id
        ).count()
        ...
        if usage_count > 0:
            ...
            self.db.commit()
            logger.info(f"Deactivated communication provider: {provider.name} (ID: {provider.id})")
        else:
            self.db.delete(provider)
            self.db.commit()
            logger.info(f"Deleted communication provider: {provider.name} (ID: {provider.id})")
...
class CommunicationService:
    ...
    def send_communication(self, request: SendCommunicationRequest, admin_id: Optional[int] = None) -> CommunicationLog:
        ...
        if request.template_id:
            rendered = self.template_service.render_template(request.template_id, request.template_variables)
    ...
    def send_bulk_communication(self, request: BulkCommunicationRequest, admin_id: Optional[int] = None) -> CommunicationQueue:
    ...
    def get_communication_logs(
        ...
        total = query.count()
        logs = query.order_by(CommunicationLog.created_at.desc()).offset(skip).limit(limit).all()
    ...
    def retry_failed_communication(self, log_id: int) -> bool:
        ...
        if comm_log.status not in [CommunicationStatus.FAILED, CommunicationStatus.BOUNCED]:
            return False
    ...
    def get_communication_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        # Basic stats
        total_sent = self.db.query(CommunicationLog).filter(
            CommunicationLog.created_at >= date_from
        ).count()
        
        total_delivered = self.db.query(CommunicationLog).filter(
            and_(
                CommunicationLog.created_at >= date_from,
        ...
        total_failed = self.db.query(CommunicationLog).filter(
            and_(
                CommunicationLog.created_at >= date_from,
                CommunicationLog.status.in_([CommunicationStatus.FAILED, CommunicationStatus.BOUNCED])
            )
        ).count()
        ...
        priority_stats = self.db.query(
            CommunicationLog.priority,
            func.count(CommunicationLog.id)
        ).filter(
            CommunicationLog.created_at >= date_from
        ).group_by(CommunicationLog.priority).all()
        
        # Recent activity
        recent_activity = self.db.query(CommunicationLog).filter(
            CommunicationLog.created_at >= date_from
        ).order_by(CommunicationLog.created_at.desc()).limit(10).all()
    ...
    def _send_communication_now(self, comm_log: CommunicationLog):
        ...
        try:
            ...
            if comm_log.provider_id:
                ...
                provider = self.provider_service.get_default_provider(comm_log.communication_type)
            
            if not provider:
                raise ValueError(f"No provider available for {comm_log.communication_type.value}")
            ...
            if comm_log.communication_type == CommunicationType.EMAIL:
                ...
                raise ValueError(f"Unsupported communication type: {comm_log.communication_type.value}")
    ...
    def _process_communication_queue(self, queue_id: int):
        ...
        queue = self.db.query(CommunicationQueue).filter(
            CommunicationQueue.id == queue_id
        ).first()
        ...
        try:
            for recipient in queue.recipients:
                ...
                comm_data = {
                    ...
                    "template_variables": {**queue.template_variables, **recipient.get('variables', {})},
...
class PreferenceService:
    ...
    def get_customer_preferences(self, customer_id: int) -> Optional[CommunicationPreference]:
