...
class InvoiceService:
    ...
    def _create_invoice_item(self, invoice_id: int, item_data: InvoiceItemCreate) -> InvoiceItem:
    ...
    def _calculate_and_update_totals(self, invoice_id: int):
        ...
        self.invoice_repo.update(invoice_id, {
            ...
            "balance_due": totals["total_amount"]  # Initially, balance due equals total
        })
    ...
    def _generate_invoice_number(self) -> str:
        ...
        end_of_month = datetime(year, month + 1, 1, tzinfo=timezone.utc) if month < 12 else datetime(year + 1, 1, 1, tzinfo=timezone.utc)
        
        monthly_count = self.db.query(Invoice).filter(
...
class PaymentService:
    ...
    def create_payment(self, payment_data: PaymentCreate) -> Payment:
        ...
        try:
            ...
            payment_dict["net_amount"] = payment_dict["amount"] - payment_dict.get("processing_fee", Decimal('0'))
            ...
            logger.info(f"Created payment {payment.payment_number} for invoice {invoice.invoice_number}")
            return payment
    ...
    def _process_payment(self, payment_id: int) -> Optional[Payment]:
        ...
        updated_payment = self.payment_repo.update(payment_id, {
    ...
    def _generate_payment_number(self) -> str:
        ...
        monthly_count = self.db.query(Payment).filter(
...
class CreditNoteService:
    ...
    def create_credit_note(self, credit_note_data: CreditNoteCreate) -> CreditNote:
        ...
        try:
            ...
            logger.info(f"Created credit note {credit_note.credit_note_number} for invoice {invoice.invoice_number}")
            return credit_note
    ...
    def _apply_credit_note(self, credit_note_id: int) -> Optional[CreditNote]:
        ...
        updated_credit_note = self.credit_note_repo.update(credit_note_id, {
            "status": "applied",
    ...
    def _generate_credit_note_number(self) -> str:
        ...
        monthly_count = self.db.query(CreditNote).filter(
...
class BillingManagementService:
    ...
    def get_billing_overview(self, start_date: Optional[date] = None, end_date: Optional[date] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if stats["total_amount"] > 0:
                stats["collection_rate"] = float((stats["paid_amount"] / stats["total_amount"]) * 100)
            else:
