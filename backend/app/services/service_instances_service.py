...
class CustomerServiceService:
    ...
    async def create_customer_service(
        ...
        logger.info(f"Creating customer service for customer: {service_data.get('customer_id')}")
        ...
        try:
            ...
            customer = self.db.query(Customer).filter(Customer.id == service_data['customer_id']).first()
            if not customer:
    ...
    async def activate_service(
        ...
        try:
            ...
            if service.status not in [ServiceStatus.PENDING, ServiceStatus.SUSPENDED]:
                raise ValidationError(f"Cannot activate service in {service.status} status")
    ...
    async def suspend_service(
        ...
        try:
            ...
            # Update service status
            service = self.service_repo.update(service, {
    ...
    async def terminate_service(
        ...
        try:
            ...
            # Record status change
            await self._record_status_change(service, ServiceStatus.TERMINATED, admin_id)
    ...
    async def _generate_service_identifier(self, service_type: ServiceType, customer_id: int) -> str:
    ...
    async def _handle_service_suspension(self, service: CustomerService, suspension_data: Dict[str, Any], admin_id: int):
    ...
    async def _handle_service_pre_termination(self, service: CustomerService, admin_id: int):
        """Handle pre-termination tasks"""
        # Final billing, data backup, etc.
        logger.info(f"Handling pre-termination tasks for service: {service.id}")
    
    async def _handle_service_termination(self, service: CustomerService, termination_data: Dict[str, Any], admin_id: int):
...
class CustomerInternetServiceService:
    ...
    async def create_internet_service(
        ...
        logger.info(f"Creating internet service configuration for service: {service_id}")
        ...
        try:
            ...
            customer_service = self.db.query(CustomerService).filter(CustomerService.id == service_id).first()
            ...
            logger.info(f"Internet service configuration created: {internet_service.id}")
    ...
    async def assign_ip_address(
        ...
        try:
            ...
            internet_service = self.internet_service_repo.update(internet_service, {
            ...
            await self._create_ip_assignment_record(internet_service, ip_address, assignment_type, admin_id)
    ...
    async def update_speed_profile(
        ...
        try:
            ...
            await self._apply_speed_changes(internet_service, download_speed, upload_speed)
