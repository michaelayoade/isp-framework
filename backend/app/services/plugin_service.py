...
class PluginManager:
    ...
    def __init__(self, db: Session):
        ...
        self.hooks: Dict[str, List[Tuple[int, str, int]]] = (
            {}
        )  # hook_name -> [(plugin_id, method, priority)]
        ...
        with self.lock:
            try:
                ...
                self._log_plugin_event(
                    plugin_id, "INFO", f"Plugin {plugin.name} loaded successfully"
                )
            ...
            try:
                ...
                if plugin_id in self.loaded_plugins:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Plugin cleanup failed for {plugin.name}: {str(e)}"
                        )
                ...
                self._log_plugin_event(
                    plugin_id, "INFO", f"Plugin {plugin.name} unloaded successfully"
                )
    ...
    def execute_hook(self, hook_name: str, *args, **kwargs) -> List[Any]:
        ...
        for plugin_id, method_name, priority in sorted_hooks:
            try:
                ...
                logger.error(
                    f"Hook execution failed for plugin {plugin_id}, hook {hook_name}: {str(e)}"
                )
                self._log_plugin_event(
                    plugin_id,
                    "ERROR",
        ...
        try:
            ...
            if hasattr(module, plugin.entry_point):
                ...
                if not issubclass(plugin_class, PluginInterface):
                    logger.error(
                        f"Plugin {plugin.name} does not implement PluginInterface"
                    )
                    return None
                ...
                logger.error(
                    f"Entry point {plugin.entry_point} not found in plugin {plugin.name}"
                )
                return None
    ...
    def _register_plugin_hooks(self, plugin_id: int):
        ...
        hooks = (
            self.db.query(PluginHook)
            .filter(
                and_(PluginHook.plugin_id == plugin_id, PluginHook.is_active is True)
            )
            .all()
        )
        ...
        for hook in hooks:
            ...
            self.hooks[hook.hook_name].append(
                (plugin_id, hook.callback_method, hook.priority)
            )
    ...
    def _unregister_plugin_hooks(self, plugin_id: int):
        ...
        for hook_name in list(self.hooks.keys()):
            self.hooks[hook_name] = [
                (pid, method, priority)
                for pid, method, priority in self.hooks[hook_name]
                if pid != plugin_id
            ]
...
class PluginService:
    ...
    def create_plugin(self, plugin_data: PluginCreate, installed_by: int) -> Plugin:
        ...
        plugin = Plugin(**plugin_data.dict(), installed_by=installed_by)
    ...
    def update_plugin(
        self, plugin_id: int, plugin_data: PluginUpdate
    ) -> Optional[Plugin]:
        """Update a plugin"""
    ...
    def install_plugin(
        self, install_request: PluginInstallRequest, installed_by: int
    ) -> Dict[str, Any]:
    ...
    def execute_plugin_method(
        self, plugin_id: int, execute_request: PluginExecuteRequest
    ) -> Dict[str, Any]:
    ...
    def get_plugin_stats(self) -> Dict[str, Any]:
        ...
        active_plugins = (
            self.db.query(Plugin).filter(Plugin.status == PluginStatus.ACTIVE).count()
        )
        inactive_plugins = (
            self.db.query(Plugin).filter(Plugin.status == PluginStatus.INACTIVE).count()
        )
        error_plugins = (
            self.db.query(Plugin).filter(Plugin.status == PluginStatus.ERROR).count()
        )

        # Stats by type
        type_stats = (
            self.db.query(Plugin.plugin_type, func.count(Plugin.id))
        ...
        # Stats by status
        status_stats = (
            self.db.query(Plugin.status, func.count(Plugin.id))
        ...
        top_plugins = (
            self.db.query(Plugin).order_by(Plugin.load_count.desc()).limit(10).all()
        )
