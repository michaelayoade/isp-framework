...
class SNMPMonitoringService:
    ...
    async def discover_device_info(self, ip_address: str, credentials: SNMPCredentials) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get interface count
            interface_count = await self._snmp_get(ip_address, StandardOIDs.IF_NUMBER, credentials)
            if interface_count:
    ...
    async def collect_system_metrics(self, device: ManagedDevice) -> List[SNMPMetric]:
        ...
        try:
            ...
            self.logger.error(f"Failed to collect system metrics for device {device.id}: {e}")
            return []
    
    async def collect_interface_metrics(self, device: ManagedDevice) -> List[SNMPMetric]:
        ...
        try:
            ...
            interface_count = await self._snmp_get(device.ip_address, StandardOIDs.IF_NUMBER, credentials)
            ...
            self.logger.error(f"Failed to collect interface metrics for device {device.id}: {e}")
            return []
    ...
    async def perform_health_check(self, device: ManagedDevice) -> Dict[str, Any]:
        ...
        try:
            ...
            snmp_check = await self._check_snmp_connectivity(device.ip_address, credentials)
            health_status['checks']['snmp_connectivity'] = snmp_check
            
            # System uptime check
            uptime_check = await self._check_system_uptime(device.ip_address, credentials)
            health_status['checks']['system_uptime'] = uptime_check
            
            # Interface status check
            interface_check = await self._check_interface_status(device.ip_address, credentials)
            ...
            health_status['overall_status'] = self._calculate_overall_health(health_status['checks'])
    ...
    async def discover_devices_in_subnet(self, subnet: str, credentials: SNMPCredentials) -> List[Dict[str, Any]]:
    ...
    async def _probe_device(self, ip_address: str, credentials: SNMPCredentials) -> Dict[str, Any]:
        """Probe a single IP address for SNMP response"""
        try:
            # Quick SNMP test - get system description
            value = await self._snmp_get(ip_address, StandardOIDs.SYS_DESCR, credentials, timeout=2)
            if value:
    ...
    async def _snmp_get(
        ...
        try:
            ...
            async for (errorIndication, errorStatus, errorIndex, varBinds) in getCmd(
                ...
                if errorIndication:
                    self.logger.warning(f"SNMP error for {ip_address}: {errorIndication}")
                    return None
                
                if errorStatus:
                    self.logger.warning(f"SNMP error for {ip_address}: {errorStatus.prettyPrint()}")
                    return None
    ...
    def _determine_device_type(self, sys_descr: str) -> DeviceType:
        ...
        if any(vendor in sys_descr_lower for vendor in ['cisco', 'catalyst']):
            return DeviceType.SWITCH if 'switch' in sys_descr_lower else DeviceType.ROUTER
        elif 'mikrotik' in sys_descr_lower:
            return DeviceType.ROUTER
        elif any(term in sys_descr_lower for term in ['access point', 'ap', 'wireless']):
    ...
    async def _get_or_create_interface(self, device: ManagedDevice, if_index: int) -> Optional[DeviceInterface]:
        """Get or create interface record"""
        try:
            interface = self.db.query(DeviceInterface).filter(
    ...
    async def _check_snmp_connectivity(self, ip_address: str, credentials: SNMPCredentials) -> Dict[str, Any]:
        """Check SNMP connectivity"""
        try:
            value = await self._snmp_get(ip_address, StandardOIDs.SYS_UPTIME, credentials, timeout=3)
            return {
    ...
    async def _check_system_uptime(self, ip_address: str, credentials: SNMPCredentials) -> Dict[str, Any]:
        """Check system uptime"""
        try:
            uptime = await self._snmp_get(ip_address, StandardOIDs.SYS_UPTIME, credentials)
            if uptime:
                ...
                return {
                    'status': 'failed',
                    'message': 'Could not retrieve uptime'
                }
        except Exception as e:
            return {
                'status': 'error',
    ...
    async def _check_interface_status(self, ip_address: str, credentials: SNMPCredentials) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(1, check_limit + 1):
                oper_status = await self._snmp_get(
                    ip_address, 
                    f"{StandardOIDs.IF_OPER_STATUS}.{i}", 
                    credentials
                )
    ...
    async def _check_performance_thresholds(self, device: ManagedDevice) -> Dict[str, Any]:
        """Check performance against thresholds"""
        try:
            # Get recent metrics
            recent_time = datetime.utcnow() - timedelta(minutes=15)
            recent_metrics = self.db.query(DeviceMonitoring).filter(
            ...
            alerts = []
            if cpu_metrics:
                avg_cpu = sum(float(m.metric_value) for m in cpu_metrics[-5:]) / len(cpu_metrics[-5:])
                if avg_cpu > 80:
            ...
            return {
                ...
                'alerts': alerts,
                'message': '; '.join(alerts) if alerts else 'Performance within thresholds'
            }
    ...
    async def _process_health_alerts(self, device: ManagedDevice, health_status: Dict[str, Any]):
        ...
        try:
            ...
            if device.status.value != overall_status:
                ...
                alert = DeviceAlert(
                    ...
                    severity=AlertSeverity.CRITICAL if overall_status == 'critical' else AlertSeverity.WARNING,
