...
class BillingAutomationService:
    ...
    def generate_recurring_invoices(
        self, billing_date: Optional[datetime] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for service in services_due:
                try:
                    ...
                    logger.error(
                        f"Error creating invoice for service {service['id']}: {e}"
                    )
                    errors.append(
                        {
                            "service_id": service["id"],
            ...
            logger.info(
                f"Generated {invoices_created} recurring invoices totaling ${total_amount}"
            )
    ...
    def process_payment_retries(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Processed {retries_processed} payment retries: {successful_retries} successful, {failed_retries} failed"
            )
    ...
    def calculate_reseller_commission(
        self, reseller_id: int, period_start: datetime, period_end: datetime
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            payments = self._get_reseller_payments(
            ...
            for payment in payments:
                ...
                commission_rate = commission_structure.get(
                    service_type, commission_structure.get("default", 0.10)
                )
            ...
            logger.info(
                f"Calculated commission for reseller {reseller_id}: ${total_commission} on ${total_revenue} revenue"
            )
            ...
            return {
                ...
                "commission_rate": (
                    float(total_commission / total_revenue) if total_revenue > 0 else 0
                ),
                "service_breakdown": {
                    k: {
    ...
    def _get_services_due_for_billing(
        self, billing_date: datetime.date
    ) -> List[Dict[str, Any]]:
    ...
    def _create_service_invoice(
        self, service: Dict[str, Any], billing_date: datetime.date
    ) -> Dict[str, Any]:
    ...
    def _get_current_billing_cycle(self, service: Dict[str, Any]) -> tuple:
        ...
        if now.month == 12:
            cycle_end = now.replace(year=now.year + 1, month=1, day=1) - timedelta(
                days=1
            )
        else:
    ...
    def _calculate_activation_proration(
        self,
        monthly_fee: Decimal,
    ...
    def _calculate_suspension_proration(
        self,
        monthly_fee: Decimal,
    ...
    def _calculate_termination_proration(
        self,
        monthly_fee: Decimal,
        ...
        return self._calculate_suspension_proration(
            monthly_fee, termination_date, cycle_start, cycle_end
        )

    def _calculate_plan_change_proration(
        self,
        old_amount: Decimal,
        new_amount: Decimal,
        change_date: datetime,
    ...
    def _retry_payment(self, payment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"success": False, "error_message": "Payment gateway unavailable"}
    ...
    def _get_reseller_commission_structure(self, reseller_id: int) -> Dict[str, float]:
        ...
        return {"internet": 0.15, "voice": 0.12, "bundle": 0.18, "default": 0.10}

    def _get_reseller_payments(
        self, reseller_id: int, period_start: datetime, period_end: datetime
    ) -> List[Dict[str, Any]]:
