...
from app.schemas.bundle_service import (
    BundleServiceCreate, BundleServiceUpdate, BundleServiceInDB,
    BundleServiceProvisioningRequest, BundleServiceProvisioningResponse
)
...
class BundleServiceService:
    ...
    def create_bundle_service(
        self, 
        customer_id: int, 
        bundle_data: BundleServiceCreate
    ) -> BundleServiceInDB:
    ...
    def get_bundle_service(self, service_id: int) -> Optional[BundleServiceInDB]:
        ...
        service = self.db.query(BundleService).filter(BundleService.id == service_id).first()
        if service:
    ...
    def get_customer_bundle_services(self, customer_id: int) -> List[BundleServiceInDB]:
        ...
        services = self.db.query(BundleService).filter(
            BundleService.customer_id == customer_id
        ).all()
    ...
    def update_bundle_service(
        self, 
        service_id: int, 
        update_data: BundleServiceUpdate
    ...
    def provision_bundle_service(
        self, 
        request: BundleServiceProvisioningRequest
        ...
        return BundleServiceProvisioningResponse(
            job_id=job_id,
            status="queued"
        )
    ...
    def get_bundle_service_statistics(self) -> Dict[str, Any]:
        ...
        active_services = self.db.query(BundleService).filter(
            BundleService.status == "active"
        ).count()
        pending_services = self.db.query(BundleService).filter(
            BundleService.status == "pending"
        ).count()
