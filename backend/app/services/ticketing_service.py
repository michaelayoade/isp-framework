...
class TicketService:
    ...
    def create_ticket(self, ticket_data: Dict[str, Any], created_by: int = None) -> Ticket:
    ...
    def update_ticket(self, ticket_id: int, update_data: Dict[str, Any], updated_by: int = None) -> Ticket:
    ...
    def change_ticket_status(self, ticket_id: int, new_status: str, reason: str = None, changed_by: int = None) -> Ticket:
        ...
        try:
            ...
            self._create_status_history(ticket_id, TicketStatus(new_status), changed_by, reason)
            
            logger.info(f"Changed ticket {ticket.ticket_number} status from {old_status} to {new_status}")
            return ticket
    ...
    def assign_ticket(self, ticket_id: int, assigned_to: int, assigned_team: str = None, assigned_by: int = None) -> Ticket:
        ...
        try:
            ...
            logger.info(f"Assigned ticket {ticket.ticket_number} to agent {assigned_to}")
            return ticket
    ...
    def search_tickets(self, filters: Dict[str, Any], page: int = 1, per_page: int = 20) -> Tuple[List[Ticket], int]:
        ...
        try:
            ...
            if filters.get('ticket_type'):
                query = query.filter(Ticket.ticket_type == TicketType(filters['ticket_type']))
            ...
            if filters.get('overdue'):
                ...
                if filters['overdue']:
                    query = query.filter(
                        or_(
                            and_(Ticket.first_response_due < now, Ticket.first_response_at.is_(None)),
    ...
    def get_ticket_statistics(self, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if filters:
                if filters.get('created_after'):
                    base_query = base_query.filter(Ticket.created_at >= filters['created_after'])
                if filters.get('created_before'):
                    base_query = base_query.filter(Ticket.created_at <= filters['created_before'])
            ...
            sla_performance = {
                'total_with_sla': base_query.filter(Ticket.sla_policy_id.isnot(None)).count(),
            ...
            satisfaction = base_query.filter(
                Ticket.customer_satisfaction.isnot(None)
            ).with_entities(
                func.avg(Ticket.customer_satisfaction)
            ).scalar() or 0
    ...
    def _calculate_resolution_due_date(self, priority: TicketPriority, sla_policy: SLAPolicy) -> datetime:
        ...
        return self._calculate_sla_due_date(minutes_map.get(priority, sla_policy.normal_resolution_time))
    ...
    def _create_status_history(self, ticket_id: int, status: TicketStatus, changed_by: int = None, reason: str = None):
        """Create status history entry"""
...
class TicketMessageService:
    ...
    def add_message(self, ticket_id: int, message_data: Dict[str, Any]) -> TicketMessage:
    ...
    def get_ticket_messages(self, ticket_id: int, page: int = 1, per_page: int = 20) -> Tuple[List[TicketMessage], int]:
        """Get messages for ticket with pagination"""
        try:
            query = self.db.query(TicketMessage).filter(TicketMessage.ticket_id == ticket_id)
            
            total = query.count()
            messages = query.order_by(asc(TicketMessage.created_at)).offset(
                (page - 1) * per_page
            ).limit(per_page).all()
    ...
    def _update_first_response_time(self, ticket_id: int):
        ...
        if ticket and not ticket.first_response_at:
            ...
            ticket.first_response_sla_met = (
                ticket.first_response_due is None or 
                ticket.first_response_at <= ticket.first_response_due
            )
            self.db.commit()
...
class TicketEscalationService:
    ...
    def escalate_ticket(self, ticket_id: int, escalation_data: Dict[str, Any]) -> TicketEscalation:
        """Escalate ticket to higher level"""
        try:
            escalation = TicketEscalation(
                ticket_id=ticket_id,
                escalation_reason=EscalationReason(escalation_data['escalation_reason']),
            ...
            logger.info(f"Escalated ticket {ticket_id} to level {escalation.escalation_level}")
            return escalation
...
class FieldWorkService:
    ...
    def update_field_work_status(self, work_order_id: int, status: str, update_data: Dict[str, Any] = None) -> FieldWorkOrder:
        """Update field work order status"""
        try:
            field_work = self.db.query(FieldWorkOrder).filter(FieldWorkOrder.id == work_order_id).first()
            if not field_work:
            ...
            logger.info(f"Updated field work order {field_work.work_order_number} status to {status}")
            return field_work
...
class KnowledgeBaseService:
    ...
    def create_article(self, article_data: Dict[str, Any], author_id: int) -> KnowledgeBaseArticle:
    ...
    def search_articles(self, query: str, filters: Dict[str, Any] = None, page: int = 1, per_page: int = 20) -> Tuple[List[KnowledgeBaseArticle], int]:
        ...
        try:
            ...
            if filters:
                if filters.get('category'):
                    db_query = db_query.filter(KnowledgeBaseArticle.category == filters['category'])
                
                if filters.get('is_public') is not None:
                    db_query = db_query.filter(KnowledgeBaseArticle.is_public == filters['is_public'])
            
            total = db_query.count()
            articles = db_query.order_by(desc(KnowledgeBaseArticle.view_count)).offset(
                (page - 1) * per_page
            ).limit(per_page).all()
    ...
    def assign_ticket(self, ticket_id: int, assigned_to: int, 
                     assigned_by: int, team: str = None) -> bool:
        """Assign ticket to agent/team"""
        try:
            ticket = self.db.query(Ticket).filter_by(id=ticket_id).first()
            if not ticket:
    ...
    def add_message(self, ticket_id: int, message_data: Dict[str, Any]) -> TicketMessage:
        ...
        try:
            ...
            ticket = self.db.query(Ticket).filter_by(id=ticket_id).first()
            if (ticket and not ticket.first_response_at and 
                message.author_type == 'agent' and not message.is_internal):
    ...
    def get_tickets(self, filters: Dict[str, Any] = None, 
        ...
        try:
            ...
            if filters:
                if 'date_from' in filters:
                    base_query = base_query.filter(Ticket.created_at >= filters['date_from'])
                if 'date_to' in filters:
                    base_query = base_query.filter(Ticket.created_at <= filters['date_to'])
                if 'assigned_to' in filters:
                    base_query = base_query.filter(Ticket.assigned_to == filters['assigned_to'])
            ...
            # Average response and resolution times
            avg_response_time = self.db.query(func.avg(
                func.extract('epoch', Ticket.first_response_at - Ticket.created_at) / 3600
            )).filter(
                Ticket.first_response_at.isnot(None)
            ).scalar() or 0
            
            avg_resolution_time = self.db.query(func.avg(
                func.extract('epoch', Ticket.resolved_at - Ticket.created_at) / 3600
            )).filter(
                Ticket.resolved_at.isnot(None)
            ).scalar() or 0
            ...
            # Customer satisfaction
            avg_satisfaction = self.db.query(func.avg(Ticket.customer_satisfaction))\
                .filter(Ticket.customer_satisfaction.isnot(None)).scalar() or 0
            ...
            return {
                ...
                'sla_performance': {
                    ...
                    'percentage': round((sla_met_count / resolved_tickets * 100) if resolved_tickets > 0 else 0, 1)
                },
    ...
    def _calculate_first_response_due(self, ticket: Ticket, sla_policy: SLAPolicy) -> datetime:
    ...
    def _auto_assign_ticket(self, ticket: Ticket):
        ...
        try:
            if ticket.ticket_type == TicketType.TECHNICAL:
                ...
                tech_agent = self.db.query(Administrator)\
                ...
                support_agent = self.db.query(Administrator)\
    ...
    def _create_status_history(self, ticket_id: int, previous_status, new_status, 
                             reason: str, changed_by: int, method: str):
