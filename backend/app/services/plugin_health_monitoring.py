...
class PluginHealthMonitoringService:
    ...
    async def perform_health_checks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for plugin in active_plugins:
                try:
                    ...
                    if health_result["status"] == PluginHealthStatus.CRITICAL:
                        ...
                        health_result["status"] == PluginHealthStatus.WARNING
                        and overall_status != PluginHealthStatus.CRITICAL
                    ):
                    ...
                    logger.error(
                        f"Error checking health for plugin {plugin['id']}: {e}"
                    )
                    health_result = {
                        "plugin_id": plugin["id"],
            ...
            logger.info(
                f"Health check completed: {len(health_results)} plugins checked, overall status: {overall_status}"
            )
            ...
            return {
                ...
                "healthy_plugins": len(
                    [
                        ...
                        if r["status"] == PluginHealthStatus.HEALTHY
                    ]
                ),
                "warning_plugins": len(
                    [
                        r
                        for r in health_results
                        if r["status"] == PluginHealthStatus.WARNING
                    ]
                ),
                "critical_plugins": len(
                    [
                        r
                        for r in health_results
                        if r["status"] == PluginHealthStatus.CRITICAL
                    ]
                ),
                "offline_plugins": len(
                    [
                        r
                        for r in health_results
                        if r["status"] == PluginHealthStatus.OFFLINE
                    ]
                ),
                "check_time": datetime.now(timezone.utc).isoformat(),
    ...
    async def restart_plugin(
        self, plugin_id: int, reason: str = "manual_restart"
    ) -> Dict[str, Any]:
    ...
    def get_plugin_health_status(self, plugin_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if plugin_id in self._health_cache:
                ...
                if (
                    datetime.now(timezone.utc) - cached_result["last_check"]
                ).total_seconds() < 300:
    ...
    async def _check_plugin_health(self, plugin: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            health_result["status"] = self._calculate_overall_health_status(
    ...
    async def _check_plugin_process(self, plugin: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "critical", "running": False, "error": str(e)}
    ...
    async def _check_plugin_api(self, plugin: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "critical", "error": str(e), "response_time_ms": None}
    ...
    async def _check_plugin_dependencies(
    ...
    async def _trigger_health_alert(
        self, plugin: Dict[str, Any], health_result: Dict[str, Any]
    ):
    ...
    async def _stop_plugin(self, plugin: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Mock plugin stop
        return {"success": True, "stop_time": datetime.now(timezone.utc).isoformat()}
