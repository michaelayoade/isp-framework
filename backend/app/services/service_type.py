...
class ServiceTypeService:
    ...
    def get_service_type(self, service_type_id: int) -> ServiceType:
        ...
        service_type = (
            self.db.query(ServiceType).filter(ServiceType.id == service_type_id).first()
        )
        if not service_type:
    ...
    def create_service_type(self, service_type_data: ServiceTypeCreate) -> ServiceType:
        ...
        if existing:
            raise DuplicateError(
                f"Service type with code '{service_type_data.code}' already exists"
            )

        # Create service type
        service_type_dict = service_type_data.model_dump()
        service_type_dict["is_system"] = (
            False  # User-created service types are not system types
        )
    ...
    def update_service_type(
        self, service_type_id: int, service_type_data: ServiceTypeUpdate
    ) -> ServiceType:
    ...
    def delete_service_type(self, service_type_id: int):
        ...
        if service_type.is_system:
            raise ValidationError(
                f"System service type '{service_type.code}' cannot be deleted"
            )
        ...
        plans_using_type = (
            ...
            .filter(ServicePlan.service_type_id == service_type_id)
            .count()
        )

        if plans_using_type > 0:
            raise ValidationError(
                f"Service type '{service_type.code}' is in use by {plans_using_type} service plans and cannot be deleted"
            )
        ...
        try:
            ...
            services_using_type = (
                ...
                .filter(CustomerService.service_type_id == service_type_id)
                .count()
            )

            if services_using_type > 0:
                raise ValidationError(
                    f"Service type '{service_type.code}' is in use by {services_using_type} customer services and cannot be deleted"
                )
        except ImportError:
