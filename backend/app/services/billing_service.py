...
class BillingAccountService:
    ...
    def create_billing_account(
        ...
        try:
            ...
            logger.info(f"Created billing account {account_number} for customer {customer_id}")
    ...
    def get_account_balance(self, account_id: int) -> Dict[str, Decimal]:
        ...
        return {
            ...
            "total_balance": (account.available_balance or Decimal('0.00')) + (account.reserved_balance or Decimal('0.00')),
    ...
    def update_balance(
        ...
        try:
            ...
            logger.info(f"Updated balance for account {account_id}: {balance_before} -> {balance_after}")
            return transaction
    ...
    def _get_account(self, account_id: int) -> CustomerBillingAccount:
        ...
        account = self.db.query(CustomerBillingAccount).filter(
            CustomerBillingAccount.id == account_id
        ).first()
    ...
    def _get_transaction_category(self, transaction_type: TransactionType) -> TransactionCategory:
...
class InvoiceService:
    ...
    def create_invoice(
        ...
        try:
            ...
            logger.info(f"Created invoice {invoice_number} for account {billing_account_id}")
            return invoice
    ...
    def apply_proration(
        ...
        try:
            ...
            logger.info(f"Applied proration factor {proration_factor} to invoice item {invoice_item.id}")
            return invoice_item
...
class PaymentService:
    ...
    def create_payment_method(
        ...
        try:
            ...
            logger.info(f"Created payment method {method_type} for billing account {billing_account_id}")
...
class BillingReportService:
    ...
    def get_account_summary(self, account_id: int) -> Dict[str, Any]:
        ...
        transactions = self.db.query(BillingTransaction).filter(
            BillingTransaction.billing_account_id == account_id
        ).all()
        ...
        return {
            ...
            "transaction_summary": {
                ...
                "total_credits": sum(t.amount for t in transactions if t.transaction_type in [TransactionType.CREDIT, TransactionType.PAYMENT]),
                "total_debits": sum(t.amount for t in transactions if t.transaction_type in [TransactionType.DEBIT, TransactionType.CHARGE])
            },
            "invoice_summary": {
                "total_invoices": len(invoices),
                "total_amount": sum(i.total_amount for i in invoices),
                "paid_invoices": len([i for i in invoices if i.invoice_status == InvoiceStatus.PAID]),
                "overdue_invoices": len([i for i in invoices if i.invoice_status == InvoiceStatus.OVERDUE])
            },
            "payment_summary": {
                "total_payments": len(payments),
                "total_amount": sum(p.amount for p in payments if p.payment_status == PaymentStatus.COMPLETED),
