...
class ModularNetworkProvisioningService:
    ...
    def provision_customer_service(self, customer_id: int, service_plan_id: int, 
        ...
        """
        try:
            logger.info(f"Starting modular service provisioning for customer {customer_id}")
            ...
            # Allocate IP address
            ip_allocation = self._allocate_ip_address(customer_id, assigned_device.get('id'))
            ...
            logger.info(f"Modular service provisioning completed for customer {customer_id}")
            return result
            
        except Exception as e:
            logger.error(f"Modular service provisioning failed for customer {customer_id}: {str(e)}")
            self.db.rollback()
            raise ValidationError(f"Service provisioning failed: {str(e)}")
    
    def _assign_network_device(self, customer_id: int, device_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Device assignment failed for customer {customer_id}: {str(e)}")
            raise ValidationError(f"Device assignment failed: {str(e)}")
    
    def _allocate_ip_address(self, customer_id: int, device_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    def deprovision_customer_service(self, customer_service_id: int) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"Starting service deprovisioning for customer service {customer_service_id}")
            ...
            allocations = self.db.query(IPAllocation).filter(
                IPAllocation.customer_id == customer_service.customer_id
            ).all()
            ...
            logger.info(f"Service deprovisioning completed for customer service {customer_service_id}")
            return result
    ...
    def get_customer_network_info(self, customer_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            services = self.db.query(CustomerService).filter(
                CustomerService.customer_id == customer_id
            ).all()
            
            # Get IP allocations
            allocations = self.db.query(IPAllocation).filter(
                IPAllocation.customer_id == customer_id
            ).all()
            ...
            logger.error(f"Failed to get network info for customer {customer_id}: {str(e)}")
