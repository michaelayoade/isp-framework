...
from ..models.services.instances import CustomerService, InternetService
...
class NetworkProvisioningService:
    ...
    def provision_customer_service(
        ...
        try:
            logger.info(
                f"Starting network provisioning for customer {customer_id}, service plan {service_plan_id}"
            )
            ...
            customer_service = self._create_customer_service_assignment(
                ...
                service_plan_id,
                assigned_router.id,
                assigned_sector.id if assigned_sector else None,
            )
            ...
            return {
                ...
                "assigned_sector": (
                    {"id": assigned_sector.id, "name": assigned_sector.name}
                    if assigned_sector
                    else None
                ),
            ...
            logger.error(
                f"Error provisioning network service for customer {customer_id}: {str(e)}"
            )
            raise

    def deprovision_customer_service(
        ...
        try:
            logger.info(
                f"Starting network deprovisioning for customer {customer_id}, service plan {service_plan_id}"
            )
            ...
            if not customer_service:
                logger.warning(
                    f"No active service assignment found for customer {customer_id}"
                )
                return False
            ...
            logger.error(
                f"Error deprovisioning network service for customer {customer_id}: {str(e)}"
            )
            return False
    ...
    def get_customer_network_status(self, customer_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for service in active_services:
                ...
                service_plan = (
                    self.db.query(ServicePlan)
                    .filter(ServicePlan.id == service.service_plan_id)
                    .first()
                )
            ...
            logger.error(
                f"Error getting network status for customer {customer_id}: {str(e)}"
            )
    ...
    def get_router_utilization(self, router_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            assigned_ipv4 = (
                ...
                .filter(
                    and_(IPv4IP.router_id == router_id, IPv4IP.status == "assigned")
                )
                .count()
            )
            ...
            total_ipv4 = (
                self.db.query(IPv4IP).filter(IPv4IP.router_id == router_id).count()
            )

            # Calculate utilization percentages
            customer_utilization = (
                (active_customers / router.max_customers * 100)
            ...
            logger.error(
                f"Error getting router utilization for router {router_id}: {str(e)}"
            )
            raise

    # Private helper methods

    def _assign_router_and_sector(
        self,
        customer_id: int,
        service_plan: ServicePlan,
        router_id: Optional[int],
        sector_id: Optional[int],
    ) -> Tuple[Optional[Router], Optional[RouterSector]]:
        """
            ...
            if router_id:
                ...
                if not assigned_router or assigned_router.status != "active":
                    raise ValidationError(
                        f"Requested router {router_id} is not available"
                    )
            else:
                ...
                if not available_routers:
                    raise ValidationError(
                        f"No routers available for service type {service_plan.service_type}"
                    )
    ...
    def _assign_ipv4_address(self, customer_id: int, router_id: int) -> Optional[str]:
        ...
        try:
            ...
            available_ip = (
                self.db.query(IPv4IP)
                .filter(
                    and_(IPv4IP.router_id == router_id, IPv4IP.status == "available")
                )
                .first()
            )
            ...
            if available_ip:
                ...
                logger.info(
                    f"Assigned IPv4 {available_ip.ip_address} to customer {customer_id}"
                )
    ...
    def _assign_ipv6_address(self, customer_id: int, router_id: int) -> Optional[str]:
        ...
        try:
            ...
            available_ip = (
                self.db.query(IPv6IP)
                .filter(
                    and_(IPv6IP.router_id == router_id, IPv6IP.status == "available")
                )
                .first()
            )
            ...
            if available_ip:
                ...
                logger.info(
                    f"Assigned IPv6 {available_ip.ip_address} to customer {customer_id}"
                )
                return available_ip.ip_address
    ...
    def _create_customer_service_assignment(
        self,
        customer_id: int,
        service_plan_id: int,
        router_id: int,
    ...
    def _apply_network_configuration(
        ...
        try:
            logger.info(
                f"Applying network configuration for customer {customer_id} on router {router.name}"
            )
    ...
    def _remove_network_configuration(self, customer_service: CustomerService) -> bool:
        ...
        try:
            logger.info(
                f"Removing network configuration for customer service {customer_service.id}"
            )
    ...
    def _release_customer_ip_addresses(self, customer_id: int) -> None:
        ...
        try:
            ...
            ipv4_addresses = (
                self.db.query(IPv4IP).filter(IPv4IP.customer_id == customer_id).all()
            )
            ...
            ipv6_addresses = (
                self.db.query(IPv6IP).filter(IPv6IP.customer_id == customer_id).all()
            )
            ...
            logger.error(
                f"Error releasing IP addresses for customer {customer_id}: {str(e)}"
            )
    ...
    def _get_customer_assigned_ips(self, customer_id: int) -> Dict[str, List[str]]:
        ...
        try:
            ...
            logger.error(
                f"Error getting assigned IPs for customer {customer_id}: {str(e)}"
            )
            return {"ipv4": [], "ipv6": []}
