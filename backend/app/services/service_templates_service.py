...
class ServiceTemplateService:
    ...
    async def create_service_template(
        ...
        try:
            ...
            raise ValidationError(
                "Failed to create service template due to data constraints"
            )
    ...
    async def update_service_template(
        self, template_id: int, update_data: Dict[str, Any], admin_id: int
    ) -> ServiceTemplate:
        ...
        try:
            ...
            if "code" in update_data and update_data["code"] != template.code:
                ...
                if existing_template:
                    raise ValidationError(
                        f"Service template with code '{update_data['code']}' already exists"
                    )
            ...
            if "available_locations" in update_data:
                await self._validate_location_availability(
                    update_data["available_locations"]
                )
    ...
    async def get_available_templates(
        ...
        logger.info(
            f"Getting available templates for location: {location_id}, type: {service_type}"
        )

        try:
            templates = self.template_repo.get_available_templates(
                location_id=location_id, service_type=service_type, category=category
            )

            # Additional customer-specific filtering if needed
            if customer_id:
                templates = await self._filter_templates_for_customer(
    ...
    async def activate_template(
        self, template_id: int, admin_id: int
    ) -> ServiceTemplate:
    ...
    def _validate_template_data(self, template_data: Dict[str, Any]):
        ...
        if template_data["service_type"] not in ServiceType:
            raise ValidationError(
                f"Invalid service type: {template_data['service_type']}"
            )
    ...
    async def _validate_tariff_reference(
        self, tariff_id: int, service_type: ServiceType
    ):
        ...
        if service_type == ServiceType.INTERNET:
            tariff = (
                self.db.query(InternetTariff)
                .filter(InternetTariff.id == tariff_id)
                .first()
            )
            if not tariff:
    ...
    async def _filter_templates_for_customer(
        self, templates: List[ServiceTemplate], customer_id: int
    ) -> List[ServiceTemplate]:
    ...
    async def _validate_template_activation(self, template: ServiceTemplate):
        ...
        if template.tariff_id:
            await self._validate_tariff_reference(
                template.tariff_id, template.service_type
            )
    ...
    async def _validate_template_deactivation(self, template: ServiceTemplate):
        ...
        active_count = len(
            [s for s in active_services if s.status == ServiceStatus.ACTIVE]
        )
        if active_count > 0:
            raise ValidationError(
...
class InternetServiceTemplateService:
    ...
    async def create_internet_template(
        ...
        logger.info(
            f"Creating internet service template: {template_data.get('download_speed')}Mbps"
        )
    ...
    async def get_templates_by_speed_range(
        ...
        try:
            ...
            raise BusinessLogicError(
                f"Failed to get templates by speed range: {str(e)}"
            )
...
class VoiceServiceTemplateService:
    ...
    async def create_voice_template(
        ...
        logger.info(
            f"Creating voice service template: {template_data.get('included_minutes')} minutes"
        )
    ...
    def _validate_voice_template_data(self, template_data: Dict[str, Any]):
        ...
        if not template_data.get("unlimited_minutes", False):
            if (
                "included_minutes" not in template_data
                or template_data["included_minutes"] <= 0
            ):
                raise ValidationError(
                    "Included minutes must be greater than 0 for non-unlimited plans"
                )
...
class BundleServiceTemplateService:
    ...
    async def create_bundle_template(
        ...
        logger.info(
            f"Creating bundle service template with {len(template_data.get('included_services', []))} services"
        )
    ...
    def _validate_bundle_template_data(self, template_data: Dict[str, Any]):
        ...
        if (
            not template_data.get("included_services")
            or len(template_data["included_services"]) < 2
        ):
            raise ValidationError("Bundle must include at least 2 services")

        # Validate discount settings
        if (
            template_data.get("discount_percentage", 0) < 0
            or template_data.get("discount_percentage", 0) > 100
        ):
