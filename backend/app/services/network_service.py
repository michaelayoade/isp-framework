...
from ..models.networking.ipam import DHCPReservation, IPAllocation, IPPool
from ..models.networking.networks import DeviceConnection, NetworkDevice, NetworkSite
...
class NetworkSiteService:
    ...
    def get_site_with_devices(self, site_id: int) -> Optional[NetworkSite]:
        ...
        return self.db.query(NetworkSite).filter(NetworkSite.id == site_id).first()
    ...
    def get_pool_utilization(self, pool_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "utilization_percentage": (
                (allocated_count / total_ips) * 100 if total_ips > 0 else 0
            ),
        }
...
def get_network_services(
    db: Session, webhook_triggers: WebhookTriggers
) -> Dict[str, Any]:
    """
...
def get_network_topology_service(
