...
class DeviceService:
    ...
    def create_device(self, device_data: DeviceCreate, auto_registered: bool = False) -> Device:
        """Create a new device."""
        try:
            ...
            existing = self.db.query(Device).filter(Device.mac_address == normalized_mac).first()
            if existing:
                raise ValueError(f"Device with MAC address {normalized_mac} already exists")
            
            # Verify customer exists
            customer = self.db.query(Customer).filter(Customer.id == device_data.customer_id).first()
            if not customer:
                raise ValueError(f"Customer with ID {device_data.customer_id} not found")
            ...
            logger.info(f"Created device {device.id} with MAC {normalized_mac} for customer {device_data.customer_id}")
            return device
    ...
    def get_device_by_mac(self, mac_address: str) -> Optional[Device]:
        ...
        try:
            ...
            return self.db.query(Device).options(
                joinedload(Device.customer)
            ).filter(Device.mac_address == normalized_mac).first()
        except ValueError:
            return None
    ...
    def approve_device(self, device_id: int, approval_data: DeviceApproval, approved_by: int) -> Optional[Device]:
    ...
    def list_devices(self, filters: DeviceFilters, sort: DeviceSort, page: int = 1, per_page: int = 50) -> Tuple[List[Device], int]:
        ...
        if filters.is_auto_registered is not None:
            query = query.filter(Device.is_auto_registered == filters.is_auto_registered)
            ...
            if not device:
                ...
                if request.service_id:
                    ...
                    if service:
                        ...
                        # Check device limits
                        if self._check_device_limits(service.customer_id, service.max_devices):
                            device.status = "active"
    ...
    def _build_radius_response(self, device: Device, service: CustomerService, access_granted: bool) -> RadiusDeviceResponse:
        ...
        return RadiusDeviceResponse(
            ...
            vlan_id=service.internet_config.vlan_id if service.internet_config else None
        )
    
    # Bulk Operations
    def bulk_approve_devices(self, bulk_data: DeviceBulkApproval, approved_by: int) -> DeviceBulkResponse:
    ...
    def bulk_update_devices(self, bulk_data: DeviceBulkUpdate) -> DeviceBulkResponse:
        ...
        update_data = DeviceUpdate(
            ...
            device_type=bulk_data.device_type
        )
