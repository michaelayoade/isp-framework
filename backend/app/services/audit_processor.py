...
from app.models.audit import AuditProcessingStatus, AuditQueue
...
class AuditProcessor:
    ...
    def __init__(
        self, processor_name: str = "default_audit_processor", batch_size: int = 100
    ):
    ...
    async def start_processing(self, db: Session):
        ...
        try:
            ...
            logger.error(
                f"Audit processor {self.processor_name} encountered error: {e}"
            )
    ...
    async def _process_audit_entry(self, db: Session, entry: AuditQueue):
        ...
        try:
            ...
            logger.debug(
                f"Processed audit entry for {entry.table_name}.{entry.record_id}"
            )
    ...
    def _build_change_data(self, entry: AuditQueue) -> Dict[str, Any]:
        ...
        if entry.operation == "INSERT":
            ...
            for field in all_fields:
                ...
                if old_val != new_val:
                    change_data["changes"][field] = {"old": old_val, "new": new_val}
        elif entry.operation == "DELETE":
            change_data["data"] = entry.old_values or {}

        return change_data

    async def _update_processor_status(
        self, db: Session, status: str, error_message: str = None
    ):
        """Update processor status in database."""
        try:
            processor_status = (
                db.query(AuditProcessingStatus)
        ...
        if stats["start_time"]:
            ...
            if stats["processed_count"] > 0 and runtime.total_seconds() > 0:
                stats["processing_rate"] = round(
                    stats["processed_count"] / runtime.total_seconds(), 2
                )
            else:
    ...
    def get_processor_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "running_processors": sum(
                1 for p in self.processors.values() if p.is_running
            ),
            "processors": {},
        }
...
async def start_default_audit_processor():
    ...
    await audit_processor_manager.start_processor(
        "default_audit_processor", batch_size=100
    )
...
def get_audit_processing_health(db: Session) -> Dict[str, Any]:
    ...
    return {
        ...
        "recommendations": _get_health_recommendations(
            pending_count, failed_count, len(active_processors)
        ),
    }


def _get_health_recommendations(
    pending_count: int, failed_count: int, active_processors: int
) -> List[str]:
    ...
    if pending_count > 1000:
        recommendations.append(
            "High pending queue - consider starting additional processors"
        )
    ...
    if active_processors == 0:
        recommendations.append(
            "No active processors - start audit processing immediately"
        )
