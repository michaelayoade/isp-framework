...
class CustomerStatusService:
    ...
    def get_customer_status(self, status_id: int) -> CustomerStatus:
        ...
        status = self.db.query(CustomerStatus).filter(CustomerStatus.id == status_id).first()
        if not status:
    ...
    def create_customer_status(self, status_data: CustomerStatusCreate) -> CustomerStatus:
        """Create a new customer status."""
        # Check if code already exists
        existing = self.get_customer_status_by_code(status_data.code)
        if existing:
            raise DuplicateError(f"Customer status with code '{status_data.code}' already exists")
    ...
    def update_customer_status(self, status_id: int, status_data: CustomerStatusUpdate) -> CustomerStatus:
    ...
    def delete_customer_status(self, status_id: int):
        ...
        if status.is_system:
            raise ValidationError(f"System customer status '{status.code}' cannot be deleted")
        
        # Check if it's in use
        from app.models.customer.base import Customer
        customers_using_status = self.db.query(Customer).filter(
            Customer.status_id == status_id
        ).count()
        
        if customers_using_status > 0:
            raise ValidationError(f"Customer status '{status.code}' is in use by {customers_using_status} customers and cannot be deleted")
    ...
    def get_default_customer_status(self) -> CustomerStatus:
        ...
        if not default_status:
            ...
            default_status = self.db.query(CustomerStatus).filter(
                CustomerStatus.is_active is True
            ).order_by(CustomerStatus.sort_order).first()
