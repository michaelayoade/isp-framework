...
from app.schemas.reseller import (
    ResellerCreate, ResellerUpdate, ResellerResponse, ResellerStats,
...
class ResellerService:
    ...
    def create_reseller(self, reseller_data: ResellerCreate) -> ResellerResponse:
        ...
        if existing_reseller:
            raise DuplicateError(f"Reseller with code '{reseller_data.code}' already exists")
        
        # Check if email already exists
        existing_email = self.repository.get_by_email(reseller_data.email)
        if existing_email:
            raise DuplicateError(f"Reseller with email '{reseller_data.email}' already exists")
    ...
    def update_reseller(self, reseller_id: int, update_data: ResellerUpdate) -> ResellerResponse:
        ...
        if update_data.email and update_data.email != reseller.email:
            ...
            if existing_email:
                raise DuplicateError(f"Reseller with email '{update_data.email}' already exists")
    ...
    def delete_reseller(self, reseller_id: int) -> bool:
        ...
        if customer_count > 0:
            raise ValidationError(f"Cannot delete reseller with {customer_count} assigned customers")
    ...
    def get_reseller_commission_report(self, reseller_id: int, start_date: datetime, end_date: datetime) -> ResellerCommissionReport:
        """Generate commission report for reseller"""
        logger.info(f"Generating commission report: reseller_id={reseller_id}, period={start_date} to {end_date}")
        
        reseller = self.repository.get_by_id(reseller_id)
        if not reseller:
            raise NotFoundError(f"Reseller with ID {reseller_id} not found")
        
        commission_data = self.repository.get_reseller_commission_data(reseller_id, start_date, end_date)
    ...
    def get_reseller_dashboard(self, reseller_id: int) -> ResellerDashboard:
        ...
        commission_summary = self.get_reseller_commission_report(reseller_id, month_start, now)
    ...
    def assign_customer_to_reseller(self, customer_id: int, reseller_id: int) -> bool:
        ...
        if not success:
            raise ValidationError("Failed to assign customer to reseller (customer limit reached or customer not found)")
        
        logger.info(f"Customer {customer_id} assigned to reseller {reseller_id} successfully")
        return True
