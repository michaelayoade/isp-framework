...
class APIQuotaManagementService:
    ...
    def check_rate_limit(self, api_key: str, endpoint: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'allowed': True,
                'remaining_calls': self._get_remaining_calls(api_key, endpoint, rate_limits),
    ...
    def check_quota_limits(self, api_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check daily quota
            if api_key_config['daily_quota'] and daily_usage >= api_key_config['daily_quota']:
                return {
                    'within_quota': False,
                    'quota_type': 'daily',
                    'usage': daily_usage,
            ...
            # Check monthly quota
            if api_key_config['monthly_quota'] and monthly_usage >= api_key_config['monthly_quota']:
                return {
                    'within_quota': False,
                    'quota_type': 'monthly',
                    'usage': monthly_usage,
    ...
    def aggregate_daily_usage(self, date: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            usage_by_key = defaultdict(lambda: {
                'total_calls': 0,
                'successful_calls': 0,
                'failed_calls': 0,
            ...
            logger.info(f"Aggregated daily usage for {date}: {len(aggregation_results)} API keys processed")
            
            return {
                'date': date.isoformat(),
                'api_keys_processed': len(aggregation_results),
                'total_calls': sum(stats['total_calls'] for stats in usage_by_key.values()),
    ...
    def rotate_api_key(self, api_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'old_api_key_expires_at': (datetime.now(timezone.utc) + timedelta(hours=24)).isoformat(),
    ...
    def _get_rate_limit_rules(self, api_key_config: Dict[str, Any], endpoint: str) -> List[Dict[str, Any]]:
    ...
    def _check_individual_rate_limit(self, api_key: str, endpoint: str, rule: Dict[str, Any]) -> bool:
    ...
    def _get_remaining_calls(self, api_key: str, endpoint: str, rate_limits: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    def _get_next_reset_time(self, rate_limits: List[Dict[str, Any]]) -> str:
        ...
        now = datetime.now(timezone.utc)
        next_reset = now + timedelta(seconds=min(rule['window_seconds'] for rule in rate_limits))
    ...
    def _store_daily_aggregation(self, api_key: str, date: datetime.date, stats: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _get_usage_data(self, api_key: str, period_start: datetime) -> List[Dict[str, Any]]:
        """Get usage data for an API key over a period."""
        # Placeholder - implement when usage repository is available
        return []
    
    def _calculate_usage_analytics(self, usage_data: List[Dict[str, Any]], period_days: int) -> Dict[str, Any]:
