...
class ResellerAuthService:
    ...
    def create_access_token(
        ...
        if expires_delta:
            ...
            expire = datetime.now(timezone.utc) + timedelta(
                minutes=RESELLER_ACCESS_TOKEN_EXPIRE_MINUTES
            )
    ...
    def verify_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            payload = jwt.decode(
                token, RESELLER_SECRET_KEY, algorithms=[RESELLER_ALGORITHM]
            )
            return payload
    ...
    def change_reseller_password(
        self, reseller_id: int, current_password: str, new_password: str
    ) -> bool:
    ...
    def get_reseller_permissions(self, reseller_id: int) -> Dict[str, Any]:
        ...
        customer_limit_reached = (
            reseller.customer_limit is not None
            and customer_count >= reseller.customer_limit
        )
