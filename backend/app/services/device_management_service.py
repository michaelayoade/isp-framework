...
class DeviceDiscoveryService:
    ...
    async def discover_devices_by_subnet(self, subnet: str, snmp_community: str = "public") -> List[ManagedDevice]:
    ...
    async def register_device(self, device_data: Dict[str, Any]) -> ManagedDevice:
        ...
        logger.info(f"Registered new device: {device.hostname} ({device.management_ip})")
        return device
...
class DeviceMonitoringService:
    ...
    async def collect_device_metrics(self, device_id: int) -> Dict[str, Any]:
        ...
        device = self.db.query(ManagedDevice).filter(ManagedDevice.id == device_id).first()
        if not device:
        ...
        try:
            ...
            await self._create_alert(device_id, "device_unreachable", AlertSeverity.CRITICAL,
                                   f"Device {device.hostname} is unreachable", str(e))
    ...
    async def get_device_performance_history(self, device_id: int, metric_name: str, 
                                           hours: int = 24) -> List[DeviceMonitoring]:
        """Get performance history for a device metric"""
        start_time = datetime.utcnow() - timedelta(hours=hours)
        
        return self.db.query(DeviceMonitoring).filter(
            and_(
                DeviceMonitoring.device_id == device_id,
                DeviceMonitoring.metric_name == metric_name,
                DeviceMonitoring.timestamp >= start_time
            )
        ).order_by(DeviceMonitoring.timestamp).all()
    
    async def _create_alert(self, device_id: int, alert_type: str, severity: AlertSeverity,
                          title: str, description: str):
        """Create a device alert"""
        # Check if similar alert already exists and is active
        existing_alert = self.db.query(DeviceAlert).filter(
            and_(
...
class DeviceConfigService:
    ...
    async def backup_device_config(self, device_id: int, backup_type: str = "full") -> DeviceConfigBackup:
        ...
        try:
            ...
            logger.error(f"Failed to backup config for device {device.hostname}: {str(e)}")
    ...
    async def _is_backup_due(self, device: ManagedDevice) -> bool:
        ...
        last_backup = self.db.query(DeviceConfigBackup).filter(
            and_(
                DeviceConfigBackup.device_id == device.id,
                DeviceConfigBackup.backup_status == BackupStatus.SUCCESS
            )
        ).order_by(desc(DeviceConfigBackup.completed_at)).first()
...
class DeviceHealthService:
    ...
    async def check_device_health(self, device_id: int) -> Dict[str, Any]:
        ...
        # Performance checks
        health_status['checks']['performance'] = await self._check_performance_thresholds(device)
    ...
    async def _check_performance_thresholds(self, device: ManagedDevice) -> Dict[str, Any]:
        ...
        if cpu_metrics:
            ...
            if avg_cpu > 90:
                ...
                performance_status['warnings'].append(f"Elevated CPU usage: {avg_cpu:.1f}%")
        ...
        if memory_metrics:
            ...
            if avg_memory > 95:
                performance_status['critical'].append(f"High memory usage: {avg_memory:.1f}%")
            elif avg_memory > 85:
                performance_status['warnings'].append(f"Elevated memory usage: {avg_memory:.1f}%")
