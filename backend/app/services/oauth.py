...
from app.models import Administrator, OAuthAuthorizationCode, OAuthClient, OAuthToken
...
class OAuthService:
    ...
    def create_oauth_client(
        ...
        client_data = {
            "client_id": client_id,
            "client_secret": (
                get_password_hash(client_secret) if client_secret else None
            ),
    ...
    def validate_grant_type(self, client: OAuthClient, grant_type: str) -> bool:
        ...
        if grant_type not in allowed_grants:
            logger.warning(
                f"Grant type '{grant_type}' not allowed for client: {client.client_id}"
            )
            return False
    ...
    def validate_scope(self, client: OAuthClient, requested_scope: str = None) -> bool:
        ...
        if not requested_scopes.issubset(allowed_scopes):
            ...
            logger.warning(
                f"Invalid scopes {invalid_scopes} for client: {client.client_id}"
            )
            return False
    ...
    def refresh_access_token(self, refresh_token: str) -> Optional[OAuthToken]:
        ...
        new_token = self.generate_access_token(
            client=client, user=user, scope=old_token.scope
        )
    ...
    def revoke_token(self, token: str) -> bool:
        ...
        if oauth_token and not oauth_token.is_revoked:
            update_data = {"is_revoked": True, "revoked_at": datetime.now(timezone.utc)}
    ...
    def password_grant_flow(
        self, client: OAuthClient, username: str, password: str, scope: str = None
    ) -> Optional[Dict[str, Any]]:
        """Handle OAuth password grant flow"""
        # Validate grant type
        if not self.validate_grant_type(client, "password"):
            raise ValueError(
                "invalid_grant: password grant not allowed for this client"
            )
        ...
        # Generate token
        token = self.generate_access_token(client=client, user=user, scope=scope)
    ...
    def client_credentials_flow(
        self, client: OAuthClient, scope: str = None
    ) -> Dict[str, Any]:
        """Handle OAuth client credentials flow"""
        # Validate grant type
        if not self.validate_grant_type(client, "client_credentials"):
            raise ValueError(
                "invalid_grant: client_credentials grant not allowed for this client"
            )
        ...
        if not client.is_confidential:
            logger.warning(
                f"Public client {client.client_id} attempted client_credentials flow"
            )
            raise ValueError(
                "invalid_client: public clients cannot use client_credentials grant"
            )

        # Generate token without user association
        token = self.generate_access_token(client=client, user=None, scope=scope)
    ...
    def exchange_authorization_code(
        ...
        if auth_code.redirect_uri != redirect_uri:
            logger.warning(
                f"Redirect URI mismatch for authorization code: {code[:10]}..."
            )
            return None
        ...
        # Mark code as used
        self.code_repo.update(
            auth_code.id, {"is_used": True, "used_at": datetime.now(timezone.utc)}
        )
        ...
        token = self.generate_access_token(
            client=client, user=user, scope=auth_code.scope
        )
