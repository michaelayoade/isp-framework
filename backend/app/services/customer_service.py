...
class CustomerServiceService:
    ...
    def get_customer_service(self, service_id: int):
        ...
        if service.discount_percentage:
            effective_price = int(
                effective_price * (100 - service.discount_percentage) / 100
            )
    ...
    def create_customer_service(
        self, service_data: CustomerServiceCreate, admin_id: int
    ):
        ...
        try:
            ...
            if not customer:
                raise NotFoundError(
                    f"Customer with id {service_data.customer_id} not found"
                )
            ...
            if not service_plan:
                raise NotFoundError(
                    f"Service plan with id {service_data.service_plan_id} not found"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to trigger customer_service.created webhook: {e}"
                )
    ...
    def update_customer_service(
        self, service_id: int, service_data: CustomerServiceUpdate, admin_id: int
    ):
        ...
        try:
            ...
            update_dict = {
                k: v for k, v in service_data.dict().items() if v is not None
            }
            ...
            try:
                ...
                logger.warning(
                    f"Failed to trigger customer_service.updated webhook: {e}"
                )
    ...
    def search_customer_services(self, search_params: CustomerServiceSearch):
        ...
        try:
            ...
            services, total_count = self.customer_service_repo.search_customer_services(
                search_dict
            )
    ...
    def get_customer_services_overview(
        ...
        try:
            ...
            overview_data = self.customer_service_repo.get_customer_services_overview(
                customer_id
            )
            ...
            try:
                ...
                logger.warning(
                    f"Failed to trigger customer_services.overview webhook: {e}"
                )

            logger.info(
                f"Retrieved customer services overview for customer: {customer_id}"
            )
            return CustomerServicesOverview(**overview_data)

        except Exception as e:
            logger.error(
                f"Error getting customer services overview for {customer_id}: {str(e)}"
            )
            raise
    ...
    def get_service_plan_assignments(
        ...
        try:
            ...
            overview_data = (
                self.customer_service_repo.get_service_plan_assignments_overview(
                    service_plan_id
                )
            )
            ...
            logger.info(
                f"Retrieved service plan assignments for plan: {service_plan_id}"
            )
            return ServicePlanAssignments(**overview_data)

        except Exception as e:
            logger.error(
                f"Error getting service plan assignments for {service_plan_id}: {str(e)}"
            )
            raise

    def provision_service(
        self, provision_request: ServiceProvisioningRequest, admin_id: int
    ):
        ...
        try:
            ...
            estimated_activation = datetime.now(timezone.utc) + timedelta(
                hours=2
    ...
    def terminate_service(
        self, service_id: int, admin_id: int, end_date: Optional[datetime] = None
    ):
        ...
        try:
            ...
            logger.info(
                f"Customer service terminated: {service_id} by admin {admin_id}"
            )
            return service
    ...
    def get_expiring_services(self, days_ahead: int = 30):
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(services)} services expiring within {days_ahead} days"
            )
            return services
    ...
    def get_revenue_analytics(self, service_plan_id: Optional[int] = None):
        ...
        try:
            revenue_data = self.customer_service_repo.get_revenue_by_service_plan(
                service_plan_id
            )

            logger.info(
                f"Retrieved revenue analytics for {'all plans' if not service_plan_id else f'plan {service_plan_id}'}"
            )
            return revenue_data
    ...
    def bulk_status_update(
        self, service_ids: List[int], new_status: str, admin_id: int
    ):
        ...
        try:
            ...
            logger.info(
                f"Bulk status update completed: {len(updated_services)} services updated to {new_status}"
            )
