...
from ..repositories.radius import (
    RadiusSessionRepository, CustomerOnlineRepository, CustomerStatisticsRepository
)
...
class RadiusSessionService:
    ...
    def update_session_usage(self, session_id: str, in_bytes: int, out_bytes: int, 
        ...
        session = self.repo.update_session_usage(session_id, in_bytes, out_bytes, time_on)
        
        if session:
            # Update online customer data
            self.online_repo.update_session_data(session_id, in_bytes, out_bytes, time_on)
    ...
    def get_session_by_session_id(self, session_id: str) -> RadiusSession:
        ...
        if not session:
            raise NotFoundError(f"RADIUS session with session ID {session_id} not found")
        return session
    
    def get_customer_sessions(self, customer_id: int, days: int = 30) -> List[RadiusSession]:
        """Get customer sessions for the last N days"""
        end_date = datetime.now(timezone.utc)
        start_date = end_date - timedelta(days=days)
        return self.repo.get_customer_sessions_by_date(customer_id, start_date, end_date)
...
class CustomerOnlineService:
    ...
    def get_online_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_session_time_hours": round(total_time / 3600, 2) if total_time else 0,
            "average_session_duration_minutes": round(total_time / len(online_customers) / 60, 2) if online_customers and total_time else 0
        }
...
class CustomerStatisticsService:
    ...
    def get_customer_statistics(self, customer_id: int, days: int = 30) -> List[CustomerStatistics]:
        """Get customer statistics for the last N days"""
        end_date = datetime.now(timezone.utc)
        start_date = end_date - timedelta(days=days)
        
        return self.db.query(CustomerStatistics).filter(
    ...
    def get_top_users_by_usage(self, days: int = 30, limit: int = 10) -> List[CustomerStatistics]:
    ...
    def generate_daily_statistics(self, date: datetime) -> int:
        ...
        sessions = self.session_repo.get_sessions_by_date_range(start_of_day, end_of_day)
        ...
        for session in sessions:
            if session.customer_id:
                ...
                customer_data[session.customer_id]["out_bytes"] += session.out_bytes or 0
        ...
        logger.info(f"Generated daily statistics for {created_count} customers on {date.date()}")
    ...
    def generate_monthly_statistics(self, year: int, month: int) -> int:
        ...
        daily_stats = self.db.query(CustomerStatistics).filter(
        ...
        logger.info(f"Generated monthly statistics for {created_count} customers for {year}-{month:02d}")
        return created_count
    ...
    def get_network_utilization(self, days: int = 30) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_daily_usage_gb": round(total_data / days / (1024**3), 2) if total_data else 0
        }
