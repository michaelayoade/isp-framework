...
class BillingTypeService:
    ...
    def get_billing_type(self, billing_type_id: int) -> BillingType:
        ...
        billing_type = (
            self.db.query(BillingType).filter(BillingType.id == billing_type_id).first()
        )
        if not billing_type:
    ...
    def create_billing_type(self, billing_type_data: BillingTypeCreate) -> BillingType:
        ...
        if existing:
            raise DuplicateError(
                f"Billing type with code '{billing_type_data.code}' already exists"
            )

        # Create billing type
        billing_type_dict = billing_type_data.model_dump()
        billing_type_dict["is_system"] = (
            False  # User-created billing types are not system types
        )
    ...
    def update_billing_type(
        self, billing_type_id: int, billing_type_data: BillingTypeUpdate
    ) -> BillingType:
    ...
    def delete_billing_type(self, billing_type_id: int):
        ...
        if billing_type.is_system:
            raise ValidationError(
                f"System billing type '{billing_type.code}' cannot be deleted"
            )
        ...
        try:
            ...
            customers_using_type = (
                self.db.query(Customer)
                .filter(Customer.billing_type_id == billing_type_id)
                .count()
            )

            if customers_using_type > 0:
                raise ValidationError(
                    f"Billing type '{billing_type.code}' is in use by {customers_using_type} customers and cannot be deleted"
                )
        except ImportError:
        ...
        try:
            ...
            plans_using_type = (
                ...
                .filter(ServicePlan.billing_type_id == billing_type_id)
                .count()
            )

            if plans_using_type > 0:
                raise ValidationError(
                    f"Billing type '{billing_type.code}' is in use by {plans_using_type} service plans and cannot be deleted"
                )
        except ImportError:
