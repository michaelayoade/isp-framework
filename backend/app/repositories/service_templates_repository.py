...
class ServiceTemplateRepository(BaseRepository[ServiceTemplate]):
    ...
    def get_by_code(self, code: str) -> Optional[ServiceTemplate]:
        ...
        return (
            self.db.query(ServiceTemplate).filter(ServiceTemplate.code == code).first()
        )
    ...
    def get_active_templates(
        ...
        query = self.db.query(ServiceTemplate).filter(ServiceTemplate.is_active is True)
    ...
    def get_available_templates(
        ...
        query = self.db.query(ServiceTemplate).filter(
            ServiceTemplate.is_active is True, ServiceTemplate.is_public is True
        )
    ...
    def search_templates(
        ...
        templates = (
            query.order_by(ServiceTemplate.display_order, ServiceTemplate.name)
            .offset(offset)
            .limit(limit)
            .all()
        )
    ...
    def get_templates_with_tariffs(
        ...
        return (
            ...
            .options(joinedload(ServiceTemplate.tariff))
    ...
    def get_template_statistics(self) -> Dict[str, Any]:
        ...
        stats["active_templates"] = (
            self.db.query(ServiceTemplate)
            .filter(ServiceTemplate.is_active is True)
            .count()
        )
        ...
        type_stats = (
            ...
            .group_by(ServiceTemplate.service_type)
            .all()
        )
        ...
        category_stats = (
            self.db.query(
                ServiceTemplate.category, func.count(ServiceTemplate.id).label("count")
            )
            .group_by(ServiceTemplate.category)
            .all()
        )

        stats["by_category"] = {
            str(cat_stat[0]): cat_stat[1] for cat_stat in category_stats
        }

        # Public vs private
        stats["public_templates"] = (
            self.db.query(ServiceTemplate)
            .filter(ServiceTemplate.is_public is True)
            .count()
        )

        stats["private_templates"] = (
            self.db.query(ServiceTemplate)
            .filter(ServiceTemplate.is_public is False)
            .count()
        )
...
class InternetServiceTemplateRepository(BaseRepository[InternetServiceTemplate]):
    ...
    def get_by_speed_range(
        ...
        query = (
            self.db.query(InternetServiceTemplate)
            .join(ServiceTemplate)
            .filter(ServiceTemplate.is_active is True)
        )
...
class VoiceServiceTemplateRepository(BaseRepository[VoiceServiceTemplate]):
    ...
    def get_by_minutes_range(
        ...
        query = (
            self.db.query(VoiceServiceTemplate)
            .join(ServiceTemplate)
            .filter(ServiceTemplate.is_active is True)
        )
...
class BundleServiceTemplateRepository(BaseRepository[BundleServiceTemplate]):
    ...
    def get_with_discounts(
        self, min_discount: Optional[float] = None
    ) -> List[BundleServiceTemplate]:
        ...
        if min_discount:
            query = query.filter(
                BundleServiceTemplate.discount_percentage >= min_discount
            )
    ...
    def get_bundle_statistics(self) -> Dict[str, Any]:
        ...
        stats["with_contracts"] = (
            self.db.query(BundleServiceTemplate)
            .filter(BundleServiceTemplate.contract_months > 0)
            .count()
        )
