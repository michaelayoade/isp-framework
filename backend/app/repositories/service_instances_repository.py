...
class CustomerServiceRepository(BaseRepository[CustomerService]):
    ...
    def get_customer_services(
        ...
        return (
            ...
            .order_by(CustomerService.created_at.desc())
            .all()
        )
    ...
    def get_services_by_template(self, template_id: int) -> List[CustomerService]:
        ...
        return (
            ...
            .filter(CustomerService.template_id == template_id)
            .options(joinedload(CustomerService.customer))
            .order_by(CustomerService.created_at.desc())
            .all()
        )
    ...
    def get_expiring_services(self, days_ahead: int = 30) -> List[CustomerService]:
        ...
        return (
            ...
            .order_by(CustomerService.expires_at)
            .all()
        )
    ...
    def get_service_statistics(self) -> Dict[str, Any]:
        ...
        status_stats = (
            self.db.query(
                CustomerService.status, func.count(CustomerService.id).label("count")
            )
            .group_by(CustomerService.status)
            .all()
        )
        ...
        type_stats = (
            ...
            .group_by(CustomerService.service_type)
            .all()
        )
        ...
        stats["active_services"] = (
            self.db.query(CustomerService)
            .filter(CustomerService.status == ServiceStatus.ACTIVE)
            .count()
        )
        ...
        stats["activated_this_month"] = (
            self.db.query(CustomerService)
            .filter(CustomerService.activated_at >= month_start)
            .count()
        )
        ...
        avg_duration = (
            self.db.query(
                func.avg(
                    func.extract(
                        "epoch",
                        CustomerService.terminated_at - CustomerService.activated_at,
                    )
                    / 86400
                )
            )
            .filter(CustomerService.terminated_at.isnot(None))
            .scalar()
        )

        stats["average_service_duration_days"] = (
...
class CustomerInternetServiceRepository(BaseRepository[CustomerInternetService]):
    ...
    def get_by_pppoe_username(self, username: str) -> Optional[CustomerInternetService]:
        ...
        return (
            self.db.query(CustomerInternetService)
            .filter(CustomerInternetService.pppoe_username == username)
            .first()
        )
    ...
    def get_by_ip_address(self, ip_address: str) -> Optional[CustomerInternetService]:
        ...
        return (
            self.db.query(CustomerInternetService)
            .filter(CustomerInternetService.assigned_ip == ip_address)
            .first()
        )
    ...
    def get_services_with_fup_exceeded(self) -> List[CustomerInternetService]:
        ...
        return (
            self.db.query(CustomerInternetService)
            .join(CustomerService)
            .filter(
                CustomerService.status == ServiceStatus.ACTIVE,
                CustomerInternetService.fup_exceeded is True,
            )
            .options(
                joinedload(CustomerInternetService.customer_service).joinedload(
    ...
    def get_services_by_speed_profile(
        self, min_download: Optional[int] = None, max_download: Optional[int] = None
        ...
        if min_download:
            query = query.filter(
                CustomerInternetService.current_download_speed >= min_download
            )

        if max_download:
            query = query.filter(
                CustomerInternetService.current_download_speed <= max_download
            )
    ...
    def get_usage_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        stats["total_internet_services"] = self.db.query(
            CustomerInternetService
        ).count()

        # Services with FUP exceeded
        stats["fup_exceeded_services"] = (
            self.db.query(CustomerInternetService)
            .filter(CustomerInternetService.fup_exceeded is True)
            .count()
        )
        ...
        connection_stats = (
            ...
            .group_by(CustomerInternetService.connection_type)
            .all()
        )

        stats["by_connection_type"] = {
            str(conn[0]): conn[1] for conn in connection_stats
        }
...
class CustomerVoiceServiceRepository(BaseRepository[CustomerVoiceService]):
    ...
    def get_by_phone_number(self, phone_number: str) -> Optional[CustomerVoiceService]:
        ...
        return (
            self.db.query(CustomerVoiceService)
            .filter(CustomerVoiceService.phone_numbers.contains([phone_number]))
            .first()
        )
    ...
    def get_by_sip_username(self, sip_username: str) -> Optional[CustomerVoiceService]:
        ...
        return (
            self.db.query(CustomerVoiceService)
            .filter(CustomerVoiceService.sip_username == sip_username)
            .first()
        )

    def get_services_with_low_balance(
        self, threshold: float = 5.0
    ) -> List[CustomerVoiceService]:
        ...
        return (
            ...
            .options(
                joinedload(CustomerVoiceService.customer_service).joinedload(
                    CustomerService.customer
                )
            )
            .all()
    ...
    def get_services_with_exceeded_minutes(self) -> List[CustomerVoiceService]:
        ...
        return (
            ...
            .filter(
                CustomerService.status == ServiceStatus.ACTIVE,
                CustomerVoiceService.used_minutes
                > CustomerVoiceService.included_minutes,
            )
            .options(
                joinedload(CustomerVoiceService.customer_service).joinedload(
                    CustomerService.customer
    ...
    def get_call_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        stats["average_minutes_per_service"] = (
            float(avg_minutes) if avg_minutes else 0.0
        )

        # Services exceeding included minutes
        stats["services_exceeding_minutes"] = (
            self.db.query(CustomerVoiceService)
            .filter(
                CustomerVoiceService.used_minutes
                > CustomerVoiceService.included_minutes
...
class ServiceInstanceUtils:
    ...
    def calculate_prorated_amount(
        monthly_amount: float, start_date: datetime, end_date: datetime
    ) -> float:
