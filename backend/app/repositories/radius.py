...
class RadiusSessionRepository(BaseRepository[RadiusSession]):
    ...
    def get_customer_sessions_by_date(
    ...
    def get_session_analytics(
        ...
        return {
            ...
            "average_session_duration_minutes": (
                round(total_time / total_sessions / 60, 2)
                if total_sessions and total_time
                else 0
            ),
        }
...
class CustomerOnlineRepository(BaseRepository[CustomerOnline]):
    ...
    def get_by_ip(self, ip_address: str) -> Optional[CustomerOnline]:
        ...
        return (
            self.db.query(self.model)
            .filter(or_(self.model.ipv4 == ip_address, self.model.ipv6 == ip_address))
            .first()
        )
...
class CustomerStatisticsRepository(BaseRepository[CustomerStatistics]):
    ...
    def get_by_customer_and_period(
        self,
        customer_id: int,
        period_type: str,
        period_start: datetime,
        period_end: datetime,
    ) -> Optional[CustomerStatistics]:
    ...
    def get_top_users_by_usage(
        self, period_start: datetime, period_end: datetime, limit: int = 10
    ) -> List[CustomerStatistics]:
        ...
        return (
            ...
            .order_by(desc(self.model.in_bytes + self.model.out_bytes))
            .limit(limit)
            .all()
        )
    ...
    def get_customer_usage_summary(
        ...
        return {
            ...
            "average_daily_usage_gb": (
                round(total_bytes / days / (1024**3), 2) if total_bytes else 0
            ),
        }
    ...
    def create_or_update_stats(
        ...
        existing = self.get_by_customer_and_period(
            customer_id, period_type, period_start, period_end
        )
