...
class ServiceRepositoryFactory:
    ...
    def get_internet_template_repo(self) -> InternetServiceTemplateRepository:
        ...
        if 'internet_template' not in self._repositories:
            self._repositories['internet_template'] = InternetServiceTemplateRepository(self.db)
    ...
    def get_voice_template_repo(self) -> VoiceServiceTemplateRepository:
        ...
        if 'voice_template' not in self._repositories:
            self._repositories['voice_template'] = VoiceServiceTemplateRepository(self.db)
    ...
    def get_bundle_template_repo(self) -> BundleServiceTemplateRepository:
        ...
        if 'bundle_template' not in self._repositories:
            self._repositories['bundle_template'] = BundleServiceTemplateRepository(self.db)
    ...
    def get_internet_service_repo(self) -> CustomerInternetServiceRepository:
        ...
        if 'internet_service' not in self._repositories:
            self._repositories['internet_service'] = CustomerInternetServiceRepository(self.db)
    ...
    def get_voice_service_repo(self) -> CustomerVoiceServiceRepository:
        ...
        if 'voice_service' not in self._repositories:
            self._repositories['voice_service'] = CustomerVoiceServiceRepository(self.db)
    ...
    def get_provisioning_template_repo(self) -> ProvisioningTemplateRepository:
        ...
        if 'provisioning_template' not in self._repositories:
            self._repositories['provisioning_template'] = ProvisioningTemplateRepository(self.db)
    ...
    def get_provisioning_queue_repo(self) -> ProvisioningQueueRepository:
        ...
        if 'provisioning_queue' not in self._repositories:
            self._repositories['provisioning_queue'] = ProvisioningQueueRepository(self.db)
    ...
    def get_status_history_repo(self) -> ServiceStatusHistoryRepository:
        ...
        if 'status_history' not in self._repositories:
            self._repositories['status_history'] = ServiceStatusHistoryRepository(self.db)
    ...
    def get_usage_tracking_repo(self) -> ServiceUsageTrackingRepository:
        ...
        if 'usage_tracking' not in self._repositories:
            self._repositories['usage_tracking'] = ServiceUsageTrackingRepository(self.db)
    ...
    def get_repository_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'template_repos': len([k for k in self._repositories.keys() if 'template' in k]),
            'instance_repos': len([k for k in self._repositories.keys() if 'service' in k and 'template' not in k]),
            'provisioning_repos': len([k for k in self._repositories.keys() if 'provisioning' in k]),
            'management_repos': len([k for k in self._repositories.keys() if k in ['ip_assignment', 'status_history', 'suspension', 'usage_tracking', 'alert']])
        }
        ...
        for repo_name, repo_class in repository_types:
            try:
                ...
                if not hasattr(repo_instance, 'model'):
                    validation_results['warnings'].append(f"Repository {repo_name} missing model attribute")
            except Exception as e:
                validation_results['is_valid'] = False
                validation_results['errors'].append(f"Failed to instantiate {repo_name}: {str(e)}")
...
def get_service_repository_factory(db: Session, cache_key: Optional[str] = None) -> ServiceRepositoryFactory:
    """
