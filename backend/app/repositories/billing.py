...
class InvoiceRepository(BaseRepository[Invoice]):
    ...
    def get_customer_invoices(self, customer_id: int, limit: int = 100, offset: int = 0) -> List[Invoice]:
    ...
    def get_invoices_by_status(self, status: InvoiceStatus, limit: int = 100, offset: int = 0) -> List[Invoice]:
    ...
    def get_invoices_by_date_range(
        ...
        ).order_by(desc(self.model.invoice_date)).limit(limit).offset(offset).all()
    ...
    def search_invoices(self, search_params: Dict[str, Any]) -> List[Invoice]:
        ...
        if search_params.get("invoice_number"):
            query = query.filter(self.model.invoice_number.ilike(f"%{search_params['invoice_number']}%"))
    ...
    def get_billing_statistics(self, start_date: Optional[date] = None, end_date: Optional[date] = None) -> Dict[str, Any]:
        ...
        total_amount = query.with_entities(func.sum(self.model.total_amount)).scalar() or Decimal('0')
        paid_amount = query.with_entities(func.sum(self.model.paid_amount)).scalar() or Decimal('0')
        outstanding_amount = query.with_entities(func.sum(self.model.balance_due)).scalar() or Decimal('0')
...
class PaymentRepository(BaseRepository[Payment]):
    ...
    def get_by_payment_number(self, payment_number: str) -> Optional[Payment]:
        ...
        return self.db.query(self.model).filter(
            self.model.payment_number == payment_number
        ).first()
    ...
    def get_customer_payments(self, customer_id: int, limit: int = 100, offset: int = 0) -> List[Payment]:
    ...
    def get_payments_by_method(self, payment_method: str, limit: int = 100, offset: int = 0) -> List[Payment]:
    ...
    def search_payments(self, search_params: Dict[str, Any]) -> List[Payment]:
        ...
        if search_params.get("payment_method"):
            query = query.filter(self.model.payment_method == search_params["payment_method"])
        
        if search_params.get("payment_number"):
            query = query.filter(self.model.payment_number.ilike(f"%{search_params['payment_number']}%"))
...
class PaymentRefundRepository(BaseRepository[PaymentRefund]):
    ...
    def get_by_refund_number(self, refund_number: str) -> Optional[PaymentRefund]:
        ...
        return self.db.query(self.model).filter(
            self.model.refund_number == refund_number
        ).first()
...
class CreditNoteRepository(BaseRepository[CreditNote]):
    ...
    def get_by_credit_note_number(self, credit_note_number: str) -> Optional[CreditNote]:
        """Get credit note by number"""
        return self.db.query(self.model).filter(
            self.model.credit_note_number == credit_note_number
        ).first()
    ...
    def get_customer_credit_notes(self, customer_id: int, limit: int = 100, offset: int = 0) -> List[CreditNote]:
