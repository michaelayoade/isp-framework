...
class CustomerServiceRepository(BaseRepository[CustomerService]):
    ...
    def get_customer_services(
        self, customer_id: int, active_only: bool = True, include_details: bool = False
    ...
    def search_customer_services(
        ...
        if search_params.get("customer_id"):
            query = query.filter(
                CustomerService.customer_id == search_params["customer_id"]
            )

        if search_params.get("service_plan_id"):
            query = query.filter(
                CustomerService.service_template_id == search_params["service_plan_id"]
            )
        ...
        services = (
            query.order_by(CustomerService.created_at.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )
    ...
    def get_customer_active_service(
        self, customer_id: int, service_template_id: int
    ...
    def get_customer_services_overview(self, customer_id: int) -> Dict[str, Any]:
        ...
        aggregates = (
            ...
            .with_entities(
                ...
                func.sum(
                    func.case(
                        [(CustomerService.status == ServiceStatus.ACTIVE.value, 1)],
                        else_=0,
                    )
                ).label("active"),
                func.sum(
                    func.case(
                        [(CustomerService.status == ServiceStatus.SUSPENDED.value, 1)],
                        else_=0,
                    )
                ).label("suspended"),
                func.sum(
                    func.case(
                        [(CustomerService.status == ServiceStatus.EXPIRED.value, 1)],
                        else_=0,
                    )
                ).label("expired"),
                func.sum(
        ...
        services = self.get_customer_services(
            customer_id, active_only=False, include_details=True
        )
    ...
    def get_service_template_assignments_overview(
        ...
        aggregate = (
            ...
            .filter(CustomerService.service_template_id == service_template_id)
            .first()
        )
        ...
        logger.info(
            "Service template %s assignments overview calculated", service_template_id
        )
        return {
    ...
    def terminate_service(
        self, service_id: int, end_date: Optional[datetime] = None
    ) -> CustomerService:
    ...
    def commit_batch(
        self, objects: List[CustomerService], batch_size: int = 1000
    ) -> int:
        ...
        try:
            ...
            logger.info(
                "Committed %s CustomerService objects in batches of %s",
                total,
                batch_size,
            )
            return total
    ...
    def get_revenue_by_service_plan(
        self, service_plan_id: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        ...
        query = (
            self.db.query(
                ...
                func.count(
                    func.case(
                        [(CustomerService.status == "active", CustomerService.id)]
                    )
