...
class ServiceProvisioningRepository(BaseRepository[ServiceProvisioning]):
    ...
    def get_by_service(self, service_id: int) -> List[ServiceProvisioning]:
        ...
        return self.db.query(ServiceProvisioning).filter(
            ServiceProvisioning.service_id == service_id
        ).options(
            joinedload(ServiceProvisioning.template),
            joinedload(ServiceProvisioning.initiated_by)
        ).order_by(ServiceProvisioning.created_at.desc()).all()
    
    def get_active_provisioning(self, service_id: Optional[int] = None) -> List[ServiceProvisioning]:
        ...
        return query.options(
            ...
            joinedload(ServiceProvisioning.template)
        ).order_by(ServiceProvisioning.priority.desc(), ServiceProvisioning.created_at).all()
    ...
    def get_failed_provisioning(
        ...
        return query.options(
            joinedload(ServiceProvisioning.service).joinedload(CustomerService.customer),
            joinedload(ServiceProvisioning.template)
        ).order_by(ServiceProvisioning.updated_at.desc()).all()
    ...
    def get_overdue_provisioning(self) -> List[ServiceProvisioning]:
        ...
        return self.db.query(ServiceProvisioning).filter(
            ...
            joinedload(ServiceProvisioning.service).joinedload(CustomerService.customer),
            joinedload(ServiceProvisioning.template)
        ).order_by(ServiceProvisioning.scheduled_at).all()
    ...
    def search_provisioning(
        ...
        provisioning = query.options(
            ...
            ServiceProvisioning.created_at.desc()
        ).offset(offset).limit(limit).all()
    ...
    def get_provisioning_statistics(self) -> Dict[str, Any]:
        ...
        ).group_by(ServiceProvisioning.status).all()
        
        stats['by_status'] = {str(status[0]): status[1] for status in status_stats}
        
        # Success rate (completed vs failed)
        completed = self.db.query(ServiceProvisioning).filter(
            ServiceProvisioning.status == ProvisioningStatus.COMPLETED
        ).count()
        
        failed = self.db.query(ServiceProvisioning).filter(
            ServiceProvisioning.status == ProvisioningStatus.FAILED
        ).count()
        ...
        avg_duration = self.db.query(
            func.avg(
                func.extract('epoch', ServiceProvisioning.completed_at - ServiceProvisioning.started_at) / 60
            )
        ).filter(
            ServiceProvisioning.status == ProvisioningStatus.COMPLETED,
        ...
        stats['average_provisioning_time_minutes'] = float(avg_duration) if avg_duration else 0.0
        
        # Active workflows
        stats['active_workflows'] = self.db.query(ServiceProvisioning).filter(
            ServiceProvisioning.status.in_([
                ProvisioningStatus.PENDING,
                ProvisioningStatus.IN_PROGRESS
            ])
        ...
        stats['requiring_attention'] = self.db.query(ServiceProvisioning).filter(
            ServiceProvisioning.status.in_([
...
class ProvisioningTemplateRepository(BaseRepository[ProvisioningTemplate]):
    ...
    def get_by_service_type(self, service_type: ServiceType) -> List[ProvisioningTemplate]:
    ...
    def get_template_usage_stats(self, template_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        # Total usage
        stats['total_usage'] = self.db.query(ServiceProvisioning).filter(
...
class ProvisioningQueueRepository(BaseRepository[ProvisioningQueue]):
    ...
    def get_pending_tasks(
        ...
        return query.options(
            joinedload(ProvisioningQueue.provisioning)
        ).order_by(
            ProvisioningQueue.priority.desc(),
            ProvisioningQueue.created_at
        ).all()
    
    def get_tasks_by_provisioning(self, provisioning_id: int) -> List[ProvisioningQueue]:
        """Get all queue tasks for a specific provisioning workflow"""
        return self.db.query(ProvisioningQueue).filter(
            ProvisioningQueue.provisioning_id == provisioning_id
        ).order_by(ProvisioningQueue.step_order).all()
    ...
    def get_failed_tasks(self, hours_back: int = 24) -> List[ProvisioningQueue]:
        ...
        return self.db.query(ProvisioningQueue).filter(
            ...
            joinedload(ProvisioningQueue.provisioning)
        ).order_by(ProvisioningQueue.updated_at.desc()).all()
    ...
    def get_queue_statistics(self) -> Dict[str, Any]:
        ...
        ).group_by(ProvisioningQueue.status).all()
        
        stats['by_status'] = {status[0]: status[1] for status in status_stats}
        
        # Pending tasks
        stats['pending_tasks'] = self.db.query(ProvisioningQueue).filter(
            ProvisioningQueue.status == 'pending'
        ).count()
        ...
        avg_time = self.db.query(
            func.avg(
                func.extract('epoch', ProvisioningQueue.completed_at - ProvisioningQueue.started_at) / 60
            )
        ).filter(
...
class ProvisioningUtils:
    ...
    def get_next_step_order(provisioning_id: int, db: Session) -> int:
        ...
        max_order = db.query(
            func.max(ProvisioningQueue.step_order)
        ).filter(
            ProvisioningQueue.provisioning_id == provisioning_id
        ).scalar()
