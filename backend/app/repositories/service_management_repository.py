...
class ServiceIPAssignmentRepository(BaseRepository[ServiceIPAssignment]):
    ...
    def get_by_ip_address(self, ip_address: str) -> Optional[ServiceIPAssignment]:
        ...
        return (
            self.db.query(ServiceIPAssignment)
            .filter(ServiceIPAssignment.ip_address == ip_address)
            .first()
        )
    ...
    def get_by_service(self, service_id: int) -> List[ServiceIPAssignment]:
        ...
        return (
            self.db.query(ServiceIPAssignment)
            .filter(ServiceIPAssignment.service_id == service_id)
            .options(
                joinedload(ServiceIPAssignment.service).joinedload(
    ...
    def get_by_mac_address(self, mac_address: str) -> List[ServiceIPAssignment]:
        ...
        return (
            self.db.query(ServiceIPAssignment)
            .filter(ServiceIPAssignment.mac_address == mac_address)
            .options(
                joinedload(ServiceIPAssignment.service).joinedload(
                    CustomerService.customer
                )
    ...
    def get_expiring_assignments(
        ...
        return (
            ...
            .order_by(ServiceIPAssignment.expires_at)
            .all()
        )
    ...
    def get_network_utilization(self, network_id: int) -> Dict[str, Any]:
        ...
        total_assignments = (
            self.db.query(ServiceIPAssignment)
            .filter(ServiceIPAssignment.network_id == network_id)
            .count()
        )
        ...
        type_stats = (
            ...
            .group_by(ServiceIPAssignment.assignment_type)
            .all()
        )
        ...
        stats.update(
            {
                ...
                "by_type": {
                    str(type_stat[0]): type_stat[1] for type_stat in type_stats
                },
            }
        )
...
class ServiceStatusHistoryRepository(BaseRepository[ServiceStatusHistory]):
    ...
    def get_service_history(
        self, service_id: int, limit: int = 50
        ...
        return (
            ...
            .options(joinedload(ServiceStatusHistory.changed_by))
    ...
    def get_recent_changes(
        self, hours_back: int = 24, status_filter: Optional[ServiceStatus] = None
    ) -> List[ServiceStatusHistory]:
        ...
        return (
            query.options(
                joinedload(ServiceStatusHistory.service).joinedload(
                    CustomerService.customer
                ),
    ...
    def get_status_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        stats["total_changes"] = (
            self.db.query(ServiceStatusHistory)
            .filter(ServiceStatusHistory.changed_at >= cutoff_date)
            .count()
        )
        ...
        status_stats = (
            ...
            .filter(ServiceStatusHistory.changed_at >= cutoff_date)
            .group_by(ServiceStatusHistory.new_status)
            .all()
        )
...
class ServiceSuspensionRepository(BaseRepository[ServiceSuspension]):
    ...
    def get_active_suspensions(
        ...
        return (
            query.options(
                joinedload(ServiceSuspension.service).joinedload(
                    CustomerService.customer
                ),
                joinedload(ServiceSuspension.suspended_by),
            )
            .order_by(ServiceSuspension.suspended_at.desc())
            .all()
        )
    ...
    def get_suspensions_in_grace_period(self) -> List[ServiceSuspension]:
        ...
        return (
            self.db.query(ServiceSuspension)
            .filter(
                ServiceSuspension.is_active is True,
            ...
            .order_by(ServiceSuspension.grace_period_until)
            .all()
        )
    ...
    def get_suspensions_ready_for_escalation(self) -> List[ServiceSuspension]:
        ...
        return (
            ...
            .order_by(ServiceSuspension.escalation_at)
            .all()
        )
    ...
    def get_auto_restoration_candidates(self) -> List[ServiceSuspension]:
        ...
        return (
            ...
            .order_by(ServiceSuspension.auto_restore_at)
            .all()
        )

    def get_suspension_history(
        self, service_id: int, limit: int = 20
    ) -> List[ServiceSuspension]:
        """Get suspension history for a service"""
        return (
            self.db.query(ServiceSuspension)
            .filter(ServiceSuspension.service_id == service_id)
            .options(
                joinedload(ServiceSuspension.suspended_by),
    ...
    def get_suspension_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        stats["total_suspensions"] = (
            self.db.query(ServiceSuspension)
            .filter(ServiceSuspension.suspended_at >= cutoff_date)
            .count()
        )
        ...
        stats["active_suspensions"] = (
            self.db.query(ServiceSuspension)
            .filter(ServiceSuspension.is_active is True)
            .count()
        )
        ...
        reason_stats = (
            ...
            .filter(ServiceSuspension.suspended_at >= cutoff_date)
            .group_by(ServiceSuspension.reason)
            .all()
        )
        ...
        avg_duration = (
            self.db.query(
                func.avg(
                    func.extract(
                        "epoch",
                        ServiceSuspension.restored_at - ServiceSuspension.suspended_at,
                    )
                    / 86400
                )
            )
        ...
        stats["average_suspension_duration_days"] = (
...
class ServiceUsageTrackingRepository(BaseRepository[ServiceUsageTracking]):
    ...
    def get_high_usage_services(
        self, threshold_gb: float = 100.0, days_back: int = 7
    ) -> List[ServiceUsageTracking]:
        ...
        return (
            self.db.query(ServiceUsageTracking)
            .filter(
                ServiceUsageTracking.period_start >= cutoff_date,
                ServiceUsageTracking.total_bytes > threshold_bytes,
            )
            .options(
                joinedload(ServiceUsageTracking.service).joinedload(
                    CustomerService.customer
                )
            )
            .order_by(ServiceUsageTracking.total_bytes.desc())
            .all()
        )

    def get_usage_summary(self, service_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "total_sessions": sum(
                record.session_count or 0 for record in usage_records
            ),
            "peak_bandwidth_mbps": max(
                record.peak_bandwidth_mbps or 0 for record in usage_records
            ),
        ...
        total_duration = sum(
            record.total_session_duration or 0 for record in usage_records
        )
    ...
    def get_network_usage_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        total_bytes = (
            self.db.query(func.sum(ServiceUsageTracking.total_bytes))
            .filter(ServiceUsageTracking.period_start >= cutoff_date)
            .scalar()
        )
        ...
        # Total sessions
        total_sessions = (
            self.db.query(func.sum(ServiceUsageTracking.session_count))
            .filter(ServiceUsageTracking.period_start >= cutoff_date)
            .scalar()
        )
        ...
        # Peak bandwidth
        peak_bandwidth = (
            self.db.query(func.max(ServiceUsageTracking.peak_bandwidth_mbps))
            .filter(ServiceUsageTracking.period_start >= cutoff_date)
            .scalar()
        )
        ...
        # Total cost
        total_cost = (
            self.db.query(func.sum(ServiceUsageTracking.cost_amount))
            .filter(ServiceUsageTracking.period_start >= cutoff_date)
            .scalar()
        )
...
class ServiceAlertRepository(BaseRepository[ServiceAlert]):
    ...
    def get_active_alerts(
        self, service_id: Optional[int] = None, severity: Optional[str] = None
    ) -> List[ServiceAlert]:
        """Get all active alerts with optional filtering"""
        query = self.db.query(ServiceAlert).filter(ServiceAlert.is_active is True)
    ...
    def get_unacknowledged_alerts(self) -> List[ServiceAlert]:
        ...
        return (
            self.db.query(ServiceAlert)
            .filter(
                ServiceAlert.is_active is True, ServiceAlert.acknowledged_at.is_(None)
            )
            .options(
                joinedload(ServiceAlert.service).joinedload(CustomerService.customer)
            )
    ...
    def get_alert_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        stats["total_alerts"] = (
            self.db.query(ServiceAlert)
            .filter(ServiceAlert.created_at >= cutoff_date)
            .count()
        )

        # Active alerts
        stats["active_alerts"] = (
            self.db.query(ServiceAlert).filter(ServiceAlert.is_active is True).count()
        )

        # Alerts by severity
        severity_stats = (
            self.db.query(
                ServiceAlert.severity, func.count(ServiceAlert.id).label("count")
            )
            .filter(ServiceAlert.created_at >= cutoff_date)
            .group_by(ServiceAlert.severity)
            .all()
        )
        ...
        avg_resolution = (
            self.db.query(
                func.avg(
                    func.extract(
                        "epoch", ServiceAlert.resolved_at - ServiceAlert.created_at
                    )
                    / 3600
                )
            )
        ...
        stats["average_resolution_time_hours"] = (
            float(avg_resolution) if avg_resolution else 0.0
        )
