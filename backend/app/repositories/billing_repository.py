...
class BillingAccountRepository(BaseRepository[CustomerBillingAccount]):
    ...
    def get_by_account_number(self, account_number: str) -> Optional[CustomerBillingAccount]:
        """Get billing account by account number"""
        return self.db.query(self.model).filter(
            self.model.account_number == account_number
        ).first()
    
    def get_accounts_by_status(self, status: AccountStatus) -> List[CustomerBillingAccount]:
        """Get all accounts with specific status"""
        return self.db.query(self.model).filter(
            self.model.account_status == status
        ).all()
    
    def get_accounts_by_billing_type(self, billing_type: BillingType) -> List[CustomerBillingAccount]:
        """Get all accounts with specific billing type"""
        return self.db.query(self.model).filter(
            self.model.billing_type == billing_type
        ).all()
    
    def get_low_balance_accounts(self, threshold: Decimal) -> List[CustomerBillingAccount]:
        """Get accounts with balance below threshold"""
        return self.db.query(self.model).filter(
            self.model.available_balance < threshold
        ).all()
...
class BillingTransactionRepository(BaseRepository[BillingTransaction]):
    ...
    def get_by_account_id(
        ...
        ).order_by(desc(self.model.effective_date)).offset(offset).limit(limit).all()
    ...
    def get_by_transaction_type(
        ...
        ).order_by(desc(self.model.effective_date)).all()
    ...
    def get_transaction_summary(self, account_id: int) -> Dict[str, Any]:
        ...
        result = self.db.query(
            ...
            func.sum(
                func.case(
                    (self.model.transaction_type.in_([TransactionType.CREDIT, TransactionType.PAYMENT]), self.model.amount),
                    else_=0
                )
            ).label('total_credits'),
            func.sum(
                func.case(
                    (self.model.transaction_type.in_([TransactionType.DEBIT, TransactionType.CHARGE]), self.model.amount),
                    else_=0
                )
            ).label('total_debits')
        ).filter(
...
class InvoiceRepository(BaseRepository[Invoice]):
    ...
    def get_by_account_id(
        ...
        return self.db.query(self.model).filter(
            self.model.billing_account_id == account_id
        ).order_by(desc(self.model.invoice_date)).offset(offset).limit(limit).all()
    ...
    def get_overdue_invoices(self, as_of_date: Optional[datetime] = None) -> List[Invoice]:
        ...
        return self.db.query(self.model).filter(
            and_(
                ...
                self.model.invoice_status.in_([InvoiceStatus.PENDING, InvoiceStatus.SENT])
            )
    ...
    def get_invoice_with_items(self, invoice_id: int) -> Optional[Invoice]:
        ...
        return self.db.query(self.model).options(
            joinedload(self.model.invoice_items)
        ).filter(self.model.id == invoice_id).first()
    ...
    def search_invoices(
        ...
        total = query.count()
        invoices = query.order_by(desc(self.model.invoice_date)).offset(offset).limit(limit).all()
...
class PaymentRepository(BaseRepository[Payment]):
    ...
    def get_by_account_id(
        ...
        return self.db.query(self.model).filter(
            self.model.billing_account_id == account_id
        ).order_by(desc(self.model.payment_date)).offset(offset).limit(limit).all()
    ...
    def get_by_invoice_id(self, invoice_id: int) -> List[Payment]:
        ...
        ).order_by(desc(self.model.payment_date)).all()
    ...
    def get_payment_summary(self, account_id: int) -> Dict[str, Any]:
        ...
        result = self.db.query(
            ...
            func.sum(
                func.case(
                    (self.model.payment_status == PaymentStatus.COMPLETED, self.model.amount),
                    else_=0
                )
...
class CreditNoteRepository(BaseRepository[CreditNote]):
    ...
    def get_by_account_id(
        ...
        return self.db.query(self.model).filter(
            self.model.billing_account_id == account_id
        ).order_by(desc(self.model.credit_date)).offset(offset).limit(limit).all()
    
    def get_by_invoice_id(self, invoice_id: int) -> List[CreditNote]:
        """Get credit notes for a specific invoice"""
        return self.db.query(self.model).filter(
            self.model.original_invoice_id == invoice_id
        ).order_by(desc(self.model.credit_date)).all()
...
class PaymentPlanRepository(BaseRepository[PaymentPlan]):
    ...
    def get_plan_with_installments(self, plan_id: int) -> Optional[PaymentPlan]:
        ...
        return self.db.query(self.model).options(
            joinedload(self.model.installments)
        ).filter(self.model.id == plan_id).first()
