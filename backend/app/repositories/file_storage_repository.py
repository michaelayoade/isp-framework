...
class FileMetadataRepository(BaseRepository[FileMetadata]):
    ...
    def update_file_status(
        self, file_id: int, status: FileStatus, processed_at: Optional[datetime] = None
    ) -> Optional[FileMetadata]:
    ...
    def get_files_by_category(
        ...
        return (
            query.order_by(desc(FileMetadata.created_at))
            .limit(limit)
            .offset(offset)
            .all()
        )
    ...
    def get_file_count(
        self, category: Optional[FileCategory] = None, customer_id: Optional[int] = None
    ) -> int:
...
class ImportJobRepository(BaseRepository[ImportJob]):
    ...
    def update_job_status(
        ...
        if job:
            ...
            if (
                ...
                and not job.processing_completed_at
            ):
    ...
    def get_jobs_by_status(
        ...
        return (
            query.order_by(desc(ImportJob.created_at)).limit(limit).offset(offset).all()
        )
