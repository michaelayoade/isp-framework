...
class APIKeyRepository(BaseRepository[APIKey]):
    ...
    def list_expired_keys(self) -> List[APIKey]:
        ...
        return (
            ...
            .filter(and_(APIKey.is_active is True, APIKey.expires_at < now))
            .all()
        )
    ...
    def get_usage_statistics(self, key_id: int) -> Dict[str, Any]:
        ...
        # Total usage count
        total_usage = (
            self.db.query(APIUsage).filter(APIUsage.api_key_id == key_id).count()
        )
        ...
        usage_24h = (
            ...
            .filter(
                and_(APIUsage.api_key_id == key_id, APIUsage.created_at >= last_24h)
            )
            .count()
        )
        ...
        usage_30d = (
            self.db.query(APIUsage)
            .filter(
                and_(APIUsage.api_key_id == key_id, APIUsage.created_at >= last_30d)
            )
            .count()
        )
...
class APIUsageRepository(BaseRepository[APIUsage]):
    ...
    def get_usage_analytics(
        ...
        min_response_time = (
            query.with_entities(func.min(APIUsage.response_time)).scalar() or 0
        )

        max_response_time = (
            query.with_entities(func.max(APIUsage.response_time)).scalar() or 0
        )
        ...
        # Method usage
        methods = (
            query.with_entities(APIUsage.method, func.count().label("count"))
            .group_by(APIUsage.method)
            .all()
        )
...
class APIRateLimitRepository(BaseRepository[APIRateLimit]):
    ...
    def cleanup_old_windows(self, days_to_keep: int = 7) -> int:
        ...
        deleted_count = (
            self.db.query(APIRateLimit)
            .filter(APIRateLimit.window_end < cutoff_date)
            .delete()
        )
...
class APIVersionRepository(BaseRepository[APIVersion]):
    ...
    def list_active_versions(self) -> List[APIVersion]:
        ...
        return (
            self.db.query(APIVersion)
            .filter(APIVersion.status == "active")
...
class APIEndpointRepository(BaseRepository[APIEndpoint]):
    ...
    def get_by_path_and_method(
        self, api_version_id: int, path: str, method: str
