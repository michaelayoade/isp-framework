...
class ResellerRepository(BaseRepository[Reseller]):
    ...
    def get_resellers_by_territory(self, territory: str) -> List[Reseller]:
        ...
        return (
            self.db.query(Reseller)
            .filter(and_(Reseller.territory == territory, Reseller.is_active is True))
            .all()
        )

    def get_reseller_with_customer_count(
        self, reseller_id: int
    ) -> Optional[Dict[str, Any]]:
        """Get reseller with customer count"""
        result = (
            self.db.query(Reseller, func.count(Customer.id).label("customer_count"))
            .outerjoin(Customer, Customer.reseller_id == Reseller.id)
            .filter(Reseller.id == reseller_id)
        ...
        if result:
            ...
            return {"reseller": reseller, "customer_count": customer_count or 0}
        return None
    ...
    def get_reseller_customers(
        ...
        return (
            ...
            .filter(Customer.reseller_id == reseller_id)
            .offset(offset)
            .limit(limit)
            .all()
        )
    ...
    def get_reseller_customer_count(self, reseller_id: int) -> int:
        ...
        return (
            self.db.query(Customer).filter(Customer.reseller_id == reseller_id).count()
        )
    ...
    def get_reseller_active_customer_count(self, reseller_id: int) -> int:
        ...
        return (
            self.db.query(Customer)
            .filter(
                and_(Customer.reseller_id == reseller_id, Customer.status == "active")
            )
            .count()
        )

    def get_reseller_commission_data(
        self, reseller_id: int, start_date=None, end_date=None
    ) -> Dict[str, Any]:
        ...
        query = (
            self.db.query(
                ...
                func.count(func.distinct(Payment.customer_id)).label(
                    "unique_customers"
                ),
            )
    ...
    def get_resellers_with_stats(
        ...
        for reseller in resellers:
            ...
            results.append({"reseller": reseller, "stats": stats})
