...
class InternetTariffRepository(BaseRepository):
    ...
    def get_public_tariffs(self, partner_id: int = None) -> List[InternetTariff]:
        ...
        query = self.db.query(InternetTariff).filter(
            and_(InternetTariff.is_active is True, InternetTariff.is_public is True)
        )
    ...
    def search_tariffs(
        ...
        if filters:
            ...
            if filters.get("partner_id"):
                query = query.filter(
                    InternetTariff.partners_ids.contains([filters["partner_id"]])
                )

            # Speed filters
            if filters.get("min_speed_download"):
                query = query.filter(
                    InternetTariff.speed_download >= filters["min_speed_download"]
                )

            if filters.get("max_speed_download"):
                query = query.filter(
                    InternetTariff.speed_download <= filters["max_speed_download"]
                )
            ...
            if filters.get("billing_type"):
                query = query.filter(
                    InternetTariff.billing_types.contains([filters["billing_type"]])
                )
        ...
        tariffs = (
            query.order_by(InternetTariff.speed_download, InternetTariff.price)
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_by_title(self, title: str) -> Optional[InternetTariff]:
        """Get tariff by title"""
        return (
            self.db.query(InternetTariff).filter(InternetTariff.title == title).first()
        )
    ...
    def get_by_service_name(self, service_name: str) -> Optional[InternetTariff]:
        ...
        return (
            ...
            .filter(InternetTariff.service_name == service_name)
            .first()
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        active_tariffs = (
            self.db.query(func.count(InternetTariff.id))
            .filter(InternetTariff.is_active is True)
            .scalar()
        )
        public_tariffs = (
            self.db.query(func.count(InternetTariff.id))
        ...
        for tariff in tariffs_with_billing:
            if tariff.billing_types:
                for billing_type in tariff.billing_types:
                    billing_types_count[billing_type] = (
                        billing_types_count.get(billing_type, 0) + 1
                    )
