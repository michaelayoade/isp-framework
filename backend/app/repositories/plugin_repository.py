...
class PluginRepository(BaseRepository[Plugin]):
    ...
    def get_most_loaded_plugins(self, limit: int = 10) -> List[Plugin]:
        ...
        return self.db.query(Plugin).order_by(Plugin.load_count.desc()).limit(limit).all()
...
class PluginConfigurationRepository(BaseRepository[PluginConfiguration]):
    ...
    def get_by_plugin_and_key(self, plugin_id: int, config_key: str) -> Optional[PluginConfiguration]:
    ...
    def get_configs_by_type(self, plugin_id: int, config_type: str) -> List[PluginConfiguration]:
...
class PluginHookRepository(BaseRepository[PluginHook]):
    ...
    def get_by_hook_type(self, hook_type: str) -> List[PluginHook]:
        """Get hooks by type"""
        return self.db.query(PluginHook).filter(
            PluginHook.hook_type == hook_type
        ).all()
    
    def get_hooks_by_plugin_and_type(self, plugin_id: int, hook_type: str) -> List[PluginHook]:
    ...
    def get_most_executed_hooks(self, limit: int = 10) -> List[PluginHook]:
        ...
        return self.db.query(PluginHook).order_by(
            PluginHook.execution_count.desc()
        ).limit(limit).all()
    
    def get_slowest_hooks(self, limit: int = 10) -> List[PluginHook]:
        """Get hooks with highest average execution time"""
        return self.db.query(PluginHook).order_by(
            PluginHook.average_execution_time.desc()
        ).limit(limit).all()
...
class PluginLogRepository(BaseRepository[PluginLog]):
    ...
    def get_by_plugin_id(
        ...
        total = query.count()
        logs = query.order_by(PluginLog.created_at.desc()).offset(skip).limit(limit).all()
        return logs, total
    ...
    def get_recent_logs(
        ...
        query = self.db.query(PluginLog).filter(
            PluginLog.created_at >= cutoff_time
        )
...
class PluginRegistryRepository(BaseRepository[PluginRegistry]):
    ...
    def get_popular_plugins(self, limit: int = 10) -> List[PluginRegistry]:
        ...
        return self.db.query(PluginRegistry).order_by(
            PluginRegistry.download_count.desc()
        ).limit(limit).all()
    
    def get_top_rated_plugins(self, limit: int = 10) -> List[PluginRegistry]:
        """Get top rated plugins"""
        return self.db.query(PluginRegistry).order_by(
            PluginRegistry.rating.desc()
        ).limit(limit).all()
    ...
    def search_registry(
        ...
        total = query.count()
        plugins = query.order_by(PluginRegistry.created_at.desc()).offset(skip).limit(limit).all()
...
class PluginTemplateRepository(BaseRepository[PluginTemplate]):
    ...
    def get_by_type(self, plugin_type: PluginType) -> List[PluginTemplate]:
        ...
        return self.db.query(PluginTemplate).filter(
            PluginTemplate.plugin_type == plugin_type
        ).all()
    ...
    def get_most_used_templates(self, limit: int = 10) -> List[PluginTemplate]:
        ...
        return self.db.query(PluginTemplate).order_by(
            PluginTemplate.usage_count.desc()
        ).limit(limit).all()
    
    def get_templates_by_author(self, author: str) -> List[PluginTemplate]:
        """Get templates by author"""
        return self.db.query(PluginTemplate).filter(
            PluginTemplate.author.ilike(f"%{author}%")
        ).all()
    ...
    def search_templates(
        ...
        total = query.count()
        templates = query.order_by(PluginTemplate.created_at.desc()).offset(skip).limit(limit).all()
