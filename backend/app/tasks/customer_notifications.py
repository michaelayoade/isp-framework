...
@celery_app.task(bind=True, base=ISPFrameworkTask, name='app.tasks.customer_notifications.send_email')
def send_email(self, customer_id: int, template_name: str, context: Dict[str, Any], 
               subject: str = None, priority: str = 'normal') -> Dict[str, Any]:
    """
...
@celery_app.task(bind=True, base=ISPFrameworkTask, name='app.tasks.customer_notifications.send_sms')
def send_sms(self, customer_id: int, message: str, priority: str = 'normal') -> Dict[str, Any]:
    """
...
@celery_app.task(bind=True, base=ISPFrameworkTask, name='app.tasks.customer_notifications.send_service_suspension_notification')
def send_service_suspension_notification(self, customer_id: int, customer_service_id: int, 
                                       suspension_reason: str, grace_period_hours: int) -> Dict[str, Any]:
    """Send notification about service suspension."""
    try:
        db = next(get_db())
        
        customer = db.query(Customer).filter(Customer.id == customer_id).first()
        customer_service = db.query(CustomerService).filter(CustomerService.id == customer_service_id).first()
        ...
        sms_message = f"NOTICE: Your internet service has been suspended due to {suspension_reason}. " \
        ...
        logger.info(
            "Service suspension notifications sent",
            **result
        )
...
@celery_app.task(bind=True, base=ISPFrameworkTask, name='app.tasks.customer_notifications.send_service_restoration_notification')
def send_service_restoration_notification(self, customer_id: int, customer_service_id: int, 
                                        restoration_notes: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            "Service restoration notifications sent",
            **result
        )
...
@celery_app.task(bind=True, base=ISPFrameworkTask, name='app.tasks.customer_notifications.send_bulk_notifications')
def send_bulk_notifications(self, customer_ids: List[int], template_name: str, 
                          context: Dict[str, Any], notification_type: str = 'email') -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for customer_id in customer_ids:
            try:
                if notification_type == 'email':
                    ...
                    message = context.get('message', 'Bulk notification from ISP Framework')
                    task = send_sms.delay(
                        customer_id=customer_id,
                        message=message,
                        priority='bulk'
        ...
        logger.info(
            "Bulk notification queuing completed",
            **result
        )
