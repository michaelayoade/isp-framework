...
@celery_app.task(bind=True, base=ISPFrameworkTask, name='app.tasks.maintenance_tasks.add_to_dead_letter_queue')
...
@celery_app.task(bind=True, base=ISPFrameworkTask, name='app.tasks.maintenance_tasks.process_dead_letter_queue')
def process_dead_letter_queue(self) -> Dict[str, Any]:
    ...
    try:
        ...
        pending_items = db.query(DeadLetterQueue).filter(
            DeadLetterQueue.status == 'pending',
            DeadLetterQueue.created_at < datetime.now(timezone.utc) - timedelta(minutes=10)  # Wait 10 minutes before processing
        ).limit(50).all()
        ...
        logger.info(
            "Dead letter queue processing completed",
            **result
        )
...
@celery_app.task(bind=True, base=ISPFrameworkTask, name='app.tasks.maintenance_tasks.cleanup_failed_tasks')
def cleanup_failed_tasks(self) -> Dict[str, Any]:
    ...
    try:
        ...
        old_dlq_count = db.query(DeadLetterQueue).filter(
        ...
        old_log_count = db.query(TaskExecutionLog).filter(
...
@celery_app.task(bind=True, base=ISPFrameworkTask, name='app.tasks.maintenance_tasks.retry_dead_letter_task')
def retry_dead_letter_task(self, dlq_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if not dlq_item:
            raise ValueError(f"Dead letter queue item {dlq_id} not found or not retryable")
        ...
        logger.error(
            "Manual retry of dead letter task failed",
            dlq_id=dlq_id,
            error=str(e)
        )
        raise
...
@celery_app.task(bind=True, base=ISPFrameworkTask, name='app.tasks.maintenance_tasks.get_dead_letter_stats')
def get_dead_letter_stats(self) -> Dict[str, Any]:
    ...
    try:
        ...
        stats['failed_by_task'] = {task_name: count for task_name, count in failed_by_task}
        
        # Get recent failures (last 24 hours)
        recent_cutoff = datetime.now(timezone.utc) - timedelta(hours=24)
        recent_failures = db.query(DeadLetterQueue).filter(
...
def _log_task_execution(task_id: str, task_name: str, status: str, start_time: datetime, 
                       end_time: datetime, result: str = None, error: str = None):
