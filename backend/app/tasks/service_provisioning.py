...
from app.models.networking.ipam import IPAllocation, IPPool
from app.models.services.instances import CustomerInternetService, CustomerService
...
@celery_app.task(
    ...
    name="app.tasks.service_provisioning.provision_internet_service",
)
def provision_internet_service(
    self, customer_service_id: int, provisioning_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        internet_service = _create_internet_service_record(
            db, customer_service, ip_allocation, provisioning_data
        )

        # Step 3: Configure router/NAS
        router_config = _configure_router_for_service(
            db, customer_service, internet_service, provisioning_data
        )

        # Step 4: Create RADIUS user
        radius_config = _create_radius_user(
            db, customer, internet_service, provisioning_data
        )

        # Step 5: Update service status
        customer_service.status = "active"
        customer_service.activated_at = datetime.now(timezone.utc)
        customer_service.notes = (
            f"Automatically provisioned by task {current_task.request.id}"
        )
        ...
        logger.info("Internet service provisioning completed successfully", **result)
...
@celery_app.task(
    bind=True,
    base=ISPFrameworkTask,
    name="app.tasks.service_provisioning.suspend_service",
)
def suspend_service(
    self, customer_service_id: int, suspension_reason: str, grace_period_hours: int = 24
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        customer_service.notes = (
            f"Suspended by task {current_task.request.id}: {suspension_reason}"
        )

        # Step 2: Configure router for suspension
        router_config = _configure_router_suspension(
            db, customer_service, grace_period_hours
        )
        ...
        from app.tasks.customer_notifications import (
            send_service_suspension_notification,
        )

        notification_task = send_service_suspension_notification.delay(
            customer.id, customer_service_id, suspension_reason, grace_period_hours
        )
        ...
        logger.info("Service suspension completed successfully", **result)
...
@celery_app.task(
    bind=True,
    base=ISPFrameworkTask,
    name="app.tasks.service_provisioning.restore_service",
)
def restore_service(
    self, customer_service_id: int, restoration_notes: str = ""
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        customer_service.notes = (
            f"Restored by task {current_task.request.id}: {restoration_notes}"
        )
        ...
        # Step 4: Schedule customer notification
        from app.tasks.customer_notifications import (
            send_service_restoration_notification,
        )

        notification_task = send_service_restoration_notification.delay(
            customer.id, customer_service_id, restoration_notes
        )
        ...
        logger.info("Service restoration completed successfully", **result)
...
def _allocate_customer_ip(
    db, customer_service, provisioning_data
    ...
    try:
        ...
        ip_pool = (
            db.query(IPPool)
            .filter(IPPool.is_active is True, IPPool.pool_type == "customer")
            .first()
        )
...
def _create_internet_service_record(
    db, customer_service, ip_allocation, provisioning_data
) -> Optional[CustomerInternetService]:
...
def _configure_router_for_service(
    db, customer_service, internet_service, provisioning_data
) -> Dict[str, Any]:
...
def _configure_router_suspension(
    db, customer_service, grace_period_hours
) -> Dict[str, Any]:
