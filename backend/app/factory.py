...
def create_app(include_optional_routers: bool = None) -> FastAPI:
    ...
    if include_optional_routers is None:
        include_optional_routers = os.getenv("ISP_FULL_IMPORT", "1").lower() in (
            "1",
            "true",
            "yes",
        )
    ...
    app.add_middleware(
        ...
        allow_origins=(
            ...
            else getattr(settings, "ALLOWED_HOSTS", ["*"])
        ),
    ...
    try:
        ...
        app.include_router(
            customers.router, prefix="/api/v1/customers", tags=["customers"]
        )
        app.include_router(
            administrators.router,
            prefix="/api/v1/administrators",
            tags=["administrators"],
        )
        app.include_router(
            resellers.router, prefix="/api/v1/resellers", tags=["resellers"]
        )

        # Core business functionality
        app.include_router(billing.router, prefix="/api/v1/billing", tags=["billing"])
        app.include_router(
            ticketing.router, prefix="/api/v1/tickets", tags=["ticketing"]
        )
        app.include_router(
    ...
    if include_optional_routers:
        try:
            ...
            app.include_router(
                internet_services.router,
                prefix="/api/v1/services/internet",
                tags=["internet-services"],
            )
            app.include_router(
                voice_services.router,
                prefix="/api/v1/services/voice",
                tags=["voice-services"],
            )
            app.include_router(
                bundle_services.router,
                prefix="/api/v1/services/bundles",
                tags=["bundle-services"],
            )

            # Network and infrastructure
            app.include_router(
                network.router, prefix="/api/v1/network", tags=["network"]
            )
            app.include_router(
            ...
            # Platform features
            app.include_router(
                plugins.router, prefix="/api/v1/plugins", tags=["plugins"]
            )
            app.include_router(
                api_management.router,
                prefix="/api/v1/api-management",
                tags=["api-management"],
            )
            app.include_router(
            ...
            app.include_router(
                file_storage.router, prefix="/api/v1/files", tags=["file-storage"]
            )
            app.include_router(
    ...
    async def root():
        ...
        return {
            ...
            "version": "1.0.0",
            "docs": (
                "/docs" if getattr(settings, "DEBUG", True) else "Contact administrator"
            ),
        }
