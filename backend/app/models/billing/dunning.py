...
from .enums import DunningStatus, DunningActionType, EscalationLevel, DeliveryStatus
...
class DunningCase(Base):
    ...
    # Status and workflow
    status = Column(SQLEnum(DunningStatus), nullable=False, default=DunningStatus.ACTIVE, index=True)
    current_stage = Column(Integer, default=1)  # Dunning stage (1, 2, 3, etc.)
    escalation_level = Column(SQLEnum(EscalationLevel), default=EscalationLevel.LOW, index=True)
    ...
    preferred_contact_method = Column(String(20), default="email")  # email, sms, phone, letter
    ...
    # Relationships
    billing_account = relationship("CustomerBillingAccount", back_populates="dunning_cases")
    assigned_user = relationship("Administrator", foreign_keys=[assigned_to])
    actions = relationship("DunningAction", back_populates="dunning_case", cascade="all, delete-orphan")
    ...
    def escalate_to_next_stage(self):
        ...
        if self.escalation_level == EscalationLevel.CRITICAL:
            ...
            self.next_action_date = datetime.now(timezone.utc) + timedelta(days=self.contact_frequency_days)
    ...
    def resolve_case(self, resolution_notes=None):
        ...
        if resolution_notes:
            self.notes = f"{self.notes}\nResolved: {resolution_notes}" if self.notes else f"Resolved: {resolution_notes}"
...
class DunningAction(Base):
    ...
    action_date = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))
    ...
    delivery_status = Column(SQLEnum(DeliveryStatus), default=DeliveryStatus.PENDING, index=True)
...
class DunningRule(Base):
    ...
    def is_triggered(self, days_overdue, overdue_amount, escalation_level):
        ...
        if self.trigger_amount_threshold and overdue_amount < self.trigger_amount_threshold:
            return False
        
        # Check escalation level
        if self.trigger_escalation_level and escalation_level != self.trigger_escalation_level:
            return False
    ...
    def get_rule_summary(self):
        ...
        return {
            ...
            "trigger_conditions": {
                ...
                "amount_threshold": float(self.trigger_amount_threshold) if self.trigger_amount_threshold else None,
                "escalation_level": self.trigger_escalation_level.value if self.trigger_escalation_level else None
            },
            "action_type": self.action_type.value,
            "escalation": {
