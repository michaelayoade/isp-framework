...
class BillingType(Base):
    ...
    # Credit properties
    supports_credit_limit = Column(
        Boolean, default=False
    )  # Does this type support credit limits
    default_credit_limit = Column(Integer, default=0)  # Default credit limit (in cents)

    # UI properties
    color_hex = Column(
        String(7), default="#6B7280"
    )  # Color for billing type badges (#RRGGBB)
