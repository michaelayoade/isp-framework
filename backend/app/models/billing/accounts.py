...
from .enums import AccountStatus, BillingCycleType, BillingType
...
class CustomerBillingAccount(Base):
    ...
    billing_type = Column(
        SQLEnum(BillingType), nullable=False, default=BillingType.POSTPAID
    )
    billing_cycle = Column(
        SQLEnum(BillingCycleType), nullable=False, default=BillingCycleType.MONTHLY
    )
    ...
    balance_history = relationship(
        "BalanceHistory", back_populates="billing_account", cascade="all, delete-orphan"
    )
    ...
    def has_sufficient_balance(self):
        ...
        if self.billing_type == BillingType.PREPAID:
            ...
            return (
                ...
                or self.current_balance > -self.credit_limit
            )
    ...
    def get_account_summary(self):
        ...
        return {
            ...
            "minimum_balance": (
                float(self.minimum_balance) if self.minimum_balance else 0
            ),
            ...
            "last_billed_date": (
                self.last_billed_date.isoformat() if self.last_billed_date else None
            ),
            "next_billing_date": (
