...
from .enums import PaymentPlanStatus, InstallmentStatus
...
class PaymentPlan(Base):
    ...
    frequency = Column(String(20), nullable=False, default="monthly")  # monthly, weekly, bi-weekly
    
    # Status
    status = Column(SQLEnum(PaymentPlanStatus), nullable=False, default=PaymentPlanStatus.ACTIVE, index=True)
    ...
    # Relationships
    billing_account = relationship("CustomerBillingAccount", back_populates="payment_plans")
    ...
    def is_current(self):
        ...
        overdue_installments = [i for i in self.installments 
    ...
    def next_installment(self):
        ...
        pending_installments = [i for i in self.installments 
    ...
    def mark_as_defaulted(self, reason=None):
        ...
        if reason:
            self.notes = f"{self.notes}\nDefaulted: {reason}" if self.notes else f"Defaulted: {reason}"
    ...
    def get_plan_summary(self):
        ...
        return {
            ...
            "defaulted_date": self.defaulted_date.isoformat() if self.defaulted_date else None
        }
...
class PaymentPlanInstallment(Base):
    ...
    # Status
    status = Column(SQLEnum(InstallmentStatus), nullable=False, default=InstallmentStatus.PENDING, index=True)
    ...
    def apply_payment(self, payment_amount, payment_id=None):
        ...
        self.balance_due = max(0, self.amount + self.late_fee_applied - self.paid_amount)
    ...
    def get_installment_summary(self):
        ...
        return {
            ...
            "late_fee_applied": float(self.late_fee_applied) if self.late_fee_applied else 0,
            "is_overdue": self.is_overdue,
            "days_overdue": self.days_overdue,
            "is_paid": self.is_paid,
            "paid_date": self.paid_date.isoformat() if self.paid_date else None,
            "late_fee_date": self.late_fee_date.isoformat() if self.late_fee_date else None
        }
...
class PaymentPlanTemplate(Base):
    ...
    def can_auto_approve(self, amount):
        ...
        return (self.auto_approve and 
                self.auto_approve_limit and 
    ...
    def get_template_summary(self):
        ...
        return {
            ...
            "defaults": {
                ...
                "interest_rate": float(self.default_interest_rate) if self.default_interest_rate else 0,
                "late_fee": float(self.default_late_fee) if self.default_late_fee else 0,
            ...
            "eligibility": {
                ...
                "minimum_down_payment_percentage": float(self.minimum_down_payment_percentage) if self.minimum_down_payment_percentage else 0,
                "maximum_installments": self.maximum_installments
            },
            "auto_approval": {
                "enabled": self.auto_approve,
                "limit": float(self.auto_approve_limit) if self.auto_approve_limit else None
            }
        }
