...
class BillingCycle(Base):
    ...
    def is_ready_for_generation(self):
        ...
        return (
            self.status == "active"
            and not self.invoices_generated
            and datetime.now(timezone.utc) >= self.start_date
        )
        ...
        if self.cycle_type == BillingCycleType.MONTHLY:
            ...
            if next_start.month == 12:
                next_end = next_start.replace(
                    year=next_start.year + 1, month=1, day=self.cycle_day
                ) - timedelta(days=1)
            else:
                next_end = next_start.replace(
                    month=next_start.month + 1, day=self.cycle_day
                ) - timedelta(days=1)
    ...
    def get_cycle_summary(self):
        ...
        return {
            ...
            "generation_date": (
                self.generation_date.isoformat() if self.generation_date else None
            ),
            "statistics": {
        ...
        if self.cycle_type == BillingCycleType.MONTHLY:
            if start_date.month == 12:
                end_date = start_date.replace(
                    year=start_date.year + 1, month=1, day=self.cycle_day
                ) - timedelta(days=1)
            else:
                end_date = start_date.replace(
                    month=start_date.month + 1, day=self.cycle_day
                ) - timedelta(days=1)
        elif self.cycle_type == BillingCycleType.QUARTERLY:
...
class BillingCycleJob(Base):
    ...
    )  # generate_invoices, send_invoices, etc.
