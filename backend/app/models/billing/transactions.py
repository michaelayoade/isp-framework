...
from sqlalchemy import Column, Integer, String, Boolean, DateTime, DECIMAL, ForeignKey, Enum as SQLEnum, Index
...
class BillingTransaction(Base):
    ...
    transaction_id = Column(UUID(as_uuid=True), default=uuid.uuid4, unique=True, nullable=False, index=True)
    ...
    processing_date = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc))
    ...
    # Relationships
    billing_account = relationship("CustomerBillingAccount", back_populates="transactions")
    ...
    # Indexes
    __table_args__ = (
        Index('idx_billing_transactions_account_date', 'billing_account_id', 'processing_date'),
...
class BalanceHistory(Base):
    ...
    transaction_id = Column(UUID(as_uuid=True), ForeignKey("billing_transactions.transaction_id"))
    ...
    # Relationships
    billing_account = relationship("CustomerBillingAccount", back_populates="balance_history")
...
class TransactionBatch(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    batch_id = Column(UUID(as_uuid=True), default=uuid.uuid4, unique=True, nullable=False, index=True)
    ...
    def get_batch_summary(self):
        ...
        return {
            ...
            "processing_time": (self.completed_at - self.started_at).total_seconds() if self.started_at and self.completed_at else None
        }
