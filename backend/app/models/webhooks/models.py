...
class WebhookEventType(Base):
    ...
    name = Column(String(100), unique=True, nullable=False, index=True)  # e.g., "customer.created"
    ...
    endpoint_subscriptions = relationship("WebhookEndpoint", secondary="webhook_endpoint_events", back_populates="subscribed_events")
...
class WebhookEndpoint(Base):
    ...
    signature_algorithm = Column(SQLEnum(SignatureAlgorithm), default=SignatureAlgorithm.HMAC_SHA256)
    ...
    # Relationships
    subscribed_events = relationship("WebhookEventType", secondary="webhook_endpoint_events", back_populates="endpoint_subscriptions")
    deliveries = relationship("WebhookDelivery", back_populates="endpoint", cascade="all, delete-orphan")
    filters = relationship("WebhookFilter", back_populates="endpoint", cascade="all, delete-orphan")
    secrets = relationship("WebhookSecret", back_populates="endpoint", cascade="all, delete-orphan")
...
class WebhookEndpointEvent(Base):
    ...
    endpoint_id = Column(Integer, ForeignKey("webhook_endpoints.id", ondelete="CASCADE"), primary_key=True)
    event_type_id = Column(Integer, ForeignKey("webhook_event_types.id", ondelete="CASCADE"), primary_key=True)
...
class WebhookEvent(Base):
    ...
    event_type_id = Column(Integer, ForeignKey("webhook_event_types.id"), nullable=False)
...
class WebhookDelivery(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    delivery_id = Column(String(36), unique=True, nullable=False, index=True, default=lambda: str(uuid.uuid4()))
...
class WebhookDeliveryAttempt(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    delivery_id = Column(Integer, ForeignKey("webhook_deliveries.id", ondelete="CASCADE"), nullable=False)
...
class WebhookFilter(Base):
    ...
    # Filter logic
    field_path = Column(String(500), nullable=False)  # JSON path to field (e.g., "customer.status")
...
class WebhookSecret(Base):
    ...
    algorithm = Column(SQLEnum(SignatureAlgorithm), default=SignatureAlgorithm.HMAC_SHA256)
...
# Performance indexes
Index('idx_webhook_events_type_occurred', WebhookEvent.event_type_id, WebhookEvent.occurred_at)
Index('idx_webhook_events_processed', WebhookEvent.is_processed, WebhookEvent.created_at)
Index('idx_webhook_deliveries_status_scheduled', WebhookDelivery.status, WebhookDelivery.scheduled_at)
Index('idx_webhook_deliveries_retry', WebhookDelivery.next_retry_at, WebhookDelivery.status)
Index('idx_webhook_delivery_attempts_delivery_attempt', WebhookDeliveryAttempt.delivery_id, WebhookDeliveryAttempt.attempt_number)
Index('idx_webhook_endpoints_status_active', WebhookEndpoint.status, WebhookEndpoint.is_active)
