...
class ProvisioningJob(Base):
    ...
    job_history = relationship(
        "ProvisioningJobHistory", back_populates="job", cascade="all, delete-orphan"
    )
...
class ProvisioningJobHistory(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    job_id = Column(
        Integer, ForeignKey("provisioning_jobs.id"), nullable=False, index=True
    )
...
class ProvisioningWorkerStatus(Base):
    ...
    supported_service_types = Column(
        JSON, nullable=True
    )  # ["internet", "voice", "bundle"]
