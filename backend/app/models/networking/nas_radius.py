...
class RADIUSAccounting(Base):
    ...
    radius_session_id = Column(Integer, ForeignKey("radius_sessions_modular.id", ondelete="CASCADE"), index=True)
    nas_device_id = Column(Integer, ForeignKey("nas_devices.id"), nullable=False)
    ...
    # Record metadata
    record_timestamp = Column(DateTime(timezone=True), server_default=func.now(), index=True)
    ...
    # Relationships
    session = relationship("RADIUSSession", foreign_keys=[radius_session_id], back_populates="accounting_records")
...
class RADIUSAlert(Base):
    ...
    # Alert details
    alert_type = Column(String(50), nullable=False)  # server_down, high_reject_rate, session_limit
