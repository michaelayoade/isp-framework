...
class RadiusSession(Base):
    ...
    interim_updates = relationship(
        "RADIUSInterimUpdate", back_populates="session", cascade="all, delete-orphan"
    )
    ...
    def session_duration_seconds(self):
        ...
        if self.stop_time:
            ...
            return int(
                (datetime.now(self.start_time.tzinfo) - self.start_time).total_seconds()
            )
        return 0
    ...
    def is_active(self):
        ...
        return (
            self.status in [SessionStatus.ACTIVE, SessionStatus.INTERIM]
            and not self.stop_time
        )
...
class RADIUSInterimUpdate(Base):
    ...
    radius_session_id = Column(
        Integer, ForeignKey("radius_sessions.id", ondelete="CASCADE"), nullable=False
    )
    ...
    session = relationship(
        ...
        back_populates="interim_updates",
    )
...
class CustomerStatistics(Base):
    ...
    # Time period
    period_type = Column(
        String(20), default="daily", index=True
    ...
    def __repr__(self):
        return (
            f"<CustomerStatistics {self.login}: {self.period_type} {self.period_start}>"
        )
...
Index(
    "idx_radius_sessions_customer_active",
    RadiusSession.customer_id,
    RadiusSession.status,
)
Index(
    "idx_radius_sessions_nas_time",
    RadiusSession.nas_device_id,
    RadiusSession.start_time,
)
...
Index(
    "idx_interim_updates_session_time",
    RADIUSInterimUpdate.radius_session_id,
    RADIUSInterimUpdate.update_time,
)

# Statistics indexes
Index(
    "idx_session_stats_customer_period",
    CustomerStatistics.customer_id,
    CustomerStatistics.period_type,
    CustomerStatistics.period_start,
)
Index(
    "idx_session_stats_nas_period",
    CustomerStatistics.nas_device_id,
    CustomerStatistics.period_type,
    CustomerStatistics.period_start,
)
