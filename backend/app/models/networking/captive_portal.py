...
class CaptivePortalSettings(Base):
    ...
    # Payment Integration
    payment_gateway = Column(String(50), nullable=True)  # stripe, paystack, flutterwave, etc.
    ...
    def __repr__(self):
        return f"<CaptivePortalSettings(id={self.id}, company_name={self.company_name})>"
...
class CaptivePortalSession(Base):
    ...
    # Session Status
    status = Column(String(20), nullable=False, default="pending")  # pending, authenticated, expired, terminated
    auth_method = Column(String(20), nullable=True)  # voucher, payment, guest, credentials
...
class CaptivePortalVoucherUsage(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    voucher_id = Column(Integer, ForeignKey("captive_portal_vouchers.id"), nullable=False, index=True)
    session_id = Column(Integer, ForeignKey("captive_portal_sessions.id"), nullable=False, index=True)
...
class CaptivePortalTemplate(Base):
    ...
    # Template Content
    template_type = Column(String(50), nullable=False)  # login, payment, success, error, terms
