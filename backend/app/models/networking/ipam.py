...
class IPAllocation(Base):
    ...
    status = Column(
        Enum(AllocationStatus), nullable=False, default=AllocationStatus.ALLOCATED
    )
...
class IPAMHistory(Base):
    ...
    )  # allocated, released, reserved, modified
...
class IPAMTemplate(Base):
    ...
    # Default settings
    default_gateway_offset = Column(
        Integer, default=1
    )  # Gateway IP offset from network
    dns_servers = Column(ARRAY(String))
    domain_name = Column(String(255))
