...
class NetworkDevice(Base):
    ...
    parent_device_id = Column(
        Integer, ForeignKey("network_devices.id")
    )  # Hierarchical relationship
    primary_device_id = Column(
        Integer, ForeignKey("network_devices.id")
    )  # Backup/redundancy
    ...
    parent_device = relationship(
        "NetworkDevice", remote_side=[id], foreign_keys=[parent_device_id]
    )
    child_devices = relationship(
        "NetworkDevice", foreign_keys=[parent_device_id], overlaps="parent_device"
    )
    primary_device = relationship(
        "NetworkDevice", remote_side=[id], foreign_keys=[primary_device_id]
    )
    backup_devices = relationship(
        "NetworkDevice", foreign_keys=[primary_device_id], overlaps="primary_device"
    )
...
class DeviceConnection(Base):
    ...
    from_device = relationship(
        "NetworkDevice", foreign_keys=[from_device_id], overlaps="outgoing_connections"
    )
    to_device = relationship(
        "NetworkDevice", foreign_keys=[to_device_id], overlaps="incoming_connections"
    )
...
class DeviceMetric(Base):
    ...
    )  # cpu, memory, interface_traffic, temperature
...
class NetworkAlert(Base):
    ...
    alert_type = Column(
        String(50), nullable=False
    )  # device_down, high_cpu, interface_down
...
class NetworkChange(Base):
    ...
    change_type = Column(
        String(50), nullable=False
    )  # config, physical, ip_assignment, firmware
    title = Column(String(255), nullable=False)
    description = Column(Text)
...
class NetworkRedundancy(Base):
    ...
    primary_device_id = Column(
        Integer, ForeignKey("network_devices.id"), nullable=False
    )
    backup_device_id = Column(Integer, ForeignKey("network_devices.id"), nullable=False)

    # Redundancy configuration
    redundancy_type = Column(
        String(50), nullable=False
    )  # active_passive, active_active, load_balance
