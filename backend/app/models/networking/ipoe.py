...
class OLTProfile(Base):
    ...
    # Speed Templates
    speed_profiles = Column(
        JSON, default=dict
    )  # {"profile_name": {"down": 100000, "up": 50000}}
...
class AccessCircuit(Base):
    ...
    )  # available, assigned, active, suspended, faulty
    provision_status = Column(
...
class IPoESession(Base):
    ...
    circuit_id = Column(
        String(255),
        ForeignKey("access_circuits.circuit_id"),
        nullable=False,
        index=True,
    )
    ...
    service_id = Column(
        Integer, ForeignKey("customer_services.id"), nullable=False, index=True
    )
