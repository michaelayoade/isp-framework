...
from sqlalchemy import Column, Integer, String, DECIMAL, Boolean, DateTime, Text, ForeignKey, BigInteger, Enum
...
class Tariff(Base):
    ...
    # Relationships
    billing_options = relationship("TariffBillingOption", back_populates="tariff", cascade="all, delete-orphan")
    internet_config = relationship("InternetTariffConfig", back_populates="tariff", uselist=False, cascade="all, delete-orphan")
    zone_pricing = relationship("TariffZonePricing", back_populates="tariff", cascade="all, delete-orphan")
...
class InternetTariffConfig(Base):
    ...
    tariff_id = Column(Integer, ForeignKey("tariffs.id", ondelete="CASCADE"), primary_key=True)
    ...
    allowed_protocols = Column(JSONB, default=[])  # ['http', 'https', 'ftp'] or [] for all
    blocked_ports = Column(JSONB, default=[])     # [25, 135, 445] blocked ports
    
    # Time-based Restrictions
    time_restrictions = Column(JSONB, default={})  # {'weekday_hours': '08:00-22:00', 'weekend_hours': 'unlimited'}
...
class TariffBillingOption(Base):
    ...
    # Pricing modifiers
    price_modifier_percent = Column(DECIMAL(5, 2), default=0)  # -10 for discount, +5 for premium
...
Index('idx_zone_pricing_tariff_location', TariffZonePricing.tariff_id, TariffZonePricing.location_id)
Index('idx_promotions_tariff_valid', TariffPromotion.tariff_id, TariffPromotion.valid_from, TariffPromotion.valid_until)
