...
class FileMetadata(Base):
    ...
    permissions = relationship(
        "FilePermission", back_populates="file_metadata", cascade="all, delete-orphan"
    )
...
class ExportJob(Base):
    ...
    export_type = Column(
        String(50), nullable=False
    )  # 'customer_export', 'billing_export', etc.
    ...
    # File info
    output_file_metadata_id = Column(
        Integer, ForeignKey("file_metadata.id"), nullable=True
    )
    output_file_metadata = relationship(
        "FileMetadata", foreign_keys=[output_file_metadata_id]
    )
