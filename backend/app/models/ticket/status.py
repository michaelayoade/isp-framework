...
class TicketStatus(Base):
    ...
    code = Column(String(50), unique=True, nullable=False, index=True)  # open, in_progress, resolved, etc.
    ...
    # Workflow properties
    is_initial = Column(Boolean, default=False)  # Is this a starting status for new tickets
    is_final = Column(Boolean, default=False)  # Is this a final status (resolved, closed, cancelled)
    requires_assignment = Column(Boolean, default=False)  # Must ticket be assigned to proceed
    auto_close_after_days = Column(Integer)  # Auto-close after N days (for resolved tickets)
    
    # Customer visibility
    is_customer_visible = Column(Boolean, default=True)  # Should customers see this status
