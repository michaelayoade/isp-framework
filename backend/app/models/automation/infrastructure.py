...
from sqlalchemy import Column, Integer, String, Boolean, DateTime, ForeignKey, Text, JSON, DECIMAL, Index
...
class Site(Base):
    ...
    # Site Classification
    site_type = Column(String(50), nullable=False)  # datacenter, pop, tower, office, customer_premises
...
class AutomationDevice(Base):
    ...
    device_type = Column(String(50), nullable=False)  # router, switch, olt, onu, ap, firewall
    vendor = Column(String(50), nullable=False)  # mikrotik, cisco, huawei, zte, ubiquiti
    ...
    # Location
    site_id = Column(Integer, ForeignKey("automation_sites.id"), nullable=False, index=True)
    rack_position = Column(String(50), nullable=True)
    physical_location = Column(String(255), nullable=True)
    
    # Device Status
    status = Column(String(20), nullable=False, default="active")  # active, inactive, maintenance, faulty
    is_managed = Column(Boolean, default=True)  # Whether device is under Ansible management
...
class DeviceCredential(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    device_id = Column(Integer, ForeignKey("automation_devices.id"), nullable=False, index=True)
...
class ProvisioningTask(Base):
    ...
    # Task Classification
    task_type = Column(String(50), nullable=False)  # provision, configure, backup, update, monitor
    category = Column(String(50), nullable=True)  # customer_setup, maintenance, security, monitoring
    priority = Column(String(20), default="medium")  # low, medium, high, critical
    
    # Target Information
    device_id = Column(Integer, ForeignKey("automation_devices.id"), nullable=True, index=True)
    site_id = Column(Integer, ForeignKey("automation_sites.id"), nullable=True, index=True)
    ...
    depends_on_task_id = Column(Integer, ForeignKey("automation_provisioning_tasks.id"), nullable=True)
    
    # Execution Context
    executed_by = Column(Integer, ForeignKey("administrators.id"), nullable=True)
    execution_node = Column(String(255), nullable=True)  # Which server executed the task
...
class AnsiblePlaybook(Base):
    ...
    # Classification
    category = Column(String(50), nullable=False)  # provisioning, configuration, maintenance, monitoring
