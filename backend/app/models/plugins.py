...
class Plugin(Base):
    ...
    dependencies = relationship("PluginDependency", foreign_keys="PluginDependency.plugin_id", back_populates="plugin")
...
class PluginConfiguration(Base):
    ...
    config_type = Column(String(50), default="string")  # string, number, boolean, object, array
...
class PluginHook(Base):
    ...
    # Hook information
    hook_name = Column(String(255), nullable=False)  # e.g., "customer.created", "invoice.generated"
...
class PluginRegistry(Base):
    ...
    # Relationships
    installations = relationship("Plugin", primaryjoin="PluginRegistry.name == Plugin.name", foreign_keys="Plugin.name", viewonly=True)
