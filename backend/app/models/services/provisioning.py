...
class ServiceProvisioning(Base):
    ...
    # Provisioning Status
    status = Column(Enum(ProvisioningStatus), default=ProvisioningStatus.NOT_STARTED, index=True)
    ...
    def provisioning_duration_hours(self):
        ...
        end_time = self.provisioning_completed or datetime.now(self.provisioning_started.tzinfo)
    ...
    def add_error(self, error_message: str, error_code: str = None, details: dict = None):
...
class ProvisioningQueue(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    service_provisioning_id = Column(Integer, ForeignKey("service_provisioning.id", ondelete="CASCADE"), nullable=False)
    ...
    blocks_provisioning_ids = Column(JSONB, default=list)     # Provisioning IDs blocked by this
...
Index('idx_provisioning_progress', ServiceProvisioning.status, ServiceProvisioning.provisioning_started)

# Provisioning queue indexes
Index('idx_queue_active_priority', ProvisioningQueue.is_active, ProvisioningQueue.priority)
