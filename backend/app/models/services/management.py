...
class ServiceStatusHistory(Base):
    ...
    related_ticket_id = Column(
        Integer
    )  # ForeignKey("tickets.id") - commented until tickets table exists
...
class ServiceSuspension(Base):
    ...
    # Automation
    auto_restore_conditions = Column(
        JSONB, default=dict
    )  # Conditions for auto-restoration
    auto_suspend_conditions = Column(
        JSONB, default=dict
    )  # Conditions that triggered suspension
...
class ServiceUsageTracking(Base):
    ...
    def __repr__(self):
        return (
            f"<ServiceUsageTracking {self.customer_service_id}: {self.tracking_date}>"
        )

    @property
    def total_traffic_gb(self):
        """Calculate total traffic in GB"""
        total_bytes = float(self.bytes_downloaded or 0) + float(
...
Index(
    "idx_status_history_service_date",
    ServiceStatusHistory.customer_service_id,
    ServiceStatusHistory.changed_at,
)
Index(
    "idx_status_history_status",
...
# Suspension indexes
Index(
    "idx_suspensions_active",
    ServiceSuspension.is_active,
    ServiceSuspension.suspension_date,
)
...
Index(
    "idx_usage_tracking_service_date",
    ServiceUsageTracking.customer_service_id,
    ServiceUsageTracking.tracking_date,
)
Index(
