...
class ServiceTemplate(Base):
    ...
    internet_config = relationship(
        "InternetServiceTemplate", back_populates="service_template", uselist=False
    )
    voice_config = relationship(
        "VoiceServiceTemplate", back_populates="service_template", uselist=False
    )
    bundle_config = relationship(
        "BundleServiceTemplate", back_populates="service_template", uselist=False
    )
    customer_services = relationship(
        "CustomerService", back_populates="service_template"
    )
...
class InternetServiceTemplate(Base):
    ...
    content_filtering_level = Column(
        Enum(ContentFilterLevel), default=ContentFilterLevel.NONE
    )
    ...
    def guaranteed_speed_kbps(self):
        ...
        return {
            "download": int(
                self.download_speed_kbps * self.guaranteed_speed_percent / 100
            ),
...
Index(
    "idx_service_templates_type_active",
    ServiceTemplate.service_type,
    ServiceTemplate.is_active,
)
Index(
    "idx_service_templates_category",
    ServiceTemplate.category,
    ServiceTemplate.is_active,
)
Index(
    "idx_service_templates_featured",
    ServiceTemplate.is_featured,
    ServiceTemplate.display_order,
)
Index(
    "idx_service_templates_public", ServiceTemplate.is_public, ServiceTemplate.is_active
)

# Internet service template indexes
Index(
    "idx_internet_templates_speed",
    InternetServiceTemplate.download_speed_kbps,
    InternetServiceTemplate.upload_speed_kbps,
)
...
Index(
    "idx_voice_templates_features",
    VoiceServiceTemplate.caller_id,
    VoiceServiceTemplate.voicemail,
)
