...
class CustomerService(Base):
    ...
    ipoe_auth_enabled = Column(
        Boolean, default=False
    )  # Enable IPoE/DHCP Option 82 authentication
    ...
    # Service History
    previous_service_id = Column(
        Integer, ForeignKey("customer_services.id")
    ...
    internet_config = relationship(
        "CustomerInternetService", back_populates="customer_service", uselist=False
    )
    voice_config = relationship(
        "CustomerVoiceService", back_populates="customer_service", uselist=False
    )

    # Service management
    provisioning = relationship(
        "ServiceProvisioning", back_populates="customer_service", uselist=False
    )
    ip_assignments = relationship(
        "ServiceIPAssignment", back_populates="customer_service"
    )
    status_history = relationship(
        "ServiceStatusHistory", back_populates="customer_service"
    )
...
class CustomerInternetService(Base):
    ...
    def monthly_usage_percent(self):
        ...
        return min(
            100, (float(self.monthly_usage_gb) / template.monthly_data_limit_gb) * 100
        )
...
Index(
    "idx_customer_services_customer_status",
