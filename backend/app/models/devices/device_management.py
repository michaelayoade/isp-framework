...
class ManagedDevice(Base):
    ...
    monitoring_data = relationship(
        "DeviceMonitoring", back_populates="device", cascade="all, delete-orphan"
    )
    config_backups = relationship(
        "DeviceConfigBackup", back_populates="device", cascade="all, delete-orphan"
    )
    alerts = relationship(
        "DeviceAlert", back_populates="device", cascade="all, delete-orphan"
    )
...
class DeviceInterface(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    device_id = Column(
        Integer, ForeignKey("managed_devices.id"), nullable=False, index=True
    )
...
class DeviceAlert(Base):
    ...
    )  # cpu_high, interface_down, device_unreachable
...
Index(
    "idx_managed_devices_type_status", ManagedDevice.device_type, ManagedDevice.status
)
Index(
    "idx_device_monitoring_device_metric",
    DeviceMonitoring.device_id,
    DeviceMonitoring.metric_name,
)
...
Index(
    "idx_device_config_backups_device_status",
    DeviceConfigBackup.device_id,
    DeviceConfigBackup.backup_status,
)
