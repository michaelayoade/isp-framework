...
class FieldWorkOrder(Base):
    ...
    # Work Details
    work_type = Column(String(100), nullable=False)      # installation, repair, maintenance, survey
    ...
    # Scheduling
    status = Column(Enum(FieldWorkStatus), default=FieldWorkStatus.SCHEDULED, index=True)
    priority = Column(String(20), default='normal')  # Using String to avoid circular import
    ...
    def is_overdue(self):
        ...
        if not self.scheduled_date or self.status in [FieldWorkStatus.COMPLETED, FieldWorkStatus.CANCELLED]:
            return False
