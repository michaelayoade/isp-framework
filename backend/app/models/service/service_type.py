...
class ServiceType(Base):
    ...
    # Service properties
    requires_installation = Column(
        Boolean, default=True
    )  # Does this service need installation
    supports_bandwidth_limits = Column(
        Boolean, default=True
    )  # Can bandwidth be limited
    is_recurring = Column(Boolean, default=True)  # Is this a recurring service
    allows_multiple_instances = Column(
        Boolean, default=False
    )  # Can customer have multiple of this type

    # Billing properties
    default_billing_cycle = Column(
        String(20), default="monthly"
    )  # monthly, quarterly, yearly
    requires_equipment = Column(
        Boolean, default=False
    )  # Does service require equipment rental

    # UI properties
    color_hex = Column(
        String(7), default="#6B7280"
    )  # Color for service type badges (#RRGGBB)
    ...
    customer_services = relationship(
        "CustomerService", back_populates="service_type_ref"
    )
