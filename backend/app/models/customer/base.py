...
class Customer(Base):
    ...
    )  # Portal ID for RADIUS/portal/PPPoE auth
    ...
    )  # Optional - customers can be direct or via reseller
    ...
    extended_info = relationship(
        "CustomerExtended", back_populates="customer", uselist=False
    )
    info = relationship("CustomerInfo", back_populates="customer", uselist=False)
    billing_config = relationship(
        "CustomerBilling", back_populates="customer", uselist=False
    )
    contacts = relationship("CustomerContact", back_populates="customer")
    label_associations = relationship(
        "CustomerLabelAssociation", back_populates="customer"
    )
    documents = relationship("CustomerDocument", back_populates="customer")
    billing_account = relationship(
        "CustomerBillingAccount", back_populates="customer", uselist=False
    )
    ...
    communication_preferences = relationship(
        "CommunicationPreference", back_populates="customer", uselist=False
    )
    ...
    files = relationship(
        "FileMetadata", back_populates="customer", cascade="all, delete-orphan"
    )
...
class CustomerLabel(Base):
    ...
    # Relationships
    label_associations = relationship(
        "CustomerLabelAssociation", back_populates="label"
    )
...
class CustomerLabelAssociation(Base):
    ...
    label_id = Column(
        Integer, ForeignKey("customer_labels.id", ondelete="CASCADE"), primary_key=True
    )
...
class CustomerDocument(Base):
    ...
    file_id = Column(
        Integer, ForeignKey("file_storage.id", ondelete="CASCADE"), nullable=False
    )

    # Document metadata
    document_type = Column(
        String(50), default="general"
    )  # id_card, contract, invoice, etc.
    title = Column(String(255), nullable=False)
    description = Column(Text)
...
class CustomerNote(Base):
    ...
    note_type = Column(
        String(50), default="general"
    )  # general, billing, technical, support
...
class CustomerTrafficCounter(Base):
    ...
    service_id = Column(
        Integer,
        ForeignKey(
            "customer_internet_service_instances.customer_service_id",
            ondelete="CASCADE",
        ),
        primary_key=True,
    )
    ...
    # Relationships
    internet_service = relationship(
        "CustomerInternetService",
        back_populates="traffic_counters",
        overlaps="customer_service,traffic_counters",
    )
...
class CustomerBonusTrafficCounter(Base):
    ...
    internet_service = relationship(
        "CustomerInternetService",
        back_populates="bonus_traffic_counters",
        overlaps="customer_service,bonus_traffic_counters",
    )
