...
from sqlalchemy import Column, Integer, String, Text, Boolean, DateTime, DECIMAL, ForeignKey, ARRAY, Index
...
class CustomerPortalPayment(Base):
    ...
    payment_method = Column(String(100), nullable=False)  # credit_card, bank_transfer, mobile_money
...
class CustomerPortalAutoPayment(Base):
    ...
    # Payment Settings
    auto_pay_amount_type = Column(String(50), default='full_balance')  # full_balance, minimum_due, fixed_amount
    ...
    require_confirmation = Column(Boolean, default=False) # Require customer confirmation
...
class CustomerPortalServiceRequest(Base):
    ...
    # Request Information
    request_type = Column(String(100), nullable=False)  # upgrade, downgrade, add_service, cancel_service, suspend_service
    ...
    # Status
    status = Column(String(100), default='submitted')   # submitted, reviewing, approved, rejected, scheduled, completed
...
class CustomerPortalUsageView(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    customer_id = Column(Integer, ForeignKey("customers.id", ondelete="CASCADE"), nullable=False, index=True)
    service_id = Column(Integer, ForeignKey("customer_services.id", ondelete="CASCADE"), nullable=False, index=True)
    
    # Usage Period
    usage_period = Column(String(50), nullable=False)   # daily, weekly, monthly, billing_cycle
    ...
    top_categories = Column(JSONB, default=[])          # [{"category": "streaming", "usage_mb": 1500}, ...]
...
class CustomerPortalNotification(Base):
    ...
    notification_type = Column(String(100), nullable=False)  # ticket_update, payment_due, service_alert, etc.
...
# Performance indexes
Index('idx_portal_sessions_customer', CustomerPortalSession.customer_id, CustomerPortalSession.is_active)
Index('idx_portal_sessions_token', CustomerPortalSession.session_token)
Index('idx_portal_payments_customer', CustomerPortalPayment.customer_id, CustomerPortalPayment.status)
Index('idx_portal_payments_reference', CustomerPortalPayment.payment_reference)
Index('idx_portal_payments_status', CustomerPortalPayment.status, CustomerPortalPayment.created_at)
Index('idx_portal_service_requests_customer', CustomerPortalServiceRequest.customer_id, CustomerPortalServiceRequest.status)
Index('idx_portal_service_requests_status', CustomerPortalServiceRequest.status, CustomerPortalServiceRequest.created_at)
...
Index('idx_portal_usage_customer_period', CustomerPortalUsageView.customer_id, CustomerPortalUsageView.usage_period)
Index('idx_portal_usage_service_period', CustomerPortalUsageView.service_id, CustomerPortalUsageView.period_start)
Index('idx_portal_notifications_customer', CustomerPortalNotification.customer_id, CustomerPortalNotification.is_read)
Index('idx_portal_notifications_type', CustomerPortalNotification.notification_type, CustomerPortalNotification.created_at)
