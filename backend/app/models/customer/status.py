...
class CustomerStatus(Base):
    ...
    # Workflow properties
    allows_service_activation = Column(
        Boolean, default=True
    )  # Can services be activated in this status
    allows_billing = Column(
        Boolean, default=True
    )  # Should billing continue in this status
    is_terminal = Column(
        Boolean, default=False
    )  # Is this a final status (terminated, cancelled)
