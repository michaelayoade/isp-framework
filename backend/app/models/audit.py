...
class ConfigurationSnapshot(Base):
    ...
    )  # billing, network, service, etc.
    config_key = Column(
        String(100), nullable=False, index=True
    )  # specific config identifier
...
class CDCLog(Base):
    ...
    )  # application, migration, system, etc.
...
class AuditProcessingStatus(Base):
    ...
    def get_processing_lag(self) -> Optional[int]:
        ...
        if self.last_processed_at:
            return int(
                (datetime.now(timezone.utc) - self.last_processed_at).total_seconds()
            )
        return None
...
def get_audit_processing_stats(session) -> Dict[str, Any]:
    ...
    stats = {
        ...
        "processing_count": session.query(AuditQueue)
        .filter_by(status="processing")
        .count(),
        "completed_count": session.query(AuditQueue)
        .filter_by(status="completed")
        .count(),
        "failed_count": session.query(AuditQueue).filter_by(status="failed").count(),
        "total_count": session.query(AuditQueue).count(),
        "active_processors": session.query(AuditProcessingStatus)
        .filter_by(status="active")
        .count(),
        "error_processors": session.query(AuditProcessingStatus)
    ...
    if stats["total_count"] > 0:
        stats["completion_rate"] = round(
            (stats["completed_count"] / stats["total_count"]) * 100, 2
        )
        stats["failure_rate"] = round(
            (stats["failed_count"] / stats["total_count"]) * 100, 2
        )
    else:
...
def get_configuration_history(
    session, config_type: str, config_key: str, limit: int = 10
):
...
def get_recent_changes(
    session, table_name: str = None, hours: int = 24, limit: int = 100
):
    """Get recent changes from CDC log."""
    cutoff_time = datetime.now(timezone.utc) - timedelta(hours=hours)

    query = (
        session.query(CDCLog)
