...
class TicketStatusBase(BaseModel):
    ...
    # Workflow properties
    is_initial: bool = Field(False, description="Is this a starting status for new tickets")
    is_final: bool = Field(False, description="Is this a final status")
    requires_assignment: bool = Field(False, description="Must ticket be assigned to proceed")
    auto_close_after_days: Optional[int] = Field(None, description="Auto-close after N days")
    
    # Customer visibility
    is_customer_visible: bool = Field(True, description="Should customers see this status")
    customer_description: Optional[str] = Field(None, max_length=255, description="Customer-friendly description")
    ...
    # UI properties
    color_hex: str = Field("#6B7280", max_length=7, description="Color for status badges (#RRGGBB)")
