...
class CustomerBase(BaseModel):
    ...
    gps: Optional[str] = Field(
        None, max_length=100, description="GPS coordinates (latitude,longitude)"
    )
...
class Customer(CustomerBase):
    ...
    id: int
    portal_id: str = Field(
        ..., description="Portal ID for RADIUS/portal/PPPoE authentication"
    )
    status_id: int
...
class CustomerContactBase(BaseModel):
    ...
    position: Optional[str] = Field(
        None, max_length=100, description="Job title/position"
    )
    ...
    receive_technical: bool = Field(
        False, description="Receive technical notifications"
    )
...
class CustomerExtendedBase(BaseModel):
    ...
    registration_number: Optional[str] = Field(
        None, max_length=100, description="Registration number"
    )
    ...
    dashboard_layout: Optional[Dict[str, Any]] = Field(
        None, description="Dashboard layout preferences"
    )
