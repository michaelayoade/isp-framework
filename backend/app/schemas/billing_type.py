...
class BillingTypeBase(BaseModel):
    ...
    supports_prorating: bool = Field(
        True, description="Can be prorated for partial periods"
    )
    default_cycle_days: int = Field(30, description="Default billing cycle in days")

    # Payment properties
    allows_partial_payments: bool = Field(
        False, description="Can customer pay partially"
    )
    ...
    # Credit properties
    supports_credit_limit: bool = Field(
        False, description="Does this type support credit limits"
    )
    default_credit_limit: int = Field(0, description="Default credit limit (in cents)")

    # UI properties
    color_hex: str = Field(
        "#6B7280", max_length=7, description="Color for billing type badges (#RRGGBB)"
    )
