...
class VoiceServiceBase(BaseModel):
    ...
    phone_number: Optional[str] = Field(
        None, max_length=20, description="Assigned phone number"
    )
    call_plan_type: CallPlanType = Field(..., description="Type of call plan")
    monthly_minutes: Optional[int] = Field(
        None, ge=0, description="Monthly minute allowance (null for unlimited)"
    )
    international_calling: bool = Field(
        False, description="International calling enabled"
    )
...
class VoiceServiceCreate(VoiceServiceBase):
    ...
    preferred_area_code: Optional[str] = Field(
        None, max_length=10, description="Preferred area code for number assignment"
    )
    scheduled_activation: Optional[datetime] = Field(
        None, description="Scheduled activation date"
    )
...
class VoiceServiceUpdate(BaseModel):
    ...
    call_plan_type: Optional[CallPlanType] = Field(
        None, description="Type of call plan"
    )
    monthly_minutes: Optional[int] = Field(
        None, ge=0, description="Monthly minute allowance"
    )
    international_calling: Optional[bool] = Field(
        None, description="International calling enabled"
    )
    voicemail_enabled: Optional[bool] = Field(
        None, description="Voicemail service enabled"
    )
    call_forwarding_enabled: Optional[bool] = Field(
        None, description="Call forwarding enabled"
    )
    caller_id_enabled: Optional[bool] = Field(
        None, description="Caller ID service enabled"
    )
    notes: Optional[str] = Field(None, max_length=1000, description="Service notes")
...
class VoiceServiceProvisioningRequest(BaseModel):
    ...
    priority: str = Field(
        "normal", description="Provisioning priority: low, normal, high, urgent"
    )
    scheduled_for: Optional[datetime] = Field(
        None, description="Schedule provisioning for specific time"
    )
    auto_activate: bool = Field(
        True, description="Automatically activate after successful provisioning"
    )
...
class VoiceService(VoiceServiceBase):
    ...
    sip_username: Optional[str] = Field(
        None, description="SIP username for authentication"
    )
    ...
    activation_date: Optional[datetime] = Field(
        None, description="Service activation date"
    )
    suspension_date: Optional[datetime] = Field(
        None, description="Service suspension date"
    )
    termination_date: Optional[datetime] = Field(
        None, description="Service termination date"
    )
    created_at: datetime
...
class VoiceServiceUsage(BaseModel):
    ...
    international_minutes: int = Field(
        0, ge=0, description="International call minutes"
    )
...
class VoiceServiceAnalytics(BaseModel):
    ...
    minute_limit_utilization: Optional[float] = Field(
        None, description="Percentage of minute limit used"
    )
    average_call_duration: Optional[float] = Field(
        None, description="Average call duration in minutes"
    )
    peak_usage_hour: Optional[int] = Field(
        None, ge=0, le=23, description="Peak usage hour of day"
    )
    service_quality_score: Optional[float] = Field(
        None, ge=0, le=10, description="Service quality score"
    )
