...
class ServiceTypeBase(BaseSchema):
    ...
    # Service properties
    requires_installation: bool = Field(True, description="Does this service need installation")
    supports_bandwidth_limits: bool = Field(True, description="Can bandwidth be limited")
    is_recurring: bool = Field(True, description="Is this a recurring service")
    allows_multiple_instances: bool = Field(False, description="Can customer have multiple of this type")
    
    # Billing properties
    default_billing_cycle: str = Field("monthly", max_length=20, description="Default billing cycle")
    requires_equipment: bool = Field(False, description="Does service require equipment rental")
    
    # UI properties
    color_hex: str = Field("#6B7280", max_length=7, description="Color for service type badges (#RRGGBB)")
    icon_name: Optional[str] = Field(None, max_length=50, description="Icon identifier for UI")
    
    # Technical properties
    provisioning_template: Optional[str] = Field(None, max_length=100, description="Template identifier for automation")
    qos_profile: Optional[str] = Field(None, max_length=50, description="Default QoS profile")
