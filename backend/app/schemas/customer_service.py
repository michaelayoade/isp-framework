...
class CustomerServiceBase(BaseModel):
    ...
    status: str = Field(
        "active", description="Service status: active, suspended, terminated, pending"
    )
    start_date: Optional[datetime] = Field(
        None, description="Service start date (defaults to now)"
    )
    end_date: Optional[datetime] = Field(
        None, description="Service end date (null for ongoing)"
    )
    custom_price: Optional[int] = Field(
        None, description="Custom price override in cents"
    )
    discount_percentage: Optional[int] = Field(
        0, ge=0, le=100, description="Discount percentage (0-100)"
    )
...
class ServiceProvisioningRequest(BaseModel):
    ...
    notes: Optional[str] = Field(
        None, max_length=1000, description="Provisioning notes"
    )
