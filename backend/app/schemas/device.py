...
class DeviceBase(BaseModel):
    ...
    device_type: Optional[str] = Field(
        None, max_length=50, description="Device type (router, switch, camera, etc.)"
    )
...
class DeviceGroupBase(BaseModel):
    ...
    auto_approve_devices: bool = Field(
        False, description="Auto-approve new devices in this group"
    )
    default_device_status: str = Field("pending", pattern="^(pending|active|blocked)$")
    bandwidth_limit_mbps: Optional[int] = Field(
        None, ge=1, description="Bandwidth limit in Mbps"
    )
...
class DeviceGroupUpdate(BaseModel):
    ...
    default_device_status: Optional[str] = Field(
        None, pattern="^(pending|active|blocked)$"
    )
...
class ServiceMacAuthSettings(BaseModel):
    ...
    auto_register_mac: bool = Field(
        False, description="Auto-register unknown MAC addresses"
    )
...
class RadiusDeviceResponse(BaseModel):
    ...
    bandwidth_limit_down: Optional[int] = Field(
        None, description="Download bandwidth limit in kbps"
    )
    bandwidth_limit_up: Optional[int] = Field(
        None, description="Upload bandwidth limit in kbps"
    )
    ...
    rejection_reason: Optional[str] = Field(
        None, description="Reason for rejection if access denied"
    )
...
class DeviceBulkApproval(BaseModel):
    ...
    approval_reason: Optional[str] = Field(
        None, description="Reason for bulk approval/rejection"
    )
...
class DeviceSort(BaseModel):
    ...
    field: str = Field(
        "last_seen", pattern="^(name|mac_address|status|last_seen|created_at)$"
    )
