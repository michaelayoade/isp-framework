...
class InternetServiceSpeed(BaseModel):
    ...
    burst_download_mbps: Optional[int] = Field(
        None, ge=1, description="Burst download speed in Mbps"
    )
    burst_upload_mbps: Optional[int] = Field(
        None, ge=1, description="Burst upload speed in Mbps"
    )
...
class InternetServiceBase(BaseModel):
    ...
    data_limit_gb: Optional[int] = Field(
        None, ge=0, description="Monthly data limit in GB (null for unlimited)"
    )
    static_ip_required: bool = Field(False, description="Whether static IP is required")
    installation_address: Optional[str] = Field(
        None, max_length=500, description="Installation address"
    )
...
class InternetServiceUpdate(BaseModel):
    ...
    speed_config: Optional[InternetServiceSpeed] = Field(
        None, description="Speed configuration"
    )
    data_limit_gb: Optional[int] = Field(
        None, ge=0, description="Monthly data limit in GB"
    )
    static_ip_required: Optional[bool] = Field(
        None, description="Whether static IP is required"
    )
    installation_address: Optional[str] = Field(
        None, max_length=500, description="Installation address"
    )
...
class InternetServiceUsage(BaseModel):
    ...
    peak_download_mbps: Optional[float] = Field(
        None, description="Peak download speed recorded"
    )
    peak_upload_mbps: Optional[float] = Field(
        None, description="Peak upload speed recorded"
    )
    session_count: int = Field(0, ge=0, description="Number of sessions")
    total_session_time: int = Field(
        0, ge=0, description="Total session time in seconds"
    )
...
class InternetServiceAnalytics(BaseModel):
    ...
    data_limit_utilization: Optional[float] = Field(
        None, description="Percentage of data limit used"
    )
    average_speed_mbps: Optional[float] = Field(
        None, description="Average connection speed"
    )
    uptime_percentage: float = Field(
        ..., ge=0, le=100, description="Service uptime percentage"
    )
    last_online: Optional[datetime] = Field(
        None, description="Last time service was online"
    )
