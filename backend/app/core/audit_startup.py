...
from app.core.audit_config import (
    configure_audit_tracking,
    get_audit_configuration_status,
)
...
class AuditSystemStartup:
    ...
    async def initialize_audit_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log_initialization_summary(
                model_config_result, processor_result, health_result
            )
    ...
    def _configure_model_auditing(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if config_result["success"]:
                logger.info(
                    f"Successfully enabled audit tracking for {config_result['total_enabled']} models"
                )
            else:
    ...
    async def _start_audit_processors(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "processor_stats": processor_stats}
    ...
    def _log_initialization_summary(
        self, model_result, processor_result, health_result
    ):
        ...
        if model_result["success"]:
            logger.info(
                f"✅ Model Configuration: {model_result['enabled_count']} models enabled"
            )
        else:
            logger.warning(
                f"⚠️  Model Configuration: {model_result['failed_count']} failures"
            )
        ...
        if len(self.startup_errors) == 0:
            logger.info(
                "🎉 Enhanced Audit System: Fully operational and ready for production"
            )
        else:
            logger.warning(
                f"⚠️  Enhanced Audit System: Operational with {len(self.startup_errors)} warnings"
            )
...
async def audit_startup_event():
    ...
    try:
        ...
        if result["success"]:
            logger.info(
                "FastAPI startup: Enhanced audit system initialized successfully"
            )
        else:
            logger.warning(
                "FastAPI startup: Enhanced audit system initialized with warnings"
            )

    except Exception as e:
        logger.error(
            f"FastAPI startup: Failed to initialize enhanced audit system: {e}"
        )
