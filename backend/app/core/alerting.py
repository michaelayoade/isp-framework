...
billing_errors = Counter(
    'isp_billing_errors_total',
    'Billing system errors',
    ['error_type', 'customer_id']
)
...
class GrafanaAlertManager:
    ...
    def _update_metrics(self, error_detail: ErrorDetail):
        ...
        if error_detail.category == ErrorCategory.NETWORK:
            ...
            billing_errors.labels(
                error_type=error_detail.title,
                customer_id=str(error_detail.customer_id) if error_detail.customer_id else 'unknown'
            ).inc()
    ...
    async def _send_communication_alerts(self, rule: AlertRule, error_detail: ErrorDetail):
        """Send alerts via communications module."""
        try:
            ...
            for channel in rule.channels:
                ...
                if channel == AlertChannel.EMAIL:
                    await self._send_email_alert(
                        comm_service, alert_title, alert_message, recipients, error_detail
                    )
    ...
    def _get_alert_recipients(self, channel: AlertChannel, error_detail: ErrorDetail) -> List[str]:
        ...
        if channel == AlertChannel.EMAIL:
            recipients = getattr(settings, 'ALERT_EMAIL_RECIPIENTS', ['admin@ispframework.com'])
            
            # Add customer-specific recipients for customer-facing errors
            if error_detail.impact == ErrorImpact.CUSTOMER_FACING and error_detail.customer_id:
    ...
    async def _send_email_alert(self, comm_service, title: str, message: str, recipients: List[str], error_detail: ErrorDetail):
    ...
    async def _send_webhook_alert(self, comm_service, title: str, message: str, error_detail: ErrorDetail):
    ...
    async def _send_sms_alert(self, comm_service, title: str, message: str, recipients: List[str], error_detail: ErrorDetail):
