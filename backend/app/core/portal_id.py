...
class PortalIDGenerator:
    ...
    def generate_portal_id(cls, customer_id: int, prefix: Optional[str] = None) -> str:
        ...
        if not isinstance(customer_id, int) or customer_id < cls.MIN_CUSTOMER_ID:
            raise ValidationError(f"Customer ID must be a positive integer >= {cls.MIN_CUSTOMER_ID}")
        ...
        if not re.match(r'^[A-Z0-9]+$', prefix):
            raise ValidationError("Portal ID prefix must contain only uppercase letters and numbers")
            
        portal_id = f"{prefix}{customer_id}"
        
        # Final validation
        if not cls.validate_portal_id_format(portal_id):
            raise ValidationError(f"Generated portal ID '{portal_id}' has invalid format")
...
class PortalIDService:
    ...
    def get_portal_config(self, partner_id: int = 1, service_type: str = "internet") -> Dict[str, Any]:
        """
    ...
    def generate_customer_portal_id(self, customer_id: int, partner_id: int = 1, 
                                  service_type: str = "internet") -> str:
        """
    ...
    def validate_portal_id_uniqueness(self, portal_id: str, exclude_customer_id: Optional[int] = None) -> bool:
        """
    ...
    def update_customer_portal_id(self, customer_id: int, new_portal_id: Optional[str] = None) -> str:
        """
        ...
        if new_portal_id is None:
            new_portal_id = self.generate_customer_portal_id(customer_id, customer.partner_id)
        else:
    ...
    def _log_portal_id_change(self, customer_id: int, old_portal_id: str, new_portal_id: str, 
                             changed_by: Optional[int] = None, change_reason: str = None):
    ...
    def create_portal_config(self, config_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if existing:
            raise ValidationError(f"Prefix '{prefix}' already exists for partner {config_data.get('partner_id', 1)}")
    ...
    def update_portal_config(self, config_id: int, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """Update portal configuration"""
        from app.models.portal_config import PortalConfig
        
        config = self.db.query(PortalConfig).filter(PortalConfig.id == config_id).first()
        if not config:
    ...
    def get_portal_id_history(self, customer_id: Optional[int] = None, limit: int = 100) -> List[Dict[str, Any]]:
