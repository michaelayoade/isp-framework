...
from app.models import (  # Customer Management Models; Network Models; Foundation Models; Authentication Models; Ticketing Models; Portal Models
    Administrator,
    ApiKey,
    Customer,
    CustomerExtended,
...
def enable_critical_model_auditing():
    ...
    logger.info(
        f"Audit tracking configuration complete: {enabled_count} enabled, {failed_count} failed"
    )
...
def enable_standard_model_auditing():
    ...
    logger.info(
        f"Standard model audit tracking complete: {enabled_count} enabled, {failed_count} failed"
    )
    return enabled_count, failed_count
...
def disable_excluded_model_auditing():
    ...
    logger.info(
        f"Excluded model audit configuration complete: {disabled_count} disabled"
    )
...
def configure_audit_tracking():
    ...
    try:
        ...
        if total_failed > 0:
            logger.warning(
                f"Some models failed to enable audit tracking: {total_failed}"
            )
