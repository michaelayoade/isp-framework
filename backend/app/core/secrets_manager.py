...
class SecretsManager:
    ...
    def __init__(self):
        ...
        if self.backend not in ["environment", "encrypted_file"]:
            logger.warning(
                f"Cloud backend '{self.backend}' not supported in local mode, falling back to 'environment'"
            )
        ...
        logger.info(
            "Local secrets manager initialized",
            backend=self.backend,
            encryption_enabled=bool(self.cipher_suite),
        )
    ...
    def _set_to_encrypted_file(self, key: str, value: Any) -> bool:
        ...
        try:
            ...
            log_audit_event(
                domain="secrets", event="secret_stored_file", secret_key=key
            )
    ...
    def rotate_encryption_key(self) -> bool:
        ...
        try:
            ...
            log_audit_event(domain="secrets", event="encryption_key_rotated")
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        health = {"backend": self.backend, "status": "unknown", "details": {}}

        try:
            if self.backend == "environment":
                health["status"] = "healthy"
                health["details"]["encryption_available"] = (
                    self.cipher_suite is not None
                )
                ...
                if not self.cipher_suite:
                    ...
                    secrets_file = getattr(
                        settings, "ENCRYPTED_SECRETS_FILE", "secrets.enc"
                    )
