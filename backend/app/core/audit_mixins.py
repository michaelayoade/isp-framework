...
class EnhancedAuditMixin:
    ...
    @declared_attr
    def updated_at(cls):
        return Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
...
class AuditEventCapture:
    ...
    def capture_change(
        ...
        try:
            ...
            logger.debug(f"Audit event captured: {table_name}.{record_id} - {operation}")
...
class ConfigurationVersioning:
    ...
    def create_snapshot(
        ...
        try:
            ...
            latest_version = session.query(ConfigurationSnapshot)\
            ...
            session.query(ConfigurationSnapshot)\
                .filter_by(config_type=config_type, config_key=config_key, is_active=True)\
            ...
            logger.info(f"Configuration snapshot created: {config_type}.{config_key} v{next_version}")
            return next_version
    ...
    def get_active_config(
        ...
        try:
            ...
            snapshot = session.query(ConfigurationSnapshot)\
                .filter_by(config_type=config_type, config_key=config_key, is_active=True)\
                .first()
        ...
        if hasattr(target, '__audit_enabled__') and target.__audit_enabled__:
            ...
            for column in mapper.columns:
                ...
                if value is not None:
                    new_values[column.name] = str(value) if not isinstance(value, (str, int, float, bool)) else value
            ...
            for attr in state.attrs:
                if attr.history.has_changes():
                    ...
                    if old_value is not None:
                        old_values[attr.key] = str(old_value) if not isinstance(old_value, (str, int, float, bool)) else old_value
                    if new_value is not None:
                        new_values[attr.key] = str(new_value) if not isinstance(new_value, (str, int, float, bool)) else new_value
    ...
    def before_delete(mapper, connection, target):
        ...
        if hasattr(target, '__audit_enabled__') and target.__audit_enabled__:
            ...
            for column in mapper.columns:
                value = getattr(target, column.name, None)
                if value is not None:
                    old_values[column.name] = str(value) if not isinstance(value, (str, int, float, bool)) else value
