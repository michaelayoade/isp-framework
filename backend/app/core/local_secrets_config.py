...
class LocalSecretsConfig:
    ...
    def setup_local_secrets(self, use_password: bool = False, password: str = None) -> dict:
    ...
    def validate_setup(self) -> dict:
        ...
        validation = {
            ...
            "secrets_dir_permissions": oct(self.secrets_dir.stat().st_mode)[-3:] if self.secrets_dir.exists() else None,
            "encrypted_file_exists": self.encrypted_file.exists(),
            "key_file_exists": self.key_file.exists(),
            "key_file_permissions": oct(self.key_file.stat().st_mode)[-3:] if self.key_file.exists() else None,
...
def setup_local_secrets_cli():
    ...
    parser.add_argument('--password', action='store_true', help='Use password-based key derivation')
    parser.add_argument('--migrate', action='store_true', help='Migrate environment secrets to file')
    ...
    if args.migrate:
        ...
        if migration["failed_secrets"]:
            ...
            for secret in migration["failed_secrets"]:
                print(f"  ❌ {secret['env_var']}: {secret.get('error', 'Unknown error')}")
