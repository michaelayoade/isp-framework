...
class AlertManager:
    ...
    async def _send_slack_alert(self, rule: AlertRule, error_detail: ErrorDetail):
            ...
            "attachments": [
                {
                    ...
                    "fields": [
                        ...
                        {
                            "title": "Impact",
                            "value": error_detail.impact.value.replace(
                                "_", " "
                            ).title(),
                            "short": True,
                        },
                        {
                            "title": "Timestamp",
                            "value": error_detail.timestamp.strftime(
                                "%Y-%m-%d %H:%M:%S UTC"
                            ),
                            "short": True,
                        },
        ...
        if error_detail.context:
            context_text = "\n".join(
                [f"• {k}: {v}" for k, v in error_detail.context.items()]
            )
            alert_message["attachments"][0]["fields"].append(
                {"title": "Context", "value": f"```{context_text}```", "short": False}
            )
    ...
    async def _send_pagerduty_alert(self, rule: AlertRule, error_detail: ErrorDetail):
        ...
        if (
            not hasattr(settings, "PAGERDUTY_INTEGRATION_KEY")
            or not settings.PAGERDUTY_INTEGRATION_KEY
        ):
            self.logger.warning(
                "PagerDuty integration key not configured, skipping PagerDuty alert"
            )
            return
