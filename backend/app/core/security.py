...
try:
    ...
    logger.warning(f"Redis connection failed: {e}. Token revocation will use database only.")
    redis_client = None
...
async def require_admin_access(current_user: dict = Security(get_current_user, scopes=["admin_portal"])):
    """Require admin portal access."""
    return current_user


async def require_api_access(current_user: dict = Security(get_current_user, scopes=["api"])):
    """Require general API access."""
    return current_user


async def require_billing_access(current_user: dict = Security(get_current_user, scopes=["billing"])):
    """Require billing system access."""
    return current_user


async def require_network_access(current_user: dict = Security(get_current_user, scopes=["network"])):
