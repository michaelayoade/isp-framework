    ...
    def rotate_key(self, force: bool = False) -> str:
        ...
        if not force and self.current_key_id:
            ...
            expires_at = datetime.fromisoformat(
                current_key["expires_at"].replace("Z", "+00:00")
            )
        ...
        if self.current_key_id and self.current_key_id in self.keys:
            ...
            self.keys[self.current_key_id]["deprecated_at"] = datetime.now(
                timezone.utc
            ).isoformat()
    ...
    def cleanup_expired_keys(self, keep_days: int = 30):
        ...
        for key_id, key_data in list(self.keys.items()):
            if key_data["status"] == "deprecated":
                deprecated_at = datetime.fromisoformat(
                    key_data.get("deprecated_at", key_data["created_at"]).replace(
                        "Z", "+00:00"
                    )
                )
...
async def scheduled_key_cleanup():
    ...
    try:
        ...
        if removed_keys:
            logger.info(
                "Scheduled key cleanup completed", removed_count=len(removed_keys)
            )
    except Exception as e:
