...
class EnhancedAuditMixin:
    ...
    def created_by_id(cls):
        return Column(
            Integer, ForeignKey("administrators.id"), nullable=True, index=True
        )
...
class EnhancedAuditLog(Base):
    ...
    )  # admin, system, api_client, customer
...
class ConfigurationSnapshot(Base):
    ...
    )  # manual, scheduled, pre_change, rollback
    ...
    previous_snapshot_id = Column(
        Integer, ForeignKey("configuration_snapshots.id"), nullable=True
    )
    ...
    previous_snapshot = relationship(
        "ConfigurationSnapshot", remote_side=[id], viewonly=True
    )
...
class AsyncAuditProcessor:
    ...
    async def process_audit_queue(batch_size: int = 100):
        ...
        try:
            ...
            pending_items = (
                db.query(AuditQueue)
                .filter(AuditQueue.status == "pending")
                .order_by(AuditQueue.priority.asc(), AuditQueue.created_at.asc())
                .limit(batch_size)
                .all()
            )
            ...
            for item in pending_items:
                try:
                    ...
                    if item.retry_count >= item.max_retries:
                        ...
                        item.status = "pending"
                        item.next_retry_at = datetime.now(timezone.utc) + timedelta(
...
async def create_enhanced_audit_log(
    ...
    audit_data = {
        ...
        "version_before": (
            getattr(instance, "version", None) - 1
        ...
        for attr in state.attrs:
            if attr.history.has_changes():
                ...
                new_value = (
                    ...
                    else getattr(instance, attr.key)
                )
...
class ConfigurationManager:
    ...
    def rollback_to_snapshot(
        snapshot_id: int, rolled_back_by_id: Optional[int] = None
    ) -> bool:
        ...
        try:
            snapshot = (
                db.query(ConfigurationSnapshot)
                .filter(ConfigurationSnapshot.id == snapshot_id)
                .first()
            )
