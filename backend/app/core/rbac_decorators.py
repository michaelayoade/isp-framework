...
from app.services.rbac_service import Permission, RBACService, ResourceType
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                rbac_service.enforce_permission(
                    current_admin, permission, resource_type, resource_id
                )
                ...
                logger.warning(
                    f"Permission denied for user {current_admin.username}: {e}"
                )
                raise HTTPException(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.warning(
                    f"Ownership check failed for user {current_admin.username}: {e}"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail=str(e)
                )
            ...
            try:
                ...
                if isinstance(result, dict):
                    filtered_result = rbac_service.validate_data_access(
                        current_admin, result, resource_type
                    )
                    ...
                    for item in result:
                        if isinstance(item, dict):
                            filtered_item = rbac_service.validate_data_access(
                                current_admin, item, resource_type
                            )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if not rbac_service.check_multi_tenant_isolation(
                    current_admin, target_user_id
                ):
                    raise PermissionDeniedError(
...
def get_filtered_query(
    ...
    def filter_query(query):
        return rbac_service.filter_query_by_ownership(
            query, current_admin, resource_type
        )
