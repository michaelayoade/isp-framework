    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not has_permission:
                logger.warning(
                    f"Permission denied: User {current_admin.username} lacks '{permission_code}' permission"
                )
                raise HTTPException(
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not has_any_permission:
                logger.warning(
                    f"Permission denied: User {current_admin.username} lacks any of {permission_codes} permissions"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not has_admin_role:
                logger.warning(
                    f"Admin role required: User {current_admin.username} lacks administrative role"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
